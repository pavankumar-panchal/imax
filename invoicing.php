	...
	case 'proceedforpurchase': {
			...
			for ($i = 1; $i < count($productamountsplit); $i++) {
				if ($productamountsplit[$i] == '') {
					echo (json_encode('2^Invalid Entry1^' . $productamountvalues));
					exit;
				}
			}
			...
			for ($i = 0; $i < count($descriptiontypevaluessplit); $i++) {
				if ($descriptiontypevaluessplit[$i] <> '') {
					if ($descriptionvaluesplit[$i] == '') {
						echo (json_encode('2^Invalid Entry2^' . $descriptiontypevalues));
						exit;
					}
					if ($descriptionamountvaluessplit[$i] == '') {
						echo (json_encode('2^Invalid Entry3^' . $descriptionamountvalues));
						exit;
					}
				}
			}
...
left join inv_mas_branch on inv_mas_branch.slno = inv_mas_district.branchid where inv_mas_district.districtcode = '" . $dealerdistrict . "'";
			...
			$backdateddate = strtotime('2013-03-30');
			$expirydate = strtotime('2013-04-03');

			if ($invoicedated == 'yes') {
				if ($expirydate > $currentdate)
					$invoicedate = '2013-03-30 23:55:00';
				else
					$invoicedate = date('Y-m-d') . ' ' . date('H:i:s');
			...
			#########  Mailing Starts -----------------------------------
			if (($_SERVER['HTTP_HOST'] == "192.168.2.50") || ($_SERVER['HTTP_HOST'] == "bhavesh") || ($_SERVER['HTTP_HOST'] == "archanaab")) {
				$emailid = 'nagamani.b@relyonsoft.com';
			} else {
			...
			if (($_SERVER['HTTP_HOST'] == "192.168.2.50") || ($_SERVER['HTTP_HOST'] == "bhavesh") || ($_SERVER['HTTP_HOST'] == "archanaab")) {
				$dealeremailid = 'bhavesh.d@relyonsoft.com';
			} else {
	...
	left join inv_mas_branch on inv_mas_branch.slno = inv_mas_district.branchid where inv_mas_district.districtcode = '" . $district . "'";
			...
			$bcceallemailid = 'bigmail@relyonsoft.com,bills@relyonsoft.com,vijaykumar@relyonsoft.com';
