...
switch($switchtype)
{
	case 'save':
	{
		...
		if($lastslno == '')
		{
			...
			$query = "INSERT INTO inv_mas_receipt(dealerinvoiceno,receiptamount,paymentmode,receiptremarks,privatenote,createddate,createdby,createdip,lastmodifieddate,lastmodifiedby,lastmodifiedip,customerreference,chequedate,chequeno,drawnon,depositdate,receiptdate,receipttime,module) values('".$invoiceno."','".$receiptamount."','".$paymentmode."','".$receiptremarks."','".$privatenote."','".datetimelocal('Y-m-d').' '.datetimelocal('H:i:s')."','".$userid."','".$_SERVER['REMOTE_ADDR']."','".datetimelocal('Y-m-d').' '.datetimelocal('H:i:s')."','".$userid."','".$_SERVER['REMOTE_ADDR']."','".$dealerslno."','".$chequedate."','".$chequeno."','".$drawnon."','".$depositdate."','".$receiptdate."','".datetimelocal('H:i:s')."','user_module');";
			$result = runmysqlquery($query);
			$eventquery = "Insert into inv_logs_event(userid,system,eventtype,eventdatetime,remarks) values('".$userid."','".$_SERVER['REMOTE_ADDR']."','262','".date('Y-m-d').' '.date('H:i:s')."','".$dealerslno."')";
	...
	case 'delete':
	{
		$lastslno = $_POST['lastslno'];
		$dealerslno = $_POST['dealerslno'];
		$query = "Delete from inv_mas_receipt where slno = '".$lastslno."';";
		$result = runmysqlquery($query);
		$eventquery = "Insert into inv_logs_event(userid,system,eventtype,eventdatetime,remarks) values('".$userid."','".$_SERVER['REMOTE_ADDR']."','264','".date('Y-m-d').' '.date('H:i:s')."','".$dealerslno."')";
		$eventresult = runmysqlquery($eventquery);
		echo(json_encode("1^"."Receipt Deleted Successfully"));
	}
	break;
	case 'generatedealerlist':
	{
		...
		$customerarray = array();
		$query = "SELECT slno,businessname FROM inv_mas_dealer ORDER BY businessname  LIMIT ".$startindex.",".$limit.";";
		$result = runmysqlquery($query);
		$grid = '';
		$count = 0;
	...
	case 'getdealercount':
	{
		...
		$query = "SELECT count(*) as count FROM inv_mas_dealer ORDER BY businessname";
	...
	case 'gridtoform':
	{
		...
		$resultfetch = runmysqlqueryfetch($query);
		$query1 = "select sum(receiptamount) as receiptamount  from inv_mas_receipt where dealerinvoiceno = '".$resultfetch['dealerinvoiceno']."' and restatus <> 'CANCELLED'";
		$fetch1 = runmysqlqueryfetch($query1);
		$receiptamount = $fetch1['receiptamount'];


		if($resultfetch['module'] == 'user_module' || $resultfetch['module'] == 'Online')
			$querypiece = "select inv_mas_receipt.slno,sum(inv_mas_receipt.receiptamount) as receiptamount,
			inv_mas_receipt.dealerinvoiceno,inv_mas_receipt.receiptremarks,
			inv_mas_receipt.privatenote,inv_mas_receipt.paymentmode,inv_mas_receipt.createddate,
			inv_mas_receipt.chequedate,inv_mas_receipt.chequeno,inv_mas_receipt.drawnon,inv_mas_receipt.depositdate,
			inv_mas_receipt.receiptdate,inv_mas_receipt.createddate,inv_mas_users.fullname as createdby,
			inv_dealer_invoicenumbers.netamount as invoiceamount,inv_mas_receipt.restatus,inv_mas_receipt.cancelledremarks,
			inv_mas_receipt.cancelledby,inv_mas_receipt.cancelleddate,inv_mas_receipt.module,inv_mas_dealer.businessname from inv_mas_receipt 
			left join inv_dealer_invoicenumbers on inv_dealer_invoicenumbers.slno = inv_mas_receipt.dealerinvoiceno
 			left join inv_mas_users on inv_mas_users.slno = inv_mas_receipt.createdby
			left join inv_mas_dealer on inv_mas_dealer.slno = inv_mas_receipt.customerreference
			where inv_mas_receipt.slno = '".$lastslno."' group by inv_mas_receipt.slno;";
		else
			$querypiece = "select inv_mas_receipt.slno,sum(inv_mas_receipt.receiptamount)  as  receiptamount,
			inv_mas_receipt.dealerinvoiceno,inv_mas_receipt.receiptremarks,
			inv_mas_receipt.privatenote,inv_mas_receipt.paymentmode,inv_mas_receipt.createddate,
			inv_mas_receipt.chequedate,inv_mas_receipt.chequeno,inv_mas_receipt.drawnon,
			inv_mas_receipt.depositdate,inv_mas_receipt.receiptdate,inv_mas_receipt.createddate,
			inv_mas_dealer.businessname  as createdby,inv_dealer_invoicenumbers.netamount as invoiceamount
			from inv_mas_receipt 
			left join inv_dealer_invoicenumbers on inv_dealer_invoicenumbers.slno = inv_mas_receipt.dealerinvoiceno
			left join inv_mas_dealer on inv_mas_dealer.slno = inv_dealer_invoicenumbers.dealerid  
			where inv_mas_receipt.slno = '".$lastslno."' group by inv_mas_receipt.slno;";
		...
		if($fetch['module'] == 'Online')
		{
			$receiptamount = $fetch['receiptamount'];
			// $balanceamount = 0;
			$paid = 'yes';
			$createdby = $fetch['businessname'];
		}
		else{
		...
		$gridtoformarray['invoiceno'] = $fetch['dealerinvoiceno'];
		...
		$gridtoformarray['module'] = $fetch['module'];
		
		echo(json_encode($gridtoformarray));
		
		//echo('1^'.$fetch['slno'].'^'.$fetch['invoiceno'].'^'.$receiptamount.'^'.$fetch['invoiceamount'].'^'.$fetch['receiptremarks'].'^'.$fetch['privatenote'].'^'.$fetch['paymentmode'].'^'.changedateformat($fetch['chequedate']).'^'.$fetch['chequeno'].'^'.$fetch['drawnon'].'^'.changedateformat($fetch['depositdate']).'^'.changedateformat($fetch['receiptdate']).'^'.$balanceamount.'^'.changedateformatwithtime($fetch['createddate']).'^'.$fetch['createdby'].'^'.$paid.'^'.$resultfetch['invoiceamount'].'^'.$querypiece.'^'.$result22['status'].'^'.$statusremarks);
	}
	break;
	case 'generatereceiptgrid':
		{
			...
			$querycase = "select  * from inv_mas_receipt where customerreference = '".$customerreference."' and (inv_mas_receipt.dealerinvoiceno is NULL and inv_mas_receipt.invoiceno is NULL and inv_mas_receipt.module = 'Online')";
			...
			$resultcount = "select  inv_mas_receipt.slno as slno,
			inv_dealer_invoicenumbers.netamount,inv_mas_receipt.receiptamount ,inv_mas_receipt.createddate,
			inv_mas_dealer.businessname as createdby,
			inv_mas_receipt.receiptremarks,inv_mas_receipt.privatenote,inv_mas_receipt.lastmodifieddate 
			from inv_mas_receipt 
			left join inv_mas_dealer on inv_mas_dealer.slno = inv_mas_receipt.customerreference 
			left join inv_dealer_invoicenumbers on inv_dealer_invoicenumbers.slno = inv_mas_receipt.dealerinvoiceno
			where inv_mas_dealer.slno = '".$customerreference."' and inv_dealer_invoicenumbers.status <> 'CANCELLED'".$creditamtdetails.";";
			...
			$query = "select distinct inv_mas_receipt.slno as slno,
			inv_dealer_invoicenumbers.netamount,inv_mas_receipt.receiptamount ,inv_mas_receipt.createddate,
			inv_mas_dealer.businessname as createdby,inv_mas_receipt.receiptremarks,inv_mas_receipt.privatenote,
			inv_mas_receipt.lastmodifieddate,inv_mas_receipt.invoiceno,inv_dealer_invoicenumbers.status , 
			inv_mas_receipt.restatus as receiptstatus, inv_mas_receipt.cancelledremarks as cancelledremarks 
			from inv_mas_receipt 
			left join inv_mas_dealer on inv_mas_dealer.slno = inv_mas_receipt.customerreference 
			left join inv_dealer_invoicenumbers on inv_dealer_invoicenumbers.slno = inv_mas_receipt.dealerinvoiceno
			where inv_mas_dealer.slno = '".$customerreference."' and inv_dealer_invoicenumbers.status <> 'CANCELLED'".$creditamtdetails." 
			ORDER BY inv_mas_receipt.createddate desc LIMIT ".$startlimit.",".$limit."; ";
			$result = runmysqlquery($query);
			if($startlimit == 0)
			{
			...
			<td nowrap = "nowrap" class="td-border-grid" align="left">Cancelled Remarks</td>
			<td nowrap = "nowrap" class="td-border-grid" align="left">Private Note</td>
			<td nowrap = "nowrap" class="td-border-grid" align="left">Created Date</td>
			<td nowrap = "nowrap" class="td-border-grid" align="left">Last Modified Date</td>
			<td nowrap = "nowrap" class="td-border-grid" align="left">Invoice Status</td>
			<td nowrap = "nowrap" class="td-border-grid" align="left">Receipt Status</td></tr>';
			}
	...
	{
		$dealerreference = $_POST['dealerreference'];
		$query = "select slno, invoiceno from inv_dealer_invoicenumbers where dealerreference = '".$dealerreference."' and inv_dealer_invoicenumbers.status <> 'CANCELLED' ";
		$result = runmysqlquery($query);
		if(mysqli_num_rows($result) > 0)
		{
			...
			$grid .= '</select><input type="hidden" name="receipttype" id="receipttype">';
	...
	case 'cancelreceipt':
	{
		...
		$query = "update inv_mas_receipt set restatus = 'CANCELLED',cancelleddate = '".date('Y-m-d').' '.date('H:i:s')."',cancelledip = '".$_SERVER['REMOTE_ADDR']."',cancelledby = '".$userid ."',cancelledremarks = '".$cancelremarks."',receiptamount=null  where slno  ='".$receiptno."';";
		$result = runmysqlquery($query);
		
		//resend the mail after Cancelled sucessfully	
		//sendreceipt($receiptno,'cancelled');
		
		echo(json_encode('1^'.'Record Cancelled Successfully.'));
	}
	break;
...
{
	$query = "Select * from inv_dealer_invoicenumbers where slno = '".$invoiceno."';";
	$resultfetch= runmysqlqueryfetch($query);
	$netamount = $resultfetch['netamount'];
	$query = "select sum(receiptamount) as receiptamount from inv_mas_receipt where dealerinvoiceno = '".$invoiceno."' and restatus <> 'CANCELLED'" ;
	$resultfetch = runmysqlqueryfetch($query);
