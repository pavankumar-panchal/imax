...
switch($switchtype)
{
	case 'save':
	{
		...
		if($lastslno == '')
		{
			...
			$eventquery = "Insert into inv_logs_event(userid,system,eventtype,eventdatetime,remarks) values('".$userid."','".$_SERVER['REMOTE_ADDR']."','278','".date('Y-m-d').' '.date('H:i:s')."','".$customerreference."')";
		...
		else
		{
			...
			$result = runmysqlquery($query);
			$eventquery = "Insert into inv_logs_event(userid,system,eventtype,eventdatetime,remarks) values('".$userid."','".$_SERVER['REMOTE_ADDR']."','279','".date('Y-m-d').' '.date('H:i:s')."','".$customerreference."')";
			$eventresult = runmysqlquery($eventquery);
		}
	...
	case 'delete':
	{
		...
		$result = runmysqlquery($query);
		$eventquery = "Insert into inv_logs_event(userid,system,eventtype,eventdatetime,remarks) values('".$userid."','".$_SERVER['REMOTE_ADDR']."','280','".date('Y-m-d').' '.date('H:i:s')."','".$customerreference."')";
		$eventresult = runmysqlquery($eventquery);
	...
	case 'gridtoform':
	{
		...
		if($resultfetch['module'] == 'user_module')
			...
			left join inv_mas_dealer on inv_mas_dealer.slno = inv_matrixinvoicenumbers.dealerid  where inv_mas_receipt.slno = '".$lastslno."' group by inv_mas_receipt.slno;";
	...
	case 'cancelreceipt':
	{
		...
		sendreceipt('','cancelled',$receiptno);
