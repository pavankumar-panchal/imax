...
switch($switchtype)
{
	case 'save':
	{
		...
		if($lastslno == "")
		{
		 ...
		 $query1 = "Insert into inv_mas_customer(slno,customerid,businessname,address, place,pincode,district,region,category,type,stdcode,website,remarks,disablelogin,
		 createddate,createdby,corporateorder,currentdealer,fax,firstproduct,activecustomer,lastmodifieddate,lastmodifiedby,createdip,lastmodifiedip,branch,companyclosed,
		 isdealer,displayinwebsite,promotionalsms,promotionalemail,oldbusinessname,gst_no,sez_enabled,tanno,panno) 
		 values 
		 ('".$cusslno."','','".trim($businessname)."','".$address."','".$place."','".$pincode."','".$district."','".$region."','".$category."','".$type."','".$stdcode."',
		 '".$website."','".$remarks."','".$disablelogin."','".changedateformatwithtime($createddate)."','".$userid."','".$corporateorder."','".$currentdealer."','".$fax."',
		 '','yes','".date('Y-m-d').' '.date('H:i:s')."','".$userid."','".$_SERVER['REMOTE_ADDR']."','".$_SERVER['REMOTE_ADDR']."','".$branch."','".$companyclosed."',
		 '".$isdealer."','".$displayinwebsite."','".$promotionalsms."','".$promotionalemail."','".trim($oldbusinessname)."','','".$sez_enabled."','".$tanno."','".$panno."');";
		 
			$result = runmysqlquery($query1);
			$eventquery = "Insert into inv_logs_event(userid,system,eventtype,eventdatetime,remarks) 
			values('".$userid."','".$_SERVER['REMOTE_ADDR']."','41','".date('Y-m-d').' '.date('H:i:s')."','".$cusslno."')";
			...
			if($gst_no!= "") {
    			...
    			,'$gst_no','$userid','$userid','user_module',NOW(),NOW())";
			...
			for($j=0;$j<count($contactressplit);$j++)
			{
				...
				$query2 = "Insert into inv_contactdetails(customerid,selectiontype,contactperson,phone,cell,emailid) values  
				('".$cusslno."','".$selectiontype."','".$contactperson."','".$phonevalue."','".$cellvalue."','".$emailidvalue."');";
		...
		if($businessname <> $e_businessname || $address <> $e_address || $place <> $e_place || $pincode <> $e_pincode || $district <> $e_district || $region <> $e_region || $category <> $e_category || $type <> $e_type || $stdcode <> $e_stdcode || $phone <> $e_phone  || $website <> $e_website || $remarks <> $e_remarks || $disablelogin <> $e_disablelogin || $corporateorder <> $e_corporateorder || $currentdealer <> $e_currentdealer || $fax <> $e_fax || $branch <> $e_branch || $companyclosed <> $e_companyclosed || $isdealer <> $e_isdealer)
			...
			else
			{
				...
				$e_tanno = $queryfetch['tanno'];
				$e_panno = $queryfetch['panno'];
			    ...
			    if($gst_no!= "") {
    	            ...
    	            if($gstnumbers == 0) {
    	                ...
    	                $insert_gst = "insert into customer_gstin_logs (gstin_id,customer_slno,effective_from,gst_no,created_by,updated_by,usertype,created_at,updated_at) values (NULL,$lastslno,'$effective_from','$gst_no','$userid','$userid','user_module',NOW(),NOW())";
    			        $result_gst_new = runmysqlquery($insert_gst);
    	            }
					else
    	            {
			    ...
			    $query1 = "UPDATE inv_mas_customer SET businessname = '".trim($businessname)."',oldbusinessname = '".trim($oldbusinessname)."',address = '".$address."',
			    place = '".$place."',pincode = '".$pincode."',district = '".$district."',region = '".$region."',category = '".$category."',type = '".$type."',
			    stdcode = '".$stdcode."',website = '".$website."',remarks = '".$remarks."',disablelogin = '".$disablelogin."',corporateorder = '".$corporateorder."',
			    currentdealer = '".$currentdealer."',fax ='".$fax."',lastmodifieddate ='".date('Y-m-d').' '.date('H:i:s')."',lastmodifiedby ='".$userid."',
			    lastmodifiedip = '".$_SERVER['REMOTE_ADDR']."' ,companyclosed = '".$companyclosed."' ,branch = '".$branch."',isdealer = '".$isdealer."' ,
			    displayinwebsite = '".$displayinwebsite."' ,promotionalsms = '".$promotionalsms."' ,promotionalemail = '".$promotionalemail."',
			    ".$addgst."sez_enabled = '".$sez_enabled."',tanno = '".$tanno."',panno = '".$panno."' WHERE slno = '".$lastslno."'";
				//echo $query1;
				//exit();
	...
	case 'delete':
	{
		...
		$recordflag25 = deleterecordcheck($lastslno,'customer_slno','customer_gstin_logs');
		...
		if(($recordflag1 == true) && ($recordflag2 == true) && ($recordflag3 == true) && ($recordflag4 == true) && ($recordflag5 == true) && ($recordflag6 == true) && ($recordflag7 == true) && ($recordflag8 == true)  && ($recordflag10 == true) && ($recordflag11 == true) && ($recordflag12 == true) && ($recordflag13 == true) && ($recordflag14 == true) && ($recordflag15 == true) && ($recordflag16 == true) && ($recordflag17 == true) && ($recordflag18 == true) && ($recordflag19 == true) && ($recordflag20 == true) && ($recordflag21 == true) && ($recordflag22 == true) && ($recordflag23 == true)&& ($recordflag24 == true))
		{
			...
			#added for new GST Table
			$query3 = "DELETE FROM customer_gstin_logs WHERE customer_slno = '".$lastslno."'";
			$result3 = runmysqlquery($query3);
			#Ends newly added here 
...
where inv_customerproduct.customerreference IS NOT NULL and inv_mas_customer.businessname LIKE '%".$textfield."%' ".$regionpiece.$district_typepiece.$state_typepiece.$dealer_typepiece.$branchpiece.$typepiece.$categorypiece."  ORDER BY businessname";
	...
	case 'searchcustomer':
	{
		...
		if(!empty($searchtext))
		{
			$query = "select slno,businessname from inv_mas_customer where businessname like '%".$searchtext."%'";
			$result = runmysqlquery($query);
			...
			if($count > 0)
			{
				while($fetch = mysqli_fetch_array($result))
				{
					//$associativeArray += [$fetch['slno'] => $fetch['businessname']];
					$customerdata[$fetch['slno']] = $fetch['businessname'];
		...
		{
			$query = "select slno,businessname from inv_mas_customer where businessname limit 100";
			$result = runmysqlquery($query);
			$count = mysqli_num_rows($result);
			if($count > 0)
			{
		...
		$responsearray['customerdata'] = $customerdata;
	...
	case 'generatecustomerlist':
	{
		...
		while($fetch = mysqli_fetch_array($result))
		{
			...
			$count++;
			//$customerarray[$fetch['slno']] = $fetch['businessname'];
		}
		//echo($grid);
		// $responsearray['customerdata'] = $customerarray;
		// echo(json_encode($responsearray));
		...
		if($fetch1['count'] > 0)
		{
			...
			inv_mas_customer.address, inv_mas_customer.place, inv_mas_customer.district,inv_mas_district.statecode as state,
			 ...
			 inv_mas_customer.type, inv_mas_customer.isdealer,inv_mas_customer.remarks, inv_mas_customer.currentdealer, 
			  ...
			  inv_mas_customer.displayinwebsite, inv_mas_customer.promotionalsms,inv_mas_customer.promotionalemail ,inv_mas_customer.gst_no ,inv_mas_customer.sez_enabled,inv_mas_customer.tanno,inv_mas_customer.panno
...
on registrations.registrations = groups.group order by groups.group desc"; //echo($query2);exit;
			...
			#Added for GST New table 
			/*$select_gst_new ="SELECT cgl.gstin_id,cgl.customer_slno,cgl.effective_from,cgl.gst_no,cgl.created_by,cgl.updated_by,cgl.updated_at,cgl.created_at,imu.fullname 
			from customer_gstin_logs cgl
			inner join inv_mas_users imu on cgl.updated_by = imu.slno
			where cgl.customer_slno = '".$lastslno."' order by cgl.gstin_id desc;";*/
			$select_gst_new ="SELECT cgl.gstin_id,cgl.customer_slno,cgl.effective_from,cgl.gst_no,cgl.created_by,cgl.updated_by,cgl.updated_at,cgl.created_at,imu.fullname,inv_mas_dealer.businessname as dealername
			from customer_gstin_logs cgl
			left join inv_mas_users imu on cgl.updated_by = imu.slno
			left join inv_mas_dealer on cgl.updated_by = inv_mas_dealer.slno
			where cgl.customer_slno = '".$lastslno."' order by cgl.gstin_id desc;";
			$result_gst_new = runmysqlquery($select_gst_new);
			...
			$last_new_gst_no = '';
			$last_effective_from = '';
			while($fetch_new_gst = mysqli_fetch_array($result_gst_new)) {
				...
				if($i_n%2 == 0)
					...
					$gstin_id = $fetch_new_gst['gstin_id'];
					$customer_slno = $fetch_new_gst['customer_slno'];
					$effective_from = $fetch_new_gst['effective_from'];
					$gst_no = $fetch_new_gst['gst_no'];
					$created_by = $fetch_new_gst['created_by'];
					$updated_by = $fetch_new_gst['updated_by'];
					//$fullname = $fetch_new_gst['fullname'];
					if($fetch_new_gst['fullname']!= "")
					    $fullname = $fetch_new_gst['fullname'];
					 else
					 	 $fullname = $fetch_new_gst['dealername'];
					
					$created_at = $fetch_new_gst['created_at'];
					$updated_at = $fetch_new_gst['updated_at'];
					...
					if($counter == 1) {
					    $last_new_gst_no = $gst_no;  
					    $last_effective_from = $effective_from;  
					}

        			$gst_new_grid .= '<tr class="newaddedgst" id="newaddedgst'.$counter.'">
            			<td class="td-border-grid" align="center" bgcolor='.$color.'>'.$counter.'</td>
                        <td class="td-border-grid" align="center" bgcolor='.$color.'>'.$gst_no.'</td>
                        <td class="td-border-grid" align="center" bgcolor='.$color.'>'.$fullname.'</td>
                        <td class="td-border-grid" align="center" bgcolor='.$color.'>'.$effective_from.'</td>
                        <td class="td-border-grid" align="center" bgcolor='.$color.'>'.$created_at.'</td>
                        <td class="td-border-grid" align="center" bgcolor='.$color.'>'.$updated_at.'</td>
                        <td class="td-border-grid" align="center" bgcolor='.$color.'><span style="color:red;cursor: pointer;" onclick="deletegstin(\''.$gstin_id.'*newaddedgst'.$counter.'\')">Delete</span></td>
                    </tr>';
			}
			
			if($last_new_gst_no == '' || empty($last_new_gst_no)) 
			{
				if(is_numeric($fetch['gst_no']))
				{
					$querygst = "select gst_no from customer_gstin_logs where gstin_id =".$fetch['gst_no'];
					$resultgst = runmysqlquery($querygst);
					$countgst = mysqli_num_rows($resultgst);
					if($countgst > 0)
					{
						$fetchgst = runmysqlqueryfetch($querygst);
						$last_new_gst_no = $fetchgst['gst_no'];
					}
				}
				else
			    	$last_new_gst_no = $fetch['gst_no'];
			}
			if($last_effective_from == '' || empty($last_effective_from)) {
			    $last_effective_from = date('Y-m-d');
			}
			#ends Added for GST New table 
			...
			$responsearray['gst_new_grid'] = $gst_new_grid;
			$responsearray['last_new_gst_no'] = $last_new_gst_no;
			$responsearray['last_effective_from'] = $last_effective_from;
		...
		else
		{
			...
			$responsearray['gst_new_grid'] = '';
			$responsearray['last_new_gst_no'] = '';
			$responsearray['last_effective_from'] = '';
            $responsearray['tanno'] = '';
            $responsearray['panno'] = '';
			echo(json_encode($responsearray));
		}
	...
	case 'deletegstin':
	{
	   ...
	   //$delete = "delete from customer_gstin_logs where gstin_id = $deleteid and customer_slno = '$customerslno' limit 1";
	   
	   $check_gst = "select count(*) as count from inv_invoicenumbers where gst_no = $deleteid";
		$res_check_gst = runmysqlqueryfetch($check_gst);

		$gstquery = "select gst_no from customer_gstin_logs where gstin_id = $deleteid";
		$gstfetch = runmysqlqueryfetch($gstquery);

	   if(!empty($gstfetch['gst_no']))
	   {
			$check_gst1 = "select count(*) as mcount from inv_matrixinvoicenumbers where gst_no = '".$gstfetch['gst_no']."'";
			$res_check_gst1 = runmysqlqueryfetch($check_gst1);

			$check_gst2 = "select count(*) as mrcount from inv_matrixreqpending where gst_no = '".$gstfetch['gst_no']."'";
			$res_check_gst2 = runmysqlqueryfetch($check_gst2);
	   }
					
	    if($res_check_gst['count'] == 0 && $res_check_gst1['mcount'] == 0 && $res_check_gst2['mrcount'] == 0) {

        	   $delete = "delete from customer_gstin_logs where gstin_id = $deleteid";
        	   $resultfetch = runmysqlquery($delete);
        	   
        	   if($resultfetch) {
        	    
        	    //$query_gst1 = runmysqlqueryfetch("SELECT (MAX(gstin_id)) AS gstin_id_inserted,effective_from,gst_no FROM customer_gstin_logs where customer_slno = '$customerslno'");
        	    
        	    /*$query_gst1 = runmysqlqueryfetch("SELECT gstin_id AS gstin_id_inserted,effective_from,gst_no FROM customer_gstin_logs where 
        	    customer_slno = '$customerslno' order by gstin_id desc limit 1");
                
                $gstin_id_inserted = $query_gst1['gstin_id_inserted'];
                $effective_from = $query_gst1['effective_from'];
                $gst_no = $query_gst1['gst_no'];
        
                $update_customer = "update inv_mas_customer set gst_no = '$gstin_id_inserted' where slno = '$customerslno' limit 1";
        		$result_update_customer = runmysqlquery($update_customer);*/

        		$query_initial_gst = "select gst_no from inv_mas_customer where slno=".$customerslno;
                $result_initial_gst = runmysqlquery($query_initial_gst);
                $count_initial_gst = mysqli_num_rows($result_initial_gst);
                

                if($count_initial_gst > 0)
                {

                	$fetch_initial_gst = runmysqlqueryfetch($query_initial_gst);
                	if(is_numeric($fetch_initial_gst['gst_no']))
                	{
                		//$gst_no = $query_gst1['gst_no'];
		                $query_gst0 = "select customer_gstin_logs.gstin_id from inv_mas_customer
						left join customer_gstin_logs on inv_mas_customer.gst_no = customer_gstin_logs.gstin_id 
						where slno =  ".$customerslno." and customer_gstin_logs.gstin_id =". $deleteid;
		                $result_gst0 = runmysqlquery($query_gst0);
		                $count_gst0 = mysqli_num_rows($result_gst0);
                		if($count_gst0 == 0)
                		{

		        			$query_gst1 = "SELECT gstin_id AS gstin_id_inserted,effective_from,gst_no FROM customer_gstin_logs where 
			        	    customer_slno = '$customerslno' order by gstin_id desc limit 1";
			        	    $result_gst1 = runmysqlquery($query_gst1);
	                		$count_gst1 = mysqli_num_rows($result_gst1);

			                if($count_gst1 > 0)
			                {
			                	$fetch_gst1 = runmysqlqueryfetch($query_gst1);
				                $gstin_id_inserted = $fetch_gst1['gstin_id_inserted'];
				                $effective_from = $fetch_gst1['effective_from'];
				                $gst_no = $fetch_gst1['gst_no'];
				        
				                $update_customer = "update inv_mas_customer set gst_no = '$gstin_id_inserted' where slno = '$customerslno'";
				        		$result_update_customer = runmysqlquery($update_customer);
	        				}
	        				else
	        				{
	        					$update_customer = "update inv_mas_customer set gst_no = '' where slno = '$customerslno'";
				        		$result_update_customer = runmysqlquery($update_customer);
	        				}
	        			}
	        		}
        		...
        		$responsearray['delete_status'] = '1*GSTIN Deleted';
        		$responsearray['effective_from'] = $fetch_gst1['effective_from'];
        		$responsearray['gst_no'] = $fetch_gst1['gst_no'];
        	   }
        	   else {
        	       $responsearray['delete_status'] = '2*GSTIN Can Not Be Deleted';
        	   }
        	   echo(json_encode($responsearray));
	    }
	    else {
	        $responsearray['delete_status'] = '3*Invoice Exists for the GSTIN selected';
	        echo(json_encode($responsearray));
	    }
	}
	break; 
	case 'searchbycustomerid':
	{
		...
		$lastslno = substr($customerid, $customeridlen - 5);//exit;
		...
		if($customeridlen == 5)
		{
			...
			if($fetch1['count'] > 0)
			{
				...
				,inv_mas_customer.sez_enabled,inv_mas_customer.tanno,inv_mas_customer.panno FROM inv_mas_customer LEFT JOIN inv_mas_product ON inv_mas_product.productcode = inv_mas_customer.firstproduct  LEFT JOIN  inv_mas_users ON  inv_mas_users.slno = inv_mas_customer.createdby left join inv_mas_district on inv_mas_customer.district = inv_mas_district.districtcode  left join inv_mas_state on inv_mas_state.statecode = inv_mas_district.statecode where inv_mas_customer.slno = '".$lastcustomerid."';";
				$fetch = runmysqlqueryfetch($query);
			...
			// 2011-12 Summary
				$query2 = "select * from 
(select distinct inv_mas_product.group from inv_mas_product where inv_mas_product.group in ('TDS','SPP','STO','XBRL','GST','SAC') and inv_mas_product.year = '".$currentyear."') as groups
left join
(select distinct inv_mas_product.group as bills from inv_invoicenumbers join inv_dealercard on inv_dealercard.invoiceid = inv_invoicenumbers.slno left join inv_mas_product on inv_dealercard.productcode = inv_mas_product.productcode where  right(inv_invoicenumbers.customerid,5) = '".$lastslno."' and inv_mas_product.year = '".$currentyear."' and (inv_invoicenumbers.cancelledby is null)) as bills
on bills.bills = groups.group
left join
...
on registrations.registrations = groups.group order by groups.group desc"; 
			...
			while($fetch_new_gst = mysqli_fetch_array($result_gst_new)) {
				...
				if($i_n%2 == 0)
        			...
        			$gst_new_grid .= '<tr class="newaddedgst" id="newaddedgst'.$counter.'">
                        ...
                        <td class="td-border-grid" align="center" bgcolor='.$color.'>'.$created_at.'</td>
                        <td class="td-border-grid" align="center" bgcolor='.$color.'>'.$updated_at.'</td>
                        <td class="td-border-grid" align="center" bgcolor='.$color.'><a href="#" onclick="deletegstin(\''.$gstin_id.'*newaddedgst'.$counter.'\')">Delete</a></td>
                    </tr>';
			}
			...
			$responsearray4['tanno'] = $fetch['tanno'];
			$responsearray4['panno'] = $fetch['panno'];
			...
			$responsearray4['gst_new_grid'] = $gst_new_grid;
			$responsearray4['last_new_gst_no'] = $last_new_gst_no;
			$responsearray4['last_effective_from'] = $last_effective_from;
			...
			else
			{
			  ...
			  $responsearray4['tanno'] = '';
			  $responsearray4['panno'] = '';
...
on registrations.registrations = groups.group order by groups.group desc"; 
		...
		//	echo('^'.$p_editcustomercontact.'^'.$p_registration.'^'.$userid.'^'.strtolower($fetch['corporateorder']).'^'.$fetch['currentdealer'].'^'.$p_editcustomerpassword.'^'.$fetch['fax'].'^'.$fetch['activecustomer'].'^'.$fetch['branch'].'^'.$fetch['companyclosed'].'^'.$fetch['districtname'].'^'.$fetch['statename'].'^'.$fetch['password'].'^'.$fetch['isdealer'].'^'.$contactarray.'^'.$rescontact.'^'.$resphone.'^'.$rescell.'^'.$resemailid.'^'.$fetch['promotionalsms'].'^'.$fetch['promotionalemail'].'^'.$grid);
			...
			from customer_gstin_logs cgl
			inner join inv_mas_users imu on cgl.updated_by = imu.slno
			where cgl.customer_slno = '".$lastslnoS."' order by cgl.gstin_id desc;";
			$result_gst_new = runmysqlquery($select_gst_new);
			...
			else
			{
			  ...
			  $responsearray4['promotionalsms'] = '';
			  $responsearray4['grid'] = '';
			  $responsearray4['gst_new_grid'] = '';
			  $responsearray4['last_new_gst_no'] = '';
			  $responsearray4['last_effective_from'] = '';
			  echo(json_encode($responsearray4));
			}
		}
		else
...
left join inv_mas_dealer on inv_customerproduct.dealerid = inv_mas_dealer.slno  where inv_customerproduct.AUTOREGISTRATIONYN = 'N' and customerreference = '".$lastslno."' order by `date`  desc,`time` desc LIMIT ".$startlimit.",".$limit."; ";
	...
	case 'customerautoregistration':
	{
		...
		*/
		...
		$query = "SELECT inv_mas_product.productname as productname,scratchnumber AS cardid,
		inv_mas_dealer.businessname AS businessname, if(inv_customerproduct.ACTIVELICENSE=1,'Active','Deactive') as activelicense,
		inv_customerproduct.remarks as remarks FROM inv_customerproduct 
		left join inv_mas_product on left(inv_customerproduct.computerid, 3) = inv_mas_product.productcode 
		left join inv_mas_users on inv_customerproduct.generatedby = inv_mas_users.slno 
		...
		where inv_customerproduct.AUTOREGISTRATIONYN = 'Y' and inv_customerproduct.customerreference = '".$lastslno."' order by inv_customerproduct.`date`  desc,`time` desc LIMIT ".$startlimit.",".$limit."; ";
...
/*						if(freeupdateproductcode($productcode) <> 'codenotfound' && $purchasetype == 'new' && $cardscheme == '1')
 ...
 where inv_customerproduct.customerreference = '".$lastslno."' and inv_mas_scratchcard.cancelled = 'no' ;  ";
	...
	case 'cardsearchdetailstoform':
	{
	  ...
	  from inv_customerproduct 
	  left join inv_mas_customer on  inv_customerproduct.customerreference = inv_mas_customer.slno where cardid = '".$cardlastslno."' GROUP BY cardid) 
	  AS inv_customerproduct on  inv_dealercard.cardid = inv_customerproduct.cardid 
	  left join inv_mas_scheme on inv_mas_scheme.slno = inv_dealercard.scheme 
	...
	case 'getcardlist':
	{
		...
		$limit = $_POST['limit'];
		$startindex = $_POST['startindex'];
		$query = "SELECT cardid,scratchnumber FROM inv_mas_scratchcard where cardid!= 0  ORDER BY scratchnumber LIMIT ".$startindex.",".$limit.";";
		$result = runmysqlquery($query);
		$grid = '';
		$count = 0;
		while($fetch = mysqli_fetch_array($result))
...
where inv_customerproduct.customerreference = '".$lastslno."'";*/
       ...
       /* $query = "select distinct inv_mas_scratchcard.cardid,inv_mas_scratchcard.scratchnumber from inv_mas_scratchcard        
		...
		$query = "select customerreference,cardid from inv_customerproduct 
		left join inv_mas_product on left(inv_customerproduct.computerid,3)=inv_mas_product.productcode
		where inv_customerproduct.customerreference =".$lastslno." and inv_mas_product.newproduct!= 1 and inv_customerproduct.cardid!=0";
		$result = runmysqlquery($query);
		...
		if($card_count > 0)
		{
			...
			while($fetch = mysqli_fetch_array($result))
			{
				$cust_query = "select cardid,scratchnumber from `inv_mas_scratchcard` where cardid =".$fetch['cardid']." and inv_mas_scratchcard.cancelled = 'no'";
				$cust_result = runmysqlquery($cust_query);
				$cust_count = mysqli_num_rows($cust_result);
				if($cust_count > 0) {
                    $cust_fetch = runmysqlqueryfetch($cust_query);
                    $reregcardlistarray[$count] = $cust_fetch['scratchnumber'] . ' | ' . $cust_fetch['cardid'] . '^' . $cust_fetch['cardid'];
                    $count++;
                }
			}
		}
...
from inv_dealercard left join inv_mas_scratchcard on inv_dealercard.cardid =inv_mas_scratchcard.cardid left join inv_mas_dealer on inv_dealercard.dealerid = inv_mas_dealer.slno left join inv_mas_product on inv_mas_product.productcode = inv_dealercard.productcode left join inv_mas_scheme on inv_mas_scheme.slno = inv_dealercard.scheme where customerreference ='".$lastslno."' and inv_mas_scratchcard.registered = 'no' order by inv_dealercard.cuscardattacheddate;";
	...
	case 'rcidetailsgrid':
	{
		...
		$resultfetch = runmysqlquery_rci($resultcount);
		$fetchresultcount = mysqli_num_rows($resultfetch);
		if($showtype == 'all')
		$limit = 100000;
		else
		...
		$result = runmysqlquery_rci($query);
		if($startlimit == 0)
		{
	...
	case 'invoicedetailsgrid':
	{
		...
		while($fetch = mysqli_fetch_array($result))
		{
			...
			if($i_n%2 == 0)
				...
				$color = "#f7faff";
			$query1 = "select sum(receiptamount) as receivedamount from inv_mas_receipt where invoiceno = '".$fetch['slno']."' and restatus != 'CANCELLED';";
