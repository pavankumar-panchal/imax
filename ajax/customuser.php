...
switch($switchtype)
{
 ...
 $query = "select DISTINCT isac.customerid as customerid, imc.slno as slno,imc.businessname as businessname from inv_spp_amc_customers isac inner join inv_mas_customer imc on isac.customerid = imc.customerid ORDER BY imc.businessname LIMIT ".$startindex.",".$limit.";";    
 ...
 for($s = 0 ; $s< $countservicetypes;$s++)
 {
  ...
  $resultinsert = runmysqlquery($insert_servicequery);
	...
	case 'getcustomercount':
	{
		...
		$query = "SELECT slno,businessname,customerid FROM inv_mas_customer where customerid IN (select DISTINCT(customerid) from inv_spp_amc_customers) ORDER BY businessname";
		
/* $query = "SELECT slno,inv_mas_customer.customerid,businessname FROM (SELECT DISTINCT(REPLACE(customerid,'-','')) AS customerid FROM inv_invoicenumbers invoice
INNER JOIN inv_mas_product product 
ON invoice.products LIKE CONCAT('%',product.productcode,'%') AND product.group = 'SPP' AND (invoice.servicetype = 'AMC Charges' 
OR YEAR(invoice.createddate) = '2016')) AS sub INNER JOIN inv_mas_customer ON inv_mas_customer.customerid = sub.customerid;";
*/
		$result = runmysqlquery($query);
