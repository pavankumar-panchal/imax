...
switch($switchtype)
{
	case 'implalldata':
	{
		...
		// $query1 = "select distinct count(*) as slnocount from imp_cfentries
		// left join inv_mas_customer on inv_mas_customer.slno = right(imp_cfentries.customerid,5)
		// left join imp_implementation on right(imp_cfentries.customerid,5) = imp_implementation.customerreference
		...
		// where right(imp_cfentries.customerid,5) not in (select customerreference from imp_implementation) ".$finalliststatus.$regionpiece.$state_typepiece.$district_typepiece.$dealer_typepiece.$branchpiece.$typepiece.$categorypiece.$implementerpersonpiece.";";
		...
		left join inv_mas_region on inv_mas_region.slno = inv_mas_dealer.region 
		left join inv_mas_branch on inv_mas_branch.slno = inv_mas_dealer.branch where imp_implementation.slno <> '99999999'  ".$finalliststatus.$regionpiece.$state_typepiece.$district_typepiece.$dealer_typepiece.$branchpiece.$typepiece.$categorypiece.$implementerpersonpiece.";";
		...
		left join inv_mas_dealer on imp_implementation.dealerid = inv_mas_dealer.slno 
		left join inv_mas_region on inv_mas_region.slno = inv_mas_dealer.region 
		left join inv_mas_branch on inv_mas_branch.slno = inv_mas_dealer.branch where imp_implementation.implementationstatus <> 'completed'   ".$finalliststatus.$regionpiece.$state_typepiece.$district_typepiece.$dealer_typepiece.$branchpiece.$typepiece.$categorypiece.$implementerpersonpiece.";";
		...
		imp_implementation.coordinatorapproval = 'yes' AND imp_implementation.implementationstatus = 'completed'  ".$finalliststatus.$regionpiece.$state_typepiece.$district_typepiece.$dealer_typepiece.$branchpiece.$typepiece.$categorypiece.$implementerpersonpiece." ;";
		...
		left join inv_mas_dealer on imp_implementation.dealerid = inv_mas_dealer.slno 
		left join inv_mas_region on inv_mas_region.slno = inv_mas_dealer.region 
		left join inv_mas_branch on inv_mas_branch.slno = inv_mas_dealer.branch where 
		imp_implementation.implementationstatus <> 'completed' and imp_implementation.branchapproval = 'no' ".$finalliststatus.$regionpiece.$state_typepiece.$district_typepiece.$dealer_typepiece.$branchpiece.$typepiece.$categorypiece.$implementerpersonpiece." ;";
		...
		//Fetch Region wise Due Data
		$query7 = "select inv_mas_region.category as regionname, count(imp_implementation.dealerid) as slnocount from inv_mas_region 
		left join inv_mas_dealer on  inv_mas_region.slno = inv_mas_dealer.region 
		left join imp_implementation on  imp_implementation.dealerid = inv_mas_dealer.slno
		left join inv_mas_customer on inv_mas_customer.slno = imp_implementation.customerreference
		left join inv_mas_implementer on inv_mas_implementer.slno = imp_implementation.assignimplemenation 
		left join inv_mas_district on inv_mas_customer.district = inv_mas_district.districtcode 
		left join inv_mas_state on inv_mas_district.statecode = inv_mas_state.slno 
		left join inv_mas_branch on inv_mas_branch.slno = inv_mas_dealer.branch where imp_implementation.implementationstatus <> 'completed' 
		".$finalliststatus.$regionpiece.$state_typepiece.$district_typepiece.$dealer_typepiece.$branchpiece.$typepiece.$categorypiece.$implementerpersonpiece."
		group by inv_mas_region.slno order by inv_mas_region.category;";
		$region_bkg = 0;$region_bkm = 0;$region_csd = 0;
		$result1 = runmysqlquery($query7);
		while($fetch7 = mysqli_fetch_array($result1))
		{	
			if($fetch7['regionname'] == 'BKG')
				$region_bkg = $fetch7['slnocount'];
			elseif($fetch7['regionname'] == 'BKM')
				$region_bkm = $fetch7['slnocount'];
			elseif($fetch7['regionname'] == 'CSD')
				$region_csd = $fetch7['slnocount'];	
			
		}
		$totalofregionduedata = $region_bkg + $region_bkm + $region_csd;
		// left join imp_cfentries on right(imp_cfentries.customerid,5) = imp_implementation.customerreference
		...
		left join inv_mas_region on inv_mas_region.slno = inv_mas_dealer.region 
		left join inv_mas_branch on inv_mas_branch.slno = inv_mas_dealer.branch
		where imp_implementation.branchapproval = 'no' AND imp_implementation.coordinatorreject = 'no' AND imp_implementation.coordinatorapproval = 'no' AND imp_implementation.branchreject = 'no' AND imp_implementation.implementationstatus = 'pending'  ".$finalliststatus.$regionpiece.$state_typepiece.$district_typepiece.$dealer_typepiece.$branchpiece.$typepiece.$categorypiece.$implementerpersonpiece."";
		...
		left join inv_mas_region on inv_mas_region.slno = inv_mas_dealer.region 
		left join inv_mas_branch on inv_mas_branch.slno = inv_mas_dealer.branch 
		where imp_implementation.branchapproval = 'no' AND imp_implementation.branchreject = 'yes' AND imp_implementation.implementationstatus = 'pending'  ".$finalliststatus.$regionpiece.$state_typepiece.$district_typepiece.$dealer_typepiece.$branchpiece.$typepiece.$categorypiece.$implementerpersonpiece."";
		...
		left join inv_mas_region on inv_mas_region.slno = inv_mas_dealer.region 
		left join inv_mas_branch on inv_mas_branch.slno = inv_mas_dealer.branch
		where imp_implementation.branchapproval = 'yes' AND imp_implementation.coordinatorreject = 'no' AND imp_implementation.coordinatorapproval = 'no' AND imp_implementation.implementationstatus = 'pending'  ".$finalliststatus.$regionpiece.$state_typepiece.$district_typepiece.$dealer_typepiece.$branchpiece.$typepiece.$categorypiece.$implementerpersonpiece."";
		...
		left join inv_mas_region on inv_mas_region.slno = inv_mas_dealer.region 
		left join inv_mas_branch on inv_mas_branch.slno = inv_mas_dealer.branch
		where imp_implementation.branchapproval = 'no' AND imp_implementation.coordinatorreject = 'yes' AND imp_implementation.coordinatorapproval = 'no' AND imp_implementation.implementationstatus = 'pending'  ".$finalliststatus.$regionpiece.$state_typepiece.$district_typepiece.$dealer_typepiece.$branchpiece.$typepiece.$categorypiece.$implementerpersonpiece."";
		...
		left join inv_mas_region on inv_mas_region.slno = inv_mas_dealer.region 
		left join inv_mas_branch on inv_mas_branch.slno = inv_mas_dealer.branch
		where imp_implementation.branchapproval = 'yes' AND imp_implementation.coordinatorreject = 'no' AND imp_implementation.coordinatorapproval = 'yes' AND imp_implementation.implementationstatus = 'pending'  ".$finalliststatus.$regionpiece.$state_typepiece.$district_typepiece.$dealer_typepiece.$branchpiece.$typepiece.$categorypiece.$implementerpersonpiece."";
		...
		left join inv_mas_region on inv_mas_region.slno = inv_mas_dealer.region 
		left join inv_mas_branch on inv_mas_branch.slno = inv_mas_dealer.branch
		where imp_implementation.branchapproval = 'yes' AND imp_implementation.coordinatorreject = 'no' AND imp_implementation.coordinatorapproval = 'yes' AND imp_implementation.implementationstatus = 'assigned'  ".$finalliststatus.$regionpiece.$state_typepiece.$district_typepiece.$dealer_typepiece.$branchpiece.$typepiece.$categorypiece.$implementerpersonpiece."";
		...
		left join inv_mas_region on inv_mas_region.slno = inv_mas_dealer.region 
		left join inv_mas_branch on inv_mas_branch.slno = inv_mas_dealer.branch
		where imp_implementation.branchapproval = 'yes' AND imp_implementation.coordinatorreject = 'no' AND imp_implementation.coordinatorapproval = 'yes' AND imp_implementation.implementationstatus = 'progess'  ".$finalliststatus.$regionpiece.$state_typepiece.$district_typepiece.$dealer_typepiece.$branchpiece.$typepiece.$categorypiece.$implementerpersonpiece."";
		...
		left join inv_mas_region on inv_mas_region.slno = inv_mas_dealer.region 
		left join inv_mas_branch on inv_mas_branch.slno = inv_mas_dealer.branch 
		where imp_implementation.branchapproval = 'yes' AND imp_implementation.coordinatorreject = 'no' AND imp_implementation.coordinatorapproval = 'yes' AND imp_implementation.implementationstatus = 'completed'  ".$finalliststatus.$regionpiece.$state_typepiece.$district_typepiece.$dealer_typepiece.$branchpiece.$typepiece.$categorypiece.$implementerpersonpiece."";
		...
		echo('1^'.$totalofalldata.'^'.$raw_total.'^'.$activated_total.'^'.$totalofactivedata.'^'.$dueactive_total.'^'.$completactive_total.'^'.$totalofactiveimpdata.'^'.$dueimp_total.'^'.$completeimp_total.'^'.$totalofregionduedata.'^'.$region_bkg.'^'.$region_bkm.'^'.$region_csd.'^'.$status_branch.'^'.$status_coordinatorapproval.'^'.$status_coordinatorreject.'^'.$status_pending.'^'.$status_assigned.'^'.$status_progess.'^'.$status_completed.'^'.$status_totaldue.'^'.$status_branchreject);
		
	}
	break;
	
	case 'implbranchduedata':
	{
		$state = $_POST['state'];
		$region = $_POST['region'];
		$dealer = $_POST['dealer'];
		...
		$query1 = "select inv_mas_branch.branchname as branchname,count(imp_implementation.dealerid) as slnocount from inv_mas_branch 
		left join inv_mas_dealer on  inv_mas_branch.slno = inv_mas_dealer.branch 
		left join imp_implementation on  imp_implementation.dealerid = inv_mas_dealer.slno
		left join imp_cfentries on right(imp_cfentries.customerid,5) = imp_implementation.customerreference
		left join inv_mas_customer on inv_mas_customer.slno = imp_implementation.customerreference
		left join inv_mas_implementer on inv_mas_implementer.slno = imp_implementation.assignimplemenation 
		left join inv_mas_district on inv_mas_customer.district = inv_mas_district.districtcode 
		left join inv_mas_state on inv_mas_district.statecode = inv_mas_state.slno 
		left join inv_mas_region on inv_mas_region.slno = inv_mas_dealer.region where inv_mas_branch.slno <> '9999999' and imp_implementation.implementationstatus <> 'completed'
		".$finalliststatus.$regionpiece.$state_typepiece.$district_typepiece.$dealer_typepiece.$branchpiece.$typepiece.$categorypiece.$implementerpersonpiece." group by inv_mas_branch.slno order by inv_mas_branch.branchname;";
		...
		{	
			...
			if($count > 1)
				$branchnamelist .='$#$';
			$branchgrid .= $fetchcount['slnocount'];
			$branchnamelist .= $fetchcount['branchname'];
			$totalofbranchduedata += $fetchcount['slnocount'];
			$count++;
			
		}
		$query2 = "select slno,branchname from inv_mas_branch group by branchname order by branchname";
		$result2 = runmysqlquery($query2);
		while($fetchcoun1t = mysqli_fetch_array($result2))
		{	
			if($count1 > 1)
				$branchname .='$$$';
			$branchname .= $fetchcoun1t['branchname'];
			$totalofbranchduedata += $fetchcount['slnocount'];
			$count1++;
		}

		echo('1^'.$branchgrid.'^'.$totalofbranchduedata.'^'.$branchname.'^'.$branchnamelist);
	}
	break;
	...
	case 'implbranchimpwise':
	{
		...
		from inv_mas_implementer left join imp_implementation on  imp_implementation.assignimplemenation = inv_mas_implementer.slno
		left join inv_mas_branch on  inv_mas_branch.slno = inv_mas_implementer.branchid
		where inv_mas_branch.slno = '".$branchslno."'  and inv_mas_implementer.disablelogin = 'no' and inv_mas_implementer.implementernotinuse = 'no' group by inv_mas_implementer.businessname";
...
left join inv_mas_branch on  inv_mas_branch.slno = inv_mas_dealer.branch
where inv_mas_branch.slno = '".$branchslno."' and inv_mas_dealer.disablelogin = 'no' and inv_mas_dealer.dealernotinuse = 'no' group by inv_mas_dealer.businessname";
		...
		if(mysqli_num_rows($result) <> 0)
		{
			...
			while($fetchcount = mysqli_fetch_array($result))
			{
				...
				$salegrid .= '<td class="imp_td-border" ><span class="imp_fontstyle4" align="center"
	...
	case 'impdetailedgrid':
	{
		...
		if($typelist == 'activatedata' || $typelist == 'totalalldata')
		{
			...
			group by impref)as dummy on dummy.impref = imp_implementation.slno where imp_implementation.slno <> '99999999'  ".$finalliststatus.$regionpiece.$state_typepiece.$district_typepiece.$dealer_typepiece.$branchpiece.$typepiece.$categorypiece.$implementerpersonpiece."; ";
		...
		// if($typelist == 'rawdata' || $typelist == 'totalalldata')
		// {
		// 	//Fetch Raw Data
		// 	$query1 = "select imp_cfentries.businessname as businessname, imp_cfentries.customerid as customerid,
		// 	imp_cfentries.dealername as dealername from imp_cfentries
		// 	left join inv_mas_customer on inv_mas_customer.slno = right(imp_cfentries.customerid,5)
		// 	left join imp_implementation on right(imp_cfentries.customerid,5) = imp_implementation.customerreference
		// 	left join inv_mas_implementer on inv_mas_implementer.slno = imp_implementation.assignimplemenation 
		// 	left join inv_mas_district on inv_mas_customer.district = inv_mas_district.districtcode 
		...
		// 			else
		// 				$color = "#f7faff";
		// 			$raw_grid .= '<tr bgcolor='.$color.'>';
		// 			$raw_grid .= "<td nowrap='nowrap' class='td-border-grid' align='left'>".$countvalue."</td>";
		// 			$raw_grid .= "<td nowrap='nowrap' class='td-border-grid' align='left'>".gridtrim($fetch1['businessname'])."</td>";
		// 			$raw_grid .= "<td nowrap='nowrap' class='td-border-grid' align='left'>".$fetch1['customerid']."</td>";
		// 			$raw_grid .= "<td nowrap='nowrap' class='td-border-grid' align='left'>".$fetch1['dealername']."</td>";
		// 			$raw_grid .= "<td nowrap='nowrap' class='td-border-grid' align='center'>-</td>";
		// 			$raw_grid .= "<td nowrap='nowrap' class='td-border-grid' align='center'>-</td>";
		// 			$raw_grid .= "<td nowrap='nowrap' class='td-border-grid' align='center'>-</td>";
		// 			$raw_grid .= "<td nowrap='nowrap' class='td-border-grid' align='center'>-</td>";
		// 			$raw_grid .= "<td nowrap='nowrap' class='td-border-grid' align='center'>-</td>";
		...
		// 	{
		// 		$raw_grid .= '<tr><td colspan="14" bgcolor="#FFFFD2" class="td-border-grid" style="font-weight:bold"><font color="#FF4F4F" >No Records</font></td></tr>';
		...
		if($typelist == 'totalalldata')
		{
			...
			// {
			// 	$grid = $startgrid.$raw_grid.$activated_grid.$endgrid;
			// }
			// elseif((mysqli_num_rows($result1) == 0) && (mysqli_num_rows($result) <> 0))
			// {
			// 	$grid = $startgrid.$activated_grid.$raw_grid.$endgrid;
			// }
			// elseif((mysqli_num_rows($result1) <> 0) && (mysqli_num_rows($result) <> 0))
			// {
			// 	$grid = $startgrid.$activated_grid.$raw_grid.$endgrid;
	...
	case 'detailedactivegrid':
	{
		...
		if($typelist == 'totalactivatedata')
			$headingname = 'List of customers for whom implementation has been created successfully';
		elseif($typelist == 'duedata')
			$headingname = 'List of customers for whom implementation is DUE for completion';
		elseif($typelist == 'completeddata')
			$headingname = 'List of customers for whom implementation has been COMPLETED';
		...
		//Fetch All Data
		if($typelist == 'duedata' || $typelist == 'totalactivatedata')
		{
			//Fetch Due Data
			$query2 = "select inv_mas_customer.slno as slnonumber,inv_mas_customer.businessname as custname,inv_mas_customer.customerid as cusid,
			inv_mas_dealer.businessname as salesperson, inv_mas_implementer.businessname as Implementer,
			...
			where imp_implementation.implementationstatus <> 'completed' ".$finalliststatus.$regionpiece.$state_typepiece.$district_typepiece.$dealer_typepiece.$branchpiece.$typepiece.$categorypiece.$implementerpersonpiece.";";
			$result = runmysqlquery($query2);
			...
			if(mysqli_num_rows($result) <> 0)
			{
				while($fetch = mysqli_fetch_array($result))
				{
					...
					if($i_n%2 == 0)
						...
						$color = "#f7faff";
					$duedata_grid .= '<tr bgcolor='.$color.'>';
					$duedata_grid .= "<td nowrap='nowrap' class='td-border-grid' align='left'>".$slno."</td>";
					$duedata_grid .= "<td nowrap='nowrap' class='td-border-grid' align='left'>".gridtrim($fetch['custname'])."</td>";
					$duedata_grid .= "<td nowrap='nowrap' class='td-border-grid' align='left'>".cusidcombine($fetch['cusid'])."</td>";
					$duedata_grid .= "<td nowrap='nowrap' class='td-border-grid' align='left'>".$fetch['salesperson']."</td>";
					$duedata_grid .= "<td nowrap='nowrap' class='td-border-grid' align='left'>".$fetch['Implementer']."</td>";
					$duedata_grid .= "<td nowrap='nowrap' class='td-border-grid' align='left'>".changedateformat($fetch['invoicedate'])."</td>";
					$duedata_grid .= "<td nowrap='nowrap' class='td-border-grid' align='left'>".changedateformat($fetch['requestdate'])."</td>";
					$duedata_grid .= "<td nowrap='nowrap' class='td-border-grid' align='left'>".changedateformat($fetch['branchappdate'])."</td>";
					$duedata_grid .= "<td nowrap='nowrap' class='td-border-grid' align='left'>".changedateformat($fetch['coordinatorappdate'])."</td>";
					$duedata_grid .= "<td nowrap='nowrap' class='td-border-grid' align='left'>".changedateformat($fetch['assignimpldate'])."</td>";
					$duedata_grid .= "<td nowrap='nowrap' class='td-border-grid' align='left'>".changedateformat($fetch['completeddate'])."</td>";
					$duedata_grid .= "<td nowrap='nowrap' class='td-border-grid' align='left'>".strtolower($fetch['customization'])."</td>";
					$duedata_grid .= "<td nowrap='nowrap' class='td-border-grid' align='left'>".strtolower($fetch['attendanceapplicable'])."</td>";
					$duedata_grid .= '<td nowrap="nowrap" class="td-border-grid" align="center"> <a onclick="viewhistory(\''.$fetch['slnonumber'].'\',\''.$fetch['impslno'].'\');" class="resendtext" style = "cursor:pointer"> View History >></a> </td>';
					$duedata_grid .= "</tr>";
				}
			}
			elseif(mysqli_num_rows($result) == 0)
			{
				$duedata_grid .= '<tr><td colspan="14" bgcolor="#FFFFD2" class="td-border-grid" style="font-weight:bold"><font color="#FF4F4F" >No Records</font></td></tr>';
			}
		}
		if($typelist == 'completeddata' || $typelist == 'totalactivatedata')
		{
			...
			imp_implementation.branchapproval = 'yes' AND imp_implementation.coordinatorreject = 'no' AND 
			imp_implementation.coordinatorapproval = 'yes' AND imp_implementation.implementationstatus = 'completed' ".$finalliststatus.$regionpiece.$state_typepiece.$district_typepiece.$dealer_typepiece.$branchpiece.$typepiece.$categorypiece.$implementerpersonpiece.";";
			$result1 = runmysqlquery($query1);
			$i_n = 0;$slno1 = 0;
			if(mysqli_num_rows($result1) <> 0)
		...
		if($typelist == 'totalactivatedata')
		{
			...
			elseif((mysqli_num_rows($result) == 0) && (mysqli_num_rows($result1) <> 0))
			{
				$grid = $startgrid.$completeddata_grid.$duedata_grid.$endgrid;
			}
			elseif((mysqli_num_rows($result1) == 0) && (mysqli_num_rows($result) <> 0))
			{
				$grid = $startgrid.$duedata_grid.$completeddata_grid.$endgrid;
			}
			elseif((mysqli_num_rows($result1)<> 0) && (mysqli_num_rows($result) <> 0))
			{
				$grid = $startgrid.$duedata_grid.$completeddata_grid.$endgrid;
		...
		elseif($typelist == 'duedata')
			$grid = $startgrid.$duedata_grid.$endgrid;
		elseif($typelist == 'completeddata')
			$grid = $startgrid.$completeddata_grid.$endgrid;
	...
	case 'impdetailedimplduegrid':
	{
		...
		if($typelist == 'implsale' || $typelist == 'totalimpduedata')
		{
			...
			left join (select slno,min(visiteddate) as visiteddate,impref 
			from imp_implementationdays  where iccattachment = 'yes' group by impref)as dummy on dummy.impref = imp_implementation.slno
			where imp_implementation.branchapproval = 'no' and imp_implementation.implementationstatus <> 'completed' ".$finalliststatus.$regionpiece.$state_typepiece.$district_typepiece.$dealer_typepiece.$branchpiece.$typepiece.$categorypiece.$implementerpersonpiece."";
			$result = runmysqlquery($query2);
		...
		if($typelist == 'implimplementation' || $typelist == 'totalimpduedata')
		{
			...
			left join (select slno,min(visiteddate) as visiteddate,impref 
			from imp_implementationdays  where iccattachment = 'yes' group by impref)as dummy on dummy.impref = imp_implementation.slno where 
			imp_implementation.branchapproval <> 'no' and imp_implementation.implementationstatus <> 'completed' ".$finalliststatus.$regionpiece.$state_typepiece.$district_typepiece.$dealer_typepiece.$branchpiece.$typepiece.$categorypiece.$implementerpersonpiece.";";
			$result1 = runmysqlquery($query1);
			$i_n = 0;$slno1 = 0;
			if(mysqli_num_rows($result1) <> 0)
			{
	...
	case 'detailedregionduegrid':
	{
		...
		if($typelist == 'totalregduedata')
			$headingname = 'List of total customers with Implementation Region wise';
		elseif($typelist == 'totalregbkgdata')
			$headingname = 'List of customers with Due Implementations of BKG Region';
		elseif($typelist == 'totalregbkmdata')
			$headingname = 'List of customers with Due Implementations of BKM Region';
		elseif($typelist == 'totalregcsddata')
			$headingname = 'List of customers with Due Implementations of CSD Region';
		...
		$regionquery = 'select distinct inv_mas_region.slno,lower(inv_mas_region.category) as regionname from inv_mas_region where inv_mas_region.slno <> "4"  order by inv_mas_region.category ';
		...
		while($fetchresult = mysqli_fetch_array($fetchregion))
		{
			...
			left join (select slno,min(visiteddate) as visiteddate,impref 
			from imp_implementationdays  where iccattachment = 'yes' group by impref)as dummy on dummy.impref = imp_implementation.slno
			where imp_implementation.implementationstatus <> 'completed' and  inv_mas_region.slno = '".$fetchresult['slno']."' ".$finalliststatus.$regionpiece.$state_typepiece.$district_typepiece.$dealer_typepiece.$branchpiece.$typepiece.$categorypiece.$implementerpersonpiece.";";
			$result = runmysqlquery($query2);
			$displaygrid = "";
			$i_n = 0;$slno = 0;
			if(mysqli_num_rows($result) <> 0)
			{
				while($fetch = mysqli_fetch_array($result))
				{
					...
					if($i_n%2 == 0)
						...
						$color = "#f7faff";
					$displaygrid .= '<tr bgcolor='.$color.'>';
					$displaygrid .= "<td nowrap='nowrap' class='td-border-grid' align='left'>".$countvalue."</td>";
					$displaygrid .= "<td nowrap='nowrap' class='td-border-grid' align='left'>".gridtrim($fetch['custname'])."</td>";
					$displaygrid .= "<td nowrap='nowrap' class='td-border-grid' align='left'>".cusidcombine($fetch['cusid'])."</td>";
					$displaygrid .= "<td nowrap='nowrap' class='td-border-grid' align='left'>".$fetch['salesperson']."</td>";
					$displaygrid .= "<td nowrap='nowrap' class='td-border-grid' align='left'>".$fetch['Implementer']."</td>";
					$displaygrid .= "<td nowrap='nowrap' class='td-border-grid' align='left'>".changedateformat($fetch['invoicedate'])."</td>";
					$displaygrid .= "<td nowrap='nowrap' class='td-border-grid' align='left'>".changedateformat($fetch['requestdate'])."</td>";
					$displaygrid .= "<td nowrap='nowrap' class='td-border-grid' align='left'>".changedateformat($fetch['branchappdate'])."</td>";
					$displaygrid .= "<td nowrap='nowrap' class='td-border-grid' align='left'>".changedateformat($fetch['coordinatorappdate'])."</td>";
					$displaygrid .= "<td nowrap='nowrap' class='td-border-grid' align='left'>".changedateformat($fetch['assignimpldate'])."</td>";
					$displaygrid .= "<td nowrap='nowrap' class='td-border-grid' align='left'>".changedateformat($fetch['completeddate'])."</td>";
					$displaygrid .= "<td nowrap='nowrap' class='td-border-grid' align='left'>".strtolower($fetch['customization'])."</td>";
					$displaygrid .= "<td nowrap='nowrap' class='td-border-grid' align='left'>".strtolower($fetch['attendanceapplicable'])."</td>";
					$displaygrid .= '<td nowrap="nowrap" class="td-border-grid" align="center"> <a onclick="viewhistory(\''.$fetch['slnonumber'].'\',\''.$fetch['impslno'].'\');" class="resendtext" style = "cursor:pointer"> View History >></a> </td>';
					$displaygrid .= "</tr>";
				}
			}
			elseif(mysqli_num_rows($result) == 0)
		...
		elseif($typelist == 'totalregbkgdata')
		{
			if($regionarray[0] <> '')
			{
				$grid = $startgrid.$regionarray[0].$endgrid;
			}
			else
			{
				$nodatagrid .= '<tr><td height ="20px" colspan="14" bgcolor="#FFFFD2" class="td-border-grid" style="font-weight:bold"><font color="#FF4F4F" >No Records</font></td></tr>';
		...
		elseif($typelist == 'totalregbkmdata')
		{
			if($regionarray[1] <> '')
			{
				$grid = $startgrid.$regionarray[1].$endgrid;
			}
			else
			{
				$nodatagrid .= '<tr><td height ="20px" colspan="14" bgcolor="#FFFFD2" class="td-border-grid" style="font-weight:bold"><font color="#FF4F4F" >No Records</font></td></tr>';
		...
		elseif($typelist == 'totalregcsddata')
		{
			if($regionarray[2] <> '')
			{
				$grid = $startgrid.$regionarray[2].$endgrid;
			}
			else
			{
				$nodatagrid .= '<tr><td height ="20px" colspan="14" bgcolor="#FFFFD2" class="td-border-grid" style="font-weight:bold"><font color="#FF4F4F" >No Records</font></td></tr>';
	...
	case 'displaystatusgrid':
	{
		...
		for($i=0;$i<8;$i++)
		{
			...
			where ".$statustype[$i]." ".$finalliststatus.$regionpiece.$state_typepiece.$district_typepiece.$dealer_typepiece.$branchpiece.$typepiece.$categorypiece.$implementerpersonpiece.";";
			$result1 = runmysqlquery($query1);
	...
	case 'detailedbranchwisegrid':
	{
		...
		$branchquery1 = 'select distinct inv_mas_branch.slno,inv_mas_branch.branchname as branchname from inv_mas_branch order by inv_mas_branch.branchname ';
		$fetchbranch1 =  runmysqlquery($branchquery1);
		while($fetchresult4 = mysqli_fetch_array($fetchbranch1))
		{
			$brancharray2[] = $fetchresult4['branchname'];
		}
		
		for($k=0;$k<count($brancharray2);$k++)
		{
			if($typelist == 'branch'.($k+1))
			{
				$branchname1 = 'branchtype'.($k+1);
				$branchname1 = $brancharray2[$k];
				break;
			}
		}
		if($typelist == 'totalbranchwisedue')
			$headingname = 'List of total customers with Implementation Branch wise';
		else
			$headingname = 'List of customers with Due Implementations for '.$branchname1.'';
		...
		$branchquery = 'select distinct inv_mas_branch.slno,lower(inv_mas_branch.branchname) as branchname from inv_mas_branch order by inv_mas_branch.branchname ';
		$fetchbranch =  runmysqlquery($branchquery);
		while($fetchresult = mysqli_fetch_array($fetchbranch))
		{
			//Fetch With Sales Data
			$query2 = "select inv_mas_customer.slno as slnonumber, inv_mas_customer.businessname as custname,inv_mas_customer.customerid as cusid,
inv_mas_dealer.businessname as salesperson, inv_mas_implementer.businessname as Implementer,
...
left join (select slno,min(visiteddate) as visiteddate,impref 
from imp_implementationdays  where iccattachment = 'yes' group by impref)as dummy on dummy.impref = imp_implementation.slno
where imp_implementation.implementationstatus <> 'completed' and inv_mas_branch.slno = '".$fetchresult['slno']."' ".$finalliststatus.$regionpiece.$state_typepiece.$district_typepiece.$dealer_typepiece.$branchpiece.$typepiece.$categorypiece.$implementerpersonpiece.";";
			$result = runmysqlquery($query2);
			$displaygrid = "";
			$i_n = 0;$slno = 0;
			if(mysqli_num_rows($result) <> 0)
		...
		for($k=0;$k<count($brancharray);$k++)
		{
			if($typelist == 'branch'.($k+1))
			{
				if($brancharray[$k] <> '')
				{
					$grid = $startgrid.$brancharray[$k].$endgrid;
				}
				else
				{
					$nodatagrid .= '<tr><td height ="20px" colspan="14" bgcolor="#FFFFD2" class="td-border-grid" style="font-weight:bold"><font color="#FF4F4F" >No Records</font></td></tr>';
	...
	from imp_implementationdays  where iccattachment = 'yes' group by impref)as dummy on 
	dummy.impref = imp_implementation.slno where 
	inv_mas_dealer.slno in(".$slnolist.")";
			$result1 = runmysqlquery($query1);
		}
		$i_n = 0;$slno1 = 0;
		if(mysqli_num_rows($result1) <> 0)
	...
	{
		$query = "SELECT distinct inv_mas_customer.slno as slno,inv_mas_customer.businessname as businessname FROM imp_implementation left join inv_mas_customer on inv_mas_customer.slno = imp_implementation.customerreference ORDER BY inv_mas_customer.businessname";
		$result = runmysqlquery($query);
		$grid = '';
		$count = 1;
		while($fetch = mysqli_fetch_array($result))
	...
	case 'implementation':
	{
		...
		if($count > 0)
		{
			...
			/*$query121 = "select  count(*) as count from inv_invoicenumbers where invoiceno = '".$fetch['invoicenumber']."';";
			$fetch123 = runmysqlqueryfetch($query121);
			if($fetch123['count'] > 0)
			{
				$query156 = "select slno from inv_invoicenumbers where inv_invoicenumbers.invoiceno = '".$fetch['invoicenumber']."'";
				$result = runmysqlqueryfetch($query156);
				$invoicegrid = '1$$'.$result['slno'];
			}
			else
			{
				$query156 = "select distinct inv_mas_product.productname as product, imp_cfentries.usagetype,  imp_cfentries.purchasetype, inv_mas_scratchcard.scratchnumber, imp_cfentries.customerid from imp_cfentries 
 				left join inv_mas_product on inv_mas_product.productcode = imp_cfentries.productcode left join inv_mas_scratchcard on inv_mas_scratchcard.cardid = imp_cfentries.cardid where imp_cfentries.invoiceno = '".$fetch['invoicenumber']."'";
			...
			while($fetch11 = mysqli_fetch_array($result11))
			{
				...
				$assigngrid .= "<td nowrap='nowrap' class='td-border-grid'>".wordwrap(gridtrim($fetch11['remarks']),20,"<br>\n")."</td>";
				$assigngrid .= "</tr>";
			}
			...
			$query14 = "SELECT imp_implementationdays.slno,assigneddate,imp_implementationdays.remarks,visitedstartflag,visitedendflag,impstatus,businessname,databackuppath,iccattachmentpath from imp_implementationdays
	...
	case 'searchlist':
	{
		...
		while($fetch0 = mysqli_fetch_array($result0))
		{
			...
			left join inv_mas_dealer on imp_implementation.dealerid = inv_mas_dealer.slno 
			left join inv_mas_region on inv_mas_region.slno = inv_mas_dealer.region 
			left join inv_mas_branch on inv_mas_branch.slno = inv_mas_dealer.branch where imp_implementation.slno <> '99999999' and imp_implementation.slno = '".$fetch0['impslno']."'  ".$finalliststatus.$regionpiece.$state_typepiece.$district_typepiece.$dealer_typepiece.$branchpiece.$typepiece.$categorypiece.$implementerpersonpiece." ORDER BY inv_mas_customer.businessname;";
			$result = runmysqlquery($query);
			// left join imp_cfentries on right(imp_cfentries.customerid,5) = imp_implementation.customerreference
