...
{
	$grid = '';
	$query = "select * from inv_invoicenumbers 	where inv_invoicenumbers.slno = '".$newarray[$j]."' order by inv_invoicenumbers.slno;";
	...
	while($fetch = mysqli_fetch_array($result))
	{
		...
		if(is_numeric($fetch_customer_details['gst_no']))
		{
			if($fetch['gst_no']!= "" && $fetch['gst_no']!= '0')
			{
	        	...
	        	where inv_invoicenumbers.gst_no=".$fetch['gst_no']." order by gstin_id desc limit 1";
	...
	if($gst_tax_date <= $invoicedate)
	{
	        ...
	        //$msg = file_get_contents("../pdfbillgeneration/bill-format-gst.php");
				$msg = file_get_contents("../pdfbillgeneration/bill-format-gst-withouthead.php");
			else
			{
				if($fetchresult['irn'] == "")
					$msg = file_get_contents("../pdfbillgeneration/bill-format-gst.php");
