...
switch($switchtype)
{
	case 'generateinvoicenolist':
	{
		...
		$query = "SELECT slno,invoiceno FROM inv_dealer_invoicenumbers where purchasetype = 'Product'  ORDER BY slno";
	...
	/*case 'getuserinvoicelist':
	{
		...
		$query = "select slno, invoiceno from inv_dealer_invoicenumbers where right(customerid,5) = '".$customerreference."'";
		$result = runmysqlquery($query);
	...
	case 'listinvoicedetails':
	{
		...
		<td width="10%"  class="grey-td-border-grid" style="text-align:left;">Usage Type</td>
			...
			for($i=0,$j=0;$i<count($descriptionsplit),$j<count($productquantitysplit);$i++,$j++)
			{
				$descriptionline = explode('$',$descriptionsplit[$i]);
				{
					if($description <> '')
					{
						...
						$pingrid .= '<td  class="grey-td-border-grid" style="text-align: right;" width="10%"><input type="text"  style="width:75px; text-align: right;" autocomplete="off" maxlength="10" id="productamount'.$count.'" class="'.$classdefine.'" name="productamount'.$count.'" value="'.$descriptionline[6].$appendzero.'" '.$disable.'  onkeyup ="calculatenormalprice(\'\',\''.$count.'\');" onchange ="calculatenormalprice(\'validate\',\''.$count.'\');"></td>';
						$pingrid .= "</tr>";
		...
		<td style="text-align: right;" colspan="6"  class="grey-td-border-grid"><strong>Total</strong></td>
    ...
    $grid = '<table width="100%" border="0" cellspacing="0" cellpadding="0" bgcolor="#FFFFFF"><tr><td style="padding:3px"><div  align="center"><font color="#FF0000"><font size="2px"><strong>'.$fetchresult['invoiceheading'].'</strong></font></font></div></td></tr><tr><td><table width="100%" border="0" cellspacing="0" cellpadding="0"><tr><td width="100%" valign="top">';
	$grid .= '<table width="100%" border="0" cellspacing="0" cellpadding="4" class = "table-border-grid">';
	$grid .= '<tr class="tr-grid-header"><td width="60%" colspan="2" align="left" valign="top" ><strong>Dealer Details</strong></td>
	<td width="46%" align="left" valign="top" ><strong>Invoice Details</strong></td></tr>';
	$grid .= '<tr><td colspan="2" style="text-align:left" class ="td-border-grid"><strong>Dealer GSTIN:</strong> <span id="customerid">'.$gst_customer.'</span><input type="hidden"  style="width: 80px; text-align: right;" maxlength="10" id="seztax" name="seztax" value="'.$fetchresult['seztaxtype'].'"></td><td style="text-align:left" class ="td-border-grid"><strong>Date: </strong><span id="billdate">'.changedateformatwithtime($fetchresult['createddate']).'</span></td></tr>';
	$grid .= '<tr height="30px"><td width="60%" colspan="2" align="left" valign="top"  class ="td-border-grid" ><span style="text-align:left"><input name="businessname" type="text" class="'.$classdefine.'" id="businessname" size="53" maxlength="400"  autocomplete="off" value = "'.$fetchresult['businessname'].'" '.$disable.'/></span></td><td width="46%" align="left" valign="top"   class ="td-border-grid"><span style="text-align:left"><strong>No:</strong> <span style="text-align:right" id="billnumber">'.$fetchresult['invoiceno'].'</span></span></td></tr>';
	$grid .= '<tr height="60px"><td width="60%" colspan="2" align="left" valign="top"  class ="td-border-grid"><span style="text-align:left"><textarea name="address" type="text" class="'.$classdefine.'" id="address"   autocomplete="off" rows="2" cols="50" '.$disable.' >'.stripslashes(stripslashes($fetchresult['address'])).'</textarea></span></td> <td style="text-align:left" width="40%"    class ="td-border-grid" ><table width="100%" border="0" cellspacing="0" cellpadding="0">
	<tr><td>Marketing Exe:</td><td><span id="dealerinputdiv"style="display:block" ><input type="text" name="displaydealer" id="displaydealer" value="'.$fetchresult['dealername'].'"  '.$disable.'  class="swifttext"  style="width:160px;"/></span> <span style="display: none;" id="dealerselectiondiv">'.$dealerlist.'</span></td></tr></table></td></tr>';
    ...
    if(($fetchresult['cgst'] != '0') &&  ($fetchresult['sgst'] != '0') || ($customer_gst_code == 29 && $fetchresult['cgst'] == '0' &&  $fetchresult['sgst'] == '0' && ($fetch['seztaxtype'] == 'yes' || $fetch_first_gst_details['sez_enabled'] == 'yes')))
    {
		...
		$pingrid .= '<tr height="30px"><td style="text-align: right;" colspan="6"  class="grey-td-border-grid"><strong>CGST @ '.$cgst_tax_rate.'%</strong></td><td width="10%" style="text-align: right;"  class="grey-td-border-grid"><input type="text" style="width: 80px; text-align: right;" autocomplete="off" maxlength="20" id="cgst_tax" class="'.$classdefine.'" name="cgst_tax" value = "'.$fetchresult['cgst'].'" '.$disable.'></td></tr>';
		$pingrid .= '<tr height="30px"><td style="text-align: right;" colspan="6"  class="grey-td-border-grid"><strong>SGST @ '.$sgst_tax_rate.'%</strong></td><td width="10%" style="text-align: right;"  class="grey-td-border-grid"><input type="text" style="width: 80px; text-align: right;" autocomplete="off" maxlength="20" id="sgst_tax" class="'.$classdefine.'" name="sgst_tax" value = "'.$fetchresult['sgst'].'" '.$disable.'></td></tr>';
		$pingrid .= '<tr height="30px"><td style="text-align: right;" colspan="6"  class="grey-td-border-grid"><strong>Net Amount</strong></td><td width="10%" style="text-align: right;"  class="grey-td-border-grid"><input type="text" style="width: 80px; text-align: right;" autocomplete="off" maxlength="10" id="netamount" class="'.$classdefine.'" name="netamount"  value = "'.$net_amount.$appendzero.'" '.$disable.'></td></tr>';
		$pingrid .= '<tr><td style="text-align: left;" colspan="7"  class="grey-td-border-grid" id="rupeeinwords"><strong>Rupee In Words</strong>: <span id="amountinwords">'.$fetchresult['amountinwords'].'</span> only</td></tr></table>';
	}
	else
	{
		$net_amount = $fetchresult['netamount'];
		$pingrid .= '<tr height="30px"><td style="text-align: right;" colspan="6"  class="grey-td-border-grid"><strong>IGST @ '.$igst_tax_rate.'%</strong></td><td width="10%" style="text-align: right;"  class="grey-td-border-grid"><input type="text" style="width: 80px; text-align: right;" autocomplete="off" maxlength="20" id="igst_tax" class="'.$classdefine.'" name="igst_tax" value = "'.$fetchresult['igst'].'" '.$disable.'></td></tr>';
		$pingrid .= '<tr height="30px"><td style="text-align: right;" colspan="6"  class="grey-td-border-grid"><strong>Net Amount</strong></td><td width="10%" style="text-align: right;"  class="grey-td-border-grid"><input type="text" style="width: 80px; text-align: right;" autocomplete="off" maxlength="10" id="netamount" class="'.$classdefine.'" name="netamount"  value = "'.$net_amount.$appendzero.'" '.$disable.'></td></tr>';
		$pingrid .= '<tr><td style="text-align: left;" colspan="7"  class="grey-td-border-grid" id="rupeeinwords"><strong>Rupee In Words</strong>: <span id="amountinwords">'.$fetchresult['amountinwords'].'</span> only</td></tr></table>';    
	}
	...
	case 'editinvoice':
	{
		...
		$servicecount = 0;$productarraycount = 0;
		...
		if($productslnovalues <> '')
		{
			for($i = 0; $i < $productlistsplitcount; $i++)
			{
				...
				$query7 = "UPDATE inv_dealercard set productcode = '".$productlistsplit[$i]."', usagetype = '".$usagetype."', purchasetype = '".$purchasetypesplit[$i]."',addlicence = '".$addlicence."' where cardid = '".$productpinvalue[$i][1]."' and dealerinvoiceid = '".$invoiceno."' ;";
		...
		//Update the inv_dealer_invoicenumbers table
		$invoicequery = "update inv_dealer_invoicenumbers set description = '".$description."', amount = '".$totalproductprice."',igst = '".$igsttax."',cgst = '".$cgsttax."',sgst = '".$sgsttax."', netamount = '".$netamount."',businessname = '".$businessname."',phone =  '".$phone."',cell = '".$cell."',emailid = '".$emailid."',contactperson = '".$contactperson."',address ='".addslashes($address)."', amountinwords = '".$amountinwords."',  invoiceremarks = '".$invoiceremarks."', servicedescription = '".$servicegrid."',servicetype = '".$servicelist."',serviceamount = '".$serviceamountvalues."', dealername = '".$result55['dealername']."', dealerid = '".$dealerid."',status = 'EDITED', editeddate = '".date('Y-m-d').' '.date('H:i:s')."',editedip = '".$_SERVER['REMOTE_ADDR']."',editedby = '".$userid ."',editedremarks = '".$editremarks."',amountinwords = '".$amountinwords."',totalproductpricearray = '".$totalproductpricearray."',actualproductpricearray='".$actualproductpricearray."',products = '".$productlist."'  where slno  ='".$invoiceno."';";
		$invoiceresult = runmysqlquery($invoicequery);
		
		//echo "this is messis";
		//exit();
		
		$eventquery = "Insert into inv_logs_event(userid,system,eventtype,eventdatetime,remarks) values('".$userid."','".$_SERVER['REMOTE_ADDR']."','260','".date('Y-m-d').' '.date('H:i:s')."','Dealer_Invoice_editing')";
		...
		resenddealerinvoice($invoiceno);	
		
		$queryresult88 = "Select `status`,editedby,editeddate,editedremarks from inv_dealer_invoicenumbers where slno  ='".$invoiceno."';";
	...
	case 'cancelinvoice':
	{
		...
		$cnlquery = "select * from inv_dealer_invoicenumbers where slno  ='".$invoiceno."';";
		...
		if($irnstatus == 1)
		{
			...
			for($i=0,$j=0; $i<$productlistsplitcount,$j<count($productquantity); $i++,$j++)
			{
				...
				if($productpinvalue[$i][1]!= 0)
				{
					if($productquantity[$j] == 1)
					{
						...
						$cardidsplitarray = explode('-',$productpinvalue[$i][1]);
						...
						while($fetchcard = mysqli_fetch_array($resultcard))
						{
							...
							$blockquery = "insert into inv_blockcanceldetails(cardid,userid,datetime,status,remarks) 
							values('".$fetchcard['cardid']."','".$userid."','".date('Y-m-d').' '.date('H:i:s')."','Blocked','invoicing')";
							$blockresult = runmysqlquery($blockquery);
						}
					}
			...
			//Update the inv_dealer_invoicenumbers table
			$invoicequery = "update inv_dealer_invoicenumbers set status = 'CANCELLED',cancelleddate = '".date('Y-m-d').' '.date('H:i:s')."',cancelledip = '".$_SERVER['REMOTE_ADDR']."',cancelledby = '".$userid ."',cancelledremarks = '".$cancelremarks."'  where slno  ='".$invoiceno."';";
			$invoiceresult = runmysqlquery($invoicequery);
			
			$eventquery = "Insert into inv_logs_event(userid,system,eventtype,eventdatetime,remarks) values('".$userid."','".$_SERVER['REMOTE_ADDR']."','267','".date('Y-m-d').' '.date('H:i:s')."','Dealer_Invoice_cancellation')";
			$eventresult = runmysqlquery($eventquery);
			
			//resend the mail after Cancelled sucessfully	
			resenddealerinvoice($invoiceno);	
			
			$queryresult55 = "Select `status`,cancelledby,cancelleddate,cancelledremarks from inv_dealer_invoicenumbers where slno  ='".$invoiceno."';";
	...
	case 'searchdealerlist':
	{
		...
		$branchpiece = ($branch2 == "")?(""):(" AND inv_dealer_invoicenumbers.branchid = '".$branch2."' ");
		...
		{
			case "contactperson": 
				$query = "select inv_dealer_invoicenumbers.invoiceno,inv_dealer_invoicenumbers.slno from inv_dealer_invoicenumbers 
left join inv_mas_customer on inv_mas_customer.slno = right(inv_dealer_invoicenumbers.customerid,5)
left join inv_mas_district on inv_mas_district.districtcode = inv_mas_customer.district
where inv_dealer_invoicenumbers.contactperson LIKE '%".$textfield."%' AND left(inv_dealer_invoicenumbers.createddate,10) between '".$fromdate."' and '".$todate."' AND (".$finalproductlist.") ".$regionpiece.$district_typepiece.$state_typepiece.$dealer_typepiece.$branchpiece.$typepiece.$categorypiece.$generatedpiece.$statuspiece.$seriespiece."  ORDER BY invoiceno";
				break;
			case "phone":
				$query = "select inv_dealer_invoicenumbers.invoiceno,inv_dealer_invoicenumbers.slno from inv_dealer_invoicenumbers 
left join inv_mas_customer on inv_mas_customer.slno = right(inv_dealer_invoicenumbers.customerid,5)
left join inv_mas_district on inv_mas_district.districtcode = inv_mas_customer.district
where inv_dealer_invoicenumbers.phone LIKE '%".$textfield."%' OR inv_dealer_invoicenumbers.cell LIKE '%".$textfield."%' AND left(inv_dealer_invoicenumbers.createddate,10) between '".$fromdate."' and '".$todate."' AND (".$finalproductlist.") ".$regionpiece.$district_typepiece.$state_typepiece.$dealer_typepiece.$branchpiece.$typepiece.$categorypiece.$generatedpiece.$statuspiece.$seriespiece."  ORDER BY invoiceno ";
				break;
			case "place":
				$query = "select inv_dealer_invoicenumbers.invoiceno,inv_dealer_invoicenumbers.slno from inv_dealer_invoicenumbers 
left join inv_mas_customer on inv_mas_customer.slno = right(inv_dealer_invoicenumbers.customerid,5)
left join inv_mas_district on inv_mas_district.districtcode = inv_mas_customer.district
where inv_dealer_invoicenumbers.place LIKE '%".$textfield."%' AND left(inv_dealer_invoicenumbers.createddate,10) between '".$fromdate."' and '".$todate."' AND (".$finalproductlist.") ".$regionpiece.$district_typepiece.$state_typepiece.$dealer_typepiece.$branchpiece.$typepiece.$categorypiece.$generatedpiece.$statuspiece.$seriespiece."  ORDER BY invoiceno";
				break;
			case "emailid":
				$query = "select inv_dealer_invoicenumbers.invoiceno,inv_dealer_invoicenumbers.slno from inv_dealer_invoicenumbers 
left join inv_mas_customer on inv_mas_customer.slno = right(inv_dealer_invoicenumbers.customerid,5)
left join inv_mas_district on inv_mas_district.districtcode = inv_mas_customer.district
where inv_dealer_invoicenumbers.emailid LIKE '%".$textfield."%' AND left(inv_dealer_invoicenumbers.createddate,10) between '".$fromdate."' and '".$todate."' AND (".$finalproductlist.") ".$regionpiece.$district_typepiece.$state_typepiece.$dealer_typepiece.$branchpiece.$typepiece.$categorypiece.$generatedpiece.$statuspiece.$seriespiece."  ORDER BY invoiceno";
				break;
				case "cardid":
					$query = "select inv_dealer_invoicenumbers.invoiceno,inv_dealer_invoicenumbers.slno from inv_dealer_invoicenumbers 
left join inv_mas_customer on inv_mas_customer.slno = right(inv_dealer_invoicenumbers.customerid,5)
...
where inv_mas_scratchcard.cardid LIKE '%".$textfield."%' AND left(inv_dealer_invoicenumbers.createddate,10) between '".$fromdate."' and '".$todate."' AND (".$finalproductlist.") ".$regionpiece.$district_typepiece.$state_typepiece.$dealer_typepiece.$branchpiece.$typepiece.$categorypiece.$generatedpiece.$statuspiece.$seriespiece."  ORDER BY invoiceno";
					break;
			case "scratchnumber":
				$query = "select inv_dealer_invoicenumbers.invoiceno,inv_dealer_invoicenumbers.slno from inv_dealer_invoicenumbers 
left join inv_mas_customer on inv_mas_customer.slno = right(inv_dealer_invoicenumbers.customerid,5)
...
inv_mas_scratchcard.scratchnumber LIKE '%".$textfield."%' AND left(inv_dealer_invoicenumbers.createddate,10) between '".$fromdate."' and '".$todate."' AND (".$finalproductlist.") ".$regionpiece.$district_typepiece.$state_typepiece.$dealer_typepiece.$branchpiece.$typepiece.$categorypiece.$generatedpiece.$statuspiece.$seriespiece."  ORDER BY invoiceno";
				break;
...
left join inv_mas_customer on inv_mas_customer.slno = right(inv_dealer_invoicenumbers.customerid,5)
left join inv_mas_district on inv_mas_district.districtcode = inv_mas_customer.district
where inv_dealer_invoicenumbers.invoiceno LIKE '%".$textfield."%' AND left(inv_dealer_invoicenumbers.createddate,10) between '".$fromdate."' and '".$todate."' AND (".$finalproductlist.") ".$regionpiece.$district_typepiece.$state_typepiece.$dealer_typepiece.$branchpiece.$typepiece.$categorypiece.$generatedpiece.$statuspiece.$seriespiece."  ORDER BY invoiceno ";
				break;
...
left join inv_mas_customer on inv_mas_customer.slno = right(inv_dealer_invoicenumbers.customerid,5)
left join inv_mas_district on inv_mas_district.districtcode = inv_mas_customer.district
where inv_dealer_invoicenumbers.businessname LIKE '%".$textfield."%' AND left(inv_dealer_invoicenumbers.createddate,10) between '".$fromdate."' and '".$todate."' AND  (".$finalproductlist.") ".$regionpiece.$district_typepiece.$state_typepiece.$dealer_typepiece.$branchpiece.$typepiece.$categorypiece.$generatedpiece.$statuspiece.$seriespiece."  ORDER BY invoiceno";
				break;
		}
...
function updateaddlinebreak($linecount)
{
	switch($linecount)
	{
		...
		case '2':
		{
			$linebreak = '<tr><td  class="grey-td-border-grid"><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br></td><td  class="grey-td-border-grid">&nbsp;</td><td  class="grey-td-border-grid">&nbsp;</td><td  class="grey-td-border-grid">&nbsp;</td><td  class="grey-td-border-grid">&nbsp;</td><td  class="grey-td-border-grid">&nbsp;</td><td  class="grey-td-border-grid">&nbsp;</td></tr>';
		}
		break;
		case '3':
		{
