...
switch($switchtype)
{
	case 'searchinvoices':
	{
		...
		$querycase = "select distinct inv_dealer_invoicenumbers.slno,left(inv_dealer_invoicenumbers.createddate,10) as createddate,inv_dealer_invoicenumbers.invoiceno,inv_dealer_invoicenumbers.businessname,inv_dealer_invoicenumbers.amount,(IFNULL(inv_dealer_invoicenumbers.igst,0)+IFNULL(inv_dealer_invoicenumbers.sgst,0)+IFNULL(inv_dealer_invoicenumbers.cgst,0)) as servicetax,inv_dealer_invoicenumbers.netamount,inv_dealer_invoicenumbers.dealername,inv_dealer_invoicenumbers.createdby,inv_dealer_invoicenumbers.status,inv_dealer_invoicenumbers.products as products from inv_dealer_invoicenumbers
		...
		left join inv_dealercard on inv_dealercard.dealerinvoiceid = inv_dealer_invoicenumbers.slno";

		
		switch($databasefield)
		{
			case "contactperson": 
				$query = $querycase." where left(inv_dealer_invoicenumbers.createddate,10) between '".$fromdate."' and '".$todate."' AND inv_dealer_invoicenumbers.contactperson LIKE '%".$textfield."%' ".$finallistarray.$regionpiece.$district_typepiece.$state_typepiece.$dealer_typepiece.$branchpiece.$typepiece.$categorypiece.$generatedpiece.$statuspiece.$seriespiece.$usagetypepiece.$purchasetypepiece.$cancelledpiece.$receiptstatuspiece.$seriespiecenew." order by inv_dealer_invoicenumbers.slno";
				break;
			case "phone":
				$query = $querycase." where left(inv_dealer_invoicenumbers.createddate,10) between '".$fromdate."' and '".$todate."' AND inv_dealer_invoicenumbers.phone LIKE '%".$textfield."%' OR inv_dealer_invoicenumbers.cell LIKE '%".$textfield."%' ".$productlistarray.$itemlistarray.$regionpiece.$district_typepiece.$state_typepiece.$dealer_typepiece.$branchpiece.$typepiece.$categorypiece.$generatedpiece.$statuspiece.$seriespiece.$usagetypepiece.$purchasetypepiece.$cancelledpiece.$receiptstatuspiece.$seriespiecenew."  order by inv_dealer_invoicenumbers.slno ";
				break;
			case "place":
				$query = $querycase." where left(inv_dealer_invoicenumbers.createddate,10) between '".$fromdate."' and '".$todate."' AND inv_dealer_invoicenumbers.place LIKE '%".$textfield."%' ".$finallistarray.$regionpiece.$district_typepiece.$state_typepiece.$dealer_typepiece.$branchpiece.$typepiece.$categorypiece.$generatedpiece.$statuspiece.$seriespiece.$usagetypepiece.$purchasetypepiece.$cancelledpiece.$receiptstatuspiece.$seriespiecenew."  order by inv_dealer_invoicenumbers.slno";
				break;
			case "emailid":
				$query = $querycase." where left(inv_dealer_invoicenumbers.createddate,10) between '".$fromdate."' and '".$todate."' AND inv_dealer_invoicenumbers.emailid LIKE '%".$textfield."%'  ".$finallistarray.$regionpiece.$district_typepiece.$state_typepiece.$dealer_typepiece.$branchpiece.$typepiece.$categorypiece.$generatedpiece.$statuspiece.$seriespiece.$usagetypepiece.$purchasetypepiece.$cancelledpiece.$receiptstatuspiece.$seriespiecenew." order by inv_dealer_invoicenumbers.slno";
				break;
				case "cardid":
					$query = "select distinct inv_dealer_invoicenumbers.slno,left(inv_dealer_invoicenumbers.createddate,10) as createddate,inv_dealer_invoicenumbers.invoiceno,inv_dealer_invoicenumbers.businessname,inv_dealer_invoicenumbers.amount,(IFNULL(inv_dealer_invoicenumbers.igst,0)+IFNULL(inv_dealer_invoicenumbers.sgst,0)+IFNULL(inv_dealer_invoicenumbers.cgst,0)) as servicetax,inv_dealer_invoicenumbers.netamount,inv_dealer_invoicenumbers.dealername,inv_dealer_invoicenumbers.createdby,inv_dealer_invoicenumbers.status,inv_dealer_invoicenumbers.products as products from inv_dealer_invoicenumbers
					left join inv_mas_dealer on inv_mas_dealer.slno = inv_dealer_invoicenumbers.dealerreference
					left join inv_mas_district on inv_mas_district.districtcode = inv_mas_dealer.district
					left join inv_dealercard on inv_dealercard.dealerinvoiceid = inv_dealer_invoicenumbers.slno
					left join inv_mas_receipt on inv_mas_receipt.dealerinvoiceno = inv_dealer_invoicenumbers.slno
					LEFT JOIN inv_mas_scratchcard ON inv_mas_scratchcard.cardid = inv_dealercard.cardid
					where inv_mas_scratchcard.cardid LIKE '%".$textfield."%'  AND left(inv_dealer_invoicenumbers.createddate,10) between '".$fromdate."' and '".$todate."' ".$finallistarray.$regionpiece.$district_typepiece.$state_typepiece.$dealer_typepiece.$branchpiece.$typepiece.$categorypiece.$generatedpiece.$statuspiece.$seriespiece.$usagetypepiece.$purchasetypepiece.$cancelledpiece.$receiptstatuspiece.$seriespiecenew."  order by inv_dealer_invoicenumbers.slno";
					break;
			case "scratchnumber":
				$query = "select distinct inv_dealer_invoicenumbers.slno,left(inv_dealer_invoicenumbers.createddate,10) as createddate,inv_dealer_invoicenumbers.invoiceno,inv_dealer_invoicenumbers.businessname,inv_dealer_invoicenumbers.amount,(IFNULL(inv_dealer_invoicenumbers.igst,0)+IFNULL(inv_dealer_invoicenumbers.sgst,0)+IFNULL(inv_dealer_invoicenumbers.cgst,0)) as servicetax,inv_dealer_invoicenumbers.netamount,inv_dealer_invoicenumbers.dealername,inv_dealer_invoicenumbers.createdby,inv_dealer_invoicenumbers.status,inv_dealer_invoicenumbers.products as products from inv_dealer_invoicenumbers
				left join inv_mas_dealer on inv_mas_dealer.slno = inv_dealer_invoicenumbers.dealerreference
				...
				left join inv_mas_receipt on inv_mas_receipt.dealerinvoiceno = inv_dealer_invoicenumbers.slno
				LEFT JOIN inv_mas_scratchcard ON inv_mas_scratchcard.cardid = inv_dealercard.cardid
				where inv_mas_scratchcard.scratchnumber LIKE '%".$textfield."%'  AND left(inv_dealer_invoicenumbers.createddate,10) between '".$fromdate."' and '".$todate."' ".$finallistarray.$regionpiece.$district_typepiece.$state_typepiece.$dealer_typepiece.$branchpiece.$typepiece.$categorypiece.$generatedpiece.$statuspiece.$seriespiece.$usagetypepiece.$purchasetypepiece.$cancelledpiece.$receiptstatuspiece.$seriespiecenew."  order by inv_dealer_invoicenumbers.slno";
				break;
			
			case "invoiceno":
				$query = $querycase." where left(inv_dealer_invoicenumbers.createddate,10) between '".$fromdate."' and '".$todate."' AND  inv_dealer_invoicenumbers.invoiceno LIKE '%".$textfield."%' ".$finallistarray.$regionpiece.$district_typepiece.$state_typepiece.$dealer_typepiece.$branchpiece.$typepiece.$categorypiece.$generatedpiece.$statuspiece.$seriespiece.$usagetypepiece.$purchasetypepiece.$cancelledpiece.$receiptstatuspiece.$seriespiecenew."  order by inv_dealer_invoicenumbers.slno";
				break;
			case "invoiceamt":
				$query = $querycase." where left(inv_dealer_invoicenumbers.createddate,10) between '".$fromdate."' and '".$todate."' AND  inv_dealer_invoicenumbers.netamount LIKE '%".$textfield."%' ".$finallistarray.$regionpiece.$district_typepiece.$state_typepiece.$dealer_typepiece.$branchpiece.$typepiece.$categorypiece.$generatedpiece.$statuspiece.$seriespiece.$usagetypepiece.$purchasetypepiece.$cancelledpiece.$receiptstatuspiece.$seriespiecenew."  order by inv_dealer_invoicenumbers.slno";
				break;
			
			default:
				$query = $querycase." where left(inv_dealer_invoicenumbers.createddate,10) between '".$fromdate."' and '".$todate."' AND inv_dealer_invoicenumbers.businessname LIKE '%".$textfield."%' ".$finallistarray.$regionpiece.$district_typepiece.$state_typepiece.$dealer_typepiece.$branchpiece.$typepiece.$categorypiece.$generatedpiece.$statuspiece.$seriespiece.$usagetypepiece.$purchasetypepiece.$cancelledpiece.$receiptstatuspiece.$seriespiecenew."  order by inv_dealer_invoicenumbers.slno";
				break;
		} 
		//echo($query);exit;
		...
		while($fetch = mysqli_fetch_array($result1))
		{
			...
			$grid .= '<td nowrap="nowrap" class="td-border-grid" align="center"> <a onclick="viewdealerinvoice(\''.$fetch['slno'].'\');" class="resendtext" style = "cursor:pointer"> View >></a> </td>';
			$grid .= '<td nowrap="nowrap" class="td-border-grid" align="center"><div id= "resendemail'.$slnocount.'" style ="display:block;"> <a onclick="resenddealerinvoice(\''.$fetch['slno'].'\',\''.$slnocount.'\'); " class="resendtext" style = "cursor:pointer"> Resend </a></div><div id ="resendprocess'.$slnocount.'" style ="display:none;"></div></td>';
