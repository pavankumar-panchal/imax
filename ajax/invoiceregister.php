...
{ 
	...
	$query32 = "select * from inv_invoicenumbers where left(inv_invoicenumbers.createddate,10) = curdate() and offerdescription <> '' and `status` <> 'CANCELLED'";
	...
	while($fetch32 = mysqli_fetch_array($result32))
	{
		...
		for($k = 0 ;$k < count($addlesssplit);$k++)
		{
			...
			// Insert into services table 
			$insertoffer = "INSERT INTO addlessdesctoday(invoiceno,descname,descamount,createddate,dealerid,regionid,branch,branchname) values('".$fetch32['slno']."','". $descname."','". $descamount."','".$fetch32['createddate']."','".$fetch32['dealerid']."','".$fetch32['regionid']."','".$fetch32['branchid']."','".$fetch32['branch']."')";
			$result = runmysqlquery($insertoffer);
		}
	}
	
	// Insert data to invoicedetails table
	$query0 = "select * from inv_invoicenumbers where left(inv_invoicenumbers.createddate,10) = curdate() and products <> '' and `status` <> 'CANCELLED'";
...
switch($switchtype)
{
	...
	{	
		...
		where  left(inv_invoicenumbers.createddate,10) = '".date('Y-m-d')."' order by inv_invoicenumbers.slno";
		$fetch10 = runmysqlqueryfetch($resultcount);
		$fetchresultcount = $fetch10['count'];
		
		// To fetch invoice totals and other details
		$invoiceresult = "select distinct inv_invoicenumbers.slno,sum(inv_invoicenumbers.amount) as amount,
		sum(inv_invoicenumbers.servicetax+IFNULL(inv_invoicenumbers.sbtax,0)+IFNULL(inv_invoicenumbers.kktax,0)+IFNULL(inv_invoicenumbers.igst,0)+IFNULL(inv_invoicenumbers.sgst,0)+IFNULL(inv_invoicenumbers.cgst,0)) as servicetax,sum(inv_invoicenumbers.netamount) as netamount
		from inv_invoicenumbers left join dealer_online_purchase on dealer_online_purchase.onlineinvoiceno = inv_invoicenumbers.slno 
		where left(inv_invoicenumbers.createddate,10) = '".date('Y-m-d')."'  group by inv_invoicenumbers.slno with rollup  limit ".($fetchresultcount).",1 ";
		...
		$query = "select distinct inv_invoicenumbers.slno,left(inv_invoicenumbers.createddate,10) as createddate,
		inv_invoicenumbers.invoiceno,inv_invoicenumbers.customerid,inv_invoicenumbers.businessname,inv_invoicenumbers.amount,
		(inv_invoicenumbers.servicetax+IFNULL(inv_invoicenumbers.sbtax,0)+IFNULL(inv_invoicenumbers.kktax,0)+IFNULL(inv_invoicenumbers.igst,0)+IFNULL(inv_invoicenumbers.sgst,0)+IFNULL(inv_invoicenumbers.cgst,0)) as servicetax,
		inv_invoicenumbers.netamount,inv_invoicenumbers.dealername,inv_invoicenumbers.createdby,inv_invoicenumbers.status,inv_invoicenumbers.products as products,
		inv_invoicenumbers.servicedescription as servicedescription,inv_invoicenumbers.productquantity,inv_invoicenumbers.seztaxtype as seztaxtype,
		inv_invoicenumbers.seztaxfilepath as seztaxfilepath 
		from inv_invoicenumbers 
		left join dealer_online_purchase on dealer_online_purchase.onlineinvoiceno = inv_invoicenumbers.slno 
		where  left(inv_invoicenumbers.createddate,10) = '".date('Y-m-d')."' order by inv_invoicenumbers.createddate desc LIMIT ".$startlimit.",".$limit." ; "; //echo($query);exit;
		$result = runmysqlquery($query);
		if($startlimit == 0)
		{
		$grid = '<table width="100%" cellpadding="3" cellspacing="0" class="table-border-grid">';
		$grid .= '<tr class="tr-grid-header"><td nowrap = "nowrap" class="td-border-grid" align="left">Sl No</td><td nowrap = "nowrap" class="td-border-grid" align="left">Action</td><td nowrap = "nowrap" class="td-border-grid" align="left">Email</td><td nowrap = "nowrap" class="td-border-grid" align="left">SEZ Download</td><td nowrap = "nowrap" class="td-border-grid" align="left">Invoice Date</td><td nowrap = "nowrap" class="td-border-grid" align="left">Invoice No</td><td nowrap = "nowrap" class="td-border-grid" align="left">Products</td><td nowrap = "nowrap" class="td-border-grid" align="left">Quantity</td><td nowrap = "nowrap" class="td-border-grid" align="left">Status</td><td nowrap = "nowrap" class="td-border-grid" align="left">Customer ID</td><td nowrap = "nowrap" class="td-border-grid" align="left">Customer Name</td><td nowrap = "nowrap" class="td-border-grid" align="left">Sale Value</td><td nowrap = "nowrap" class="td-border-grid" align="left">Tax Amount</td><td nowrap = "nowrap" class="td-border-grid" align="left">Invoice Amount</td><td nowrap = "nowrap" class="td-border-grid" align="left">Sales Person</td><td nowrap = "nowrap" class="td-border-grid" align="left">Prepared By</td></tr>';
		}
		$i_n = 0;
...
left join inv_mas_receipt on inv_mas_receipt.invoiceno = inv_invoicenumbers.slno
left join inv_dealercard on inv_dealercard.invoiceid = inv_invoicenumbers.slno
where inv_invoicenumbers.contactperson LIKE '%".$textfield."%' ".$datepiece.$finallistarray.$regionpiece.$district_typepiece.$state_typepiece.$dealer_typepiece.$branchpiece.$typepiece.$categorypiece.$generatedpiece.$statuspiece.$seriespiece.$seriespiecenew.$usagetypepiece.$purchasetypepiece.$cancelledpiece.$receiptstatuspiece." order by inv_invoicenumbers.createddate,inv_invoicenumbers.slno desc";
				break;
			case "phone":
				$query = "select distinct inv_invoicenumbers.slno,left(inv_invoicenumbers.createddate,10) as createddate,inv_invoicenumbers.invoiceno,inv_invoicenumbers.customerid,inv_invoicenumbers.businessname,inv_invoicenumbers.amount,(inv_invoicenumbers.servicetax+IFNULL(inv_invoicenumbers.sbtax,0)+IFNULL(inv_invoicenumbers.kktax,0)+IFNULL(inv_invoicenumbers.igst,0)+IFNULL(inv_invoicenumbers.sgst,0)+IFNULL(inv_invoicenumbers.cgst,0)) as servicetax,inv_invoicenumbers.netamount,inv_invoicenumbers.dealername,inv_invoicenumbers.createdby,inv_invoicenumbers.status,inv_invoicenumbers.products as products,inv_invoicenumbers.description,inv_invoicenumbers.servicedescription,inv_invoicenumbers.branch,inv_invoicenumbers.branchid,inv_invoicenumbers.category,inv_invoicenumbers.dealerid,inv_invoicenumbers.regionid,inv_invoicenumbers.productquantity,inv_invoicenumbers.seztaxtype as seztaxtype,inv_invoicenumbers.seztaxfilepath as seztaxfilepath from inv_invoicenumbers
left join dealer_online_purchase on dealer_online_purchase.onlineinvoiceno = inv_invoicenumbers.slno
...
left join inv_mas_receipt on inv_mas_receipt.invoiceno = inv_invoicenumbers.slno
left join inv_dealercard on inv_dealercard.invoiceid = inv_invoicenumbers.slno
where inv_invoicenumbers.phone LIKE '%".$textfield."%' OR inv_invoicenumbers.cell LIKE '%".$textfield."%' ".$datepiece.$finallistarray.$regionpiece.$district_typepiece.$state_typepiece.$dealer_typepiece.$branchpiece.$typepiece.$categorypiece.$generatedpiece.$statuspiece.$seriespiece.$seriespiecenew.$usagetypepiece.$purchasetypepiece.$cancelledpiece.$receiptstatuspiece."  order by inv_invoicenumbers.createddate,inv_invoicenumbers.slno desc ";
				break;
			case "place":
				$query = "select distinct inv_invoicenumbers.slno,left(inv_invoicenumbers.createddate,10) as createddate,inv_invoicenumbers.invoiceno,inv_invoicenumbers.customerid,inv_invoicenumbers.businessname,inv_invoicenumbers.amount,(inv_invoicenumbers.servicetax+IFNULL(inv_invoicenumbers.sbtax,0)+IFNULL(inv_invoicenumbers.kktax,0)+IFNULL(inv_invoicenumbers.igst,0)+IFNULL(inv_invoicenumbers.sgst,0)+IFNULL(inv_invoicenumbers.cgst,0)) as servicetax,inv_invoicenumbers.netamount,inv_invoicenumbers.dealername,inv_invoicenumbers.createdby,inv_invoicenumbers.status,inv_invoicenumbers.products as products,inv_invoicenumbers.description,inv_invoicenumbers.servicedescription,inv_invoicenumbers.branch,inv_invoicenumbers.branchid,inv_invoicenumbers.category,inv_invoicenumbers.dealerid,inv_invoicenumbers.regionid,inv_invoicenumbers.productquantity,inv_invoicenumbers.seztaxtype as seztaxtype,inv_invoicenumbers.seztaxfilepath as seztaxfilepath from inv_invoicenumbers
left join dealer_online_purchase on dealer_online_purchase.onlineinvoiceno = inv_invoicenumbers.slno
...
left join inv_mas_receipt on inv_mas_receipt.invoiceno = inv_invoicenumbers.slno
left join inv_dealercard on inv_dealercard.invoiceid = inv_invoicenumbers.slno
where inv_invoicenumbers.place LIKE '%".$textfield."%' ".$datepiece.$finallistarray.$regionpiece.$district_typepiece.$state_typepiece.$dealer_typepiece.$branchpiece.$typepiece.$categorypiece.$generatedpiece.$statuspiece.$seriespiece.$seriespiecenew.$usagetypepiece.$purchasetypepiece.$cancelledpiece.$receiptstatuspiece."  order by inv_invoicenumbers.createddate,inv_invoicenumbers.slno desc";
				break;
			case "emailid":
				$query = "select distinct inv_invoicenumbers.slno,left(inv_invoicenumbers.createddate,10) as createddate,inv_invoicenumbers.invoiceno,inv_invoicenumbers.customerid,inv_invoicenumbers.businessname,inv_invoicenumbers.amount,(inv_invoicenumbers.servicetax+IFNULL(inv_invoicenumbers.sbtax,0)+IFNULL(inv_invoicenumbers.kktax,0)+IFNULL(inv_invoicenumbers.igst,0)+IFNULL(inv_invoicenumbers.sgst,0)+IFNULL(inv_invoicenumbers.cgst,0)) as servicetax,inv_invoicenumbers.netamount,inv_invoicenumbers.dealername,inv_invoicenumbers.createdby,inv_invoicenumbers.status,inv_invoicenumbers.products as products,inv_invoicenumbers.description,inv_invoicenumbers.servicedescription,inv_invoicenumbers.branch,inv_invoicenumbers.branchid,inv_invoicenumbers.category,inv_invoicenumbers.dealerid,inv_invoicenumbers.regionid,inv_invoicenumbers.productquantity,inv_invoicenumbers.seztaxtype as seztaxtype,inv_invoicenumbers.seztaxfilepath as seztaxfilepath from inv_invoicenumbers
left join dealer_online_purchase on dealer_online_purchase.onlineinvoiceno = inv_invoicenumbers.slno
...
left join inv_mas_receipt on inv_mas_receipt.invoiceno = inv_invoicenumbers.slno
left join inv_dealercard on inv_dealercard.invoiceid = inv_invoicenumbers.slno
where inv_invoicenumbers.emailid LIKE '%".$textfield."%'  ".$datepiece.$finallistarray.$regionpiece.$district_typepiece.$state_typepiece.$dealer_typepiece.$branchpiece.$typepiece.$categorypiece.$generatedpiece.$statuspiece.$seriespiece.$seriespiecenew.$usagetypepiece.$purchasetypepiece.$cancelledpiece.$receiptstatuspiece." order by inv_invoicenumbers.createddate,inv_invoicenumbers.slno desc";
				break;
				case "cardid":
					$query = "select distinct inv_invoicenumbers.slno,left(inv_invoicenumbers.createddate,10) as createddate,inv_invoicenumbers.invoiceno,inv_invoicenumbers.customerid,inv_invoicenumbers.businessname,inv_invoicenumbers.amount,(inv_invoicenumbers.servicetax+IFNULL(inv_invoicenumbers.sbtax,0)+IFNULL(inv_invoicenumbers.kktax,0)+IFNULL(inv_invoicenumbers.igst,0)+IFNULL(inv_invoicenumbers.sgst,0)+IFNULL(inv_invoicenumbers.cgst,0)) as servicetax,inv_invoicenumbers.netamount,inv_invoicenumbers.dealername,inv_invoicenumbers.createdby,inv_invoicenumbers.status,inv_invoicenumbers.products as products,inv_invoicenumbers.description,inv_invoicenumbers.servicedescription,inv_invoicenumbers.branch,inv_invoicenumbers.branchid,inv_invoicenumbers.category,inv_invoicenumbers.dealerid,inv_invoicenumbers.regionid,inv_invoicenumbers.productquantity,inv_invoicenumbers.seztaxtype as seztaxtype,inv_invoicenumbers.seztaxfilepath as seztaxfilepath from inv_invoicenumbers
left join dealer_online_purchase on dealer_online_purchase.onlineinvoiceno = inv_invoicenumbers.slno
...
where inv_mas_scratchcard.cardid LIKE '%".$textfield."%'  ".$datepiece.$finallistarray.$regionpiece.$district_typepiece.$state_typepiece.$dealer_typepiece.$branchpiece.$typepiece.$categorypiece.$generatedpiece.$statuspiece.$seriespiece.$seriespiecenew.$usagetypepiece.$purchasetypepiece.$cancelledpiece.$receiptstatuspiece."  order by inv_invoicenumbers.createddate,inv_invoicenumbers.slno desc";
					break;
			case "scratchnumber":
				$query = "select distinct inv_invoicenumbers.slno,left(inv_invoicenumbers.createddate,10) as createddate,inv_invoicenumbers.invoiceno,inv_invoicenumbers.customerid,inv_invoicenumbers.businessname,inv_invoicenumbers.amount,(inv_invoicenumbers.servicetax+IFNULL(inv_invoicenumbers.sbtax,0)+IFNULL(inv_invoicenumbers.kktax,0)+IFNULL(inv_invoicenumbers.igst,0)+IFNULL(inv_invoicenumbers.sgst,0)+IFNULL(inv_invoicenumbers.cgst,0)) as servicetax,inv_invoicenumbers.netamount,inv_invoicenumbers.dealername,inv_invoicenumbers.createdby,inv_invoicenumbers.status,inv_invoicenumbers.products as products,inv_invoicenumbers.description,inv_invoicenumbers.servicedescription,inv_invoicenumbers.branch,inv_invoicenumbers.branchid,inv_invoicenumbers.category,inv_invoicenumbers.dealerid,inv_invoicenumbers.regionid,inv_invoicenumbers.productquantity,inv_invoicenumbers.seztaxtype as seztaxtype,inv_invoicenumbers.seztaxfilepath as seztaxfilepath from inv_invoicenumbers
left join dealer_online_purchase on dealer_online_purchase.onlineinvoiceno = inv_invoicenumbers.slno
...
left join inv_mas_receipt on inv_mas_receipt.invoiceno = inv_invoicenumbers.slno
left join inv_dealercard on inv_dealercard.invoiceid = inv_invoicenumbers.slno
where inv_invoicenumbers.invoiceno LIKE '%".$textfield."%' ".$datepiece.$finallistarray.$regionpiece.$district_typepiece.$state_typepiece.$dealer_typepiece.$branchpiece.$typepiece.$categorypiece.$generatedpiece.$statuspiece.$seriespiece.$seriespiecenew.$usagetypepiece.$purchasetypepiece.$cancelledpiece.$receiptstatuspiece."  order by inv_invoicenumbers.createddate,inv_invoicenumbers.slno desc";
				break;
			case "invoiceamt":
				$query = "select distinct inv_invoicenumbers.slno,left(inv_invoicenumbers.createddate,10) as createddate,inv_invoicenumbers.invoiceno,inv_invoicenumbers.customerid,inv_invoicenumbers.businessname,inv_invoicenumbers.amount,(inv_invoicenumbers.servicetax+IFNULL(inv_invoicenumbers.sbtax,0)+IFNULL(inv_invoicenumbers.kktax,0)+IFNULL(inv_invoicenumbers.igst,0)+IFNULL(inv_invoicenumbers.sgst,0)+IFNULL(inv_invoicenumbers.cgst,0)) as servicetax,inv_invoicenumbers.netamount,inv_invoicenumbers.dealername,inv_invoicenumbers.createdby,inv_invoicenumbers.status,inv_invoicenumbers.products as products,inv_invoicenumbers.description,inv_invoicenumbers.servicedescription,inv_invoicenumbers.branch,inv_invoicenumbers.branchid,inv_invoicenumbers.category,inv_invoicenumbers.dealerid,inv_invoicenumbers.regionid,inv_invoicenumbers.productquantity,inv_invoicenumbers.seztaxtype as seztaxtype,inv_invoicenumbers.seztaxfilepath as seztaxfilepath from inv_invoicenumbers
left join dealer_online_purchase on dealer_online_purchase.onlineinvoiceno = inv_invoicenumbers.slno
...
left join inv_mas_receipt on inv_mas_receipt.invoiceno = inv_invoicenumbers.slno
left join inv_dealercard on inv_dealercard.invoiceid = inv_invoicenumbers.slno
where inv_invoicenumbers.netamount LIKE '%".$textfield."%' ".$datepiece.$finallistarray.$regionpiece.$district_typepiece.$state_typepiece.$dealer_typepiece.$branchpiece.$typepiece.$categorypiece.$generatedpiece.$statuspiece.$seriespiece.$seriespiecenew.$usagetypepiece.$purchasetypepiece.$cancelledpiece.$receiptstatuspiece."  order by inv_invoicenumbers.createddate,inv_invoicenumbers.slno desc";
				break;
			
			default:
				$query = "select distinct inv_invoicenumbers.slno,left(inv_invoicenumbers.createddate,10) as createddate,inv_invoicenumbers.invoiceno,inv_invoicenumbers.customerid,inv_invoicenumbers.businessname,inv_invoicenumbers.amount,(inv_invoicenumbers.servicetax+IFNULL(inv_invoicenumbers.sbtax,0)+IFNULL(inv_invoicenumbers.kktax,0)+IFNULL(inv_invoicenumbers.igst,0)+IFNULL(inv_invoicenumbers.sgst,0)+IFNULL(inv_invoicenumbers.cgst,0)) as servicetax,inv_invoicenumbers.netamount,inv_invoicenumbers.dealername,inv_invoicenumbers.createdby,inv_invoicenumbers.status,inv_invoicenumbers.products as products,inv_invoicenumbers.description,inv_invoicenumbers.servicedescription,inv_invoicenumbers.offerdescription,inv_invoicenumbers.branch,inv_invoicenumbers.branchid,inv_invoicenumbers.category,inv_invoicenumbers.dealerid,inv_invoicenumbers.regionid,inv_invoicenumbers.productquantity,inv_invoicenumbers.seztaxtype as seztaxtype,inv_invoicenumbers.seztaxfilepath as seztaxfilepath from inv_invoicenumbers
left join dealer_online_purchase on dealer_online_purchase.onlineinvoiceno = inv_invoicenumbers.slno
left join inv_mas_customer on inv_mas_customer.slno = right(inv_invoicenumbers.customerid,5)
left join inv_mas_district on inv_mas_district.districtcode = inv_mas_customer.district
left join inv_mas_receipt on inv_mas_receipt.invoiceno = inv_invoicenumbers.slno
left join inv_dealercard on inv_dealercard.invoiceid = inv_invoicenumbers.slno
where inv_invoicenumbers.businessname LIKE '%".$textfield."%' ".$datepiece.$finallistarray.$regionpiece.$district_typepiece.$state_typepiece.$dealer_typepiece.$branchpiece.$typepiece.$categorypiece.$generatedpiece.$statuspiece.$seriespiece.$seriespiecenew.$usagetypepiece.$purchasetypepiece.$cancelledpiece.$receiptstatuspiece."  ORDER BY createddate,inv_invoicenumbers.slno desc";
				break;
		} 
		//	echo($query); exit;	

		$totalresult = runmysqlquery($query);
		$eventquery = "Insert into inv_logs_event(userid,system,eventtype,eventdatetime,remarks) values('".$userid."','".$_SERVER['REMOTE_ADDR']."','76','".date('Y-m-d').' '.date('H:i:s')."','view_invoiceregister')";
	...
	case "getregionwiseinvoicelistBKG":
	{
		...
		if($type == 'BKG')
		{
			$addcategory = " inv_invoicenumbers.category = 'BKG'";
		}
		
		$resultcount = "select count(distinct inv_invoicenumbers.slno) as count from inv_invoicenumbers  
left join dealer_online_purchase on dealer_online_purchase.onlineinvoiceno = inv_invoicenumbers.slno
where  ".$addcategory."  and left(inv_invoicenumbers.createddate,10) = '".date('Y-m-d')."' order by inv_invoicenumbers.slno;";
		$fetch10 = runmysqlqueryfetch($resultcount);
		$fetchresultcount = $fetch10['count'];
...
from inv_invoicenumbers left join dealer_online_purchase on dealer_online_purchase.onlineinvoiceno = inv_invoicenumbers.slno where   ".$addcategory." and left(inv_invoicenumbers.createddate,10) = '".date('Y-m-d')."'  group by inv_invoicenumbers.slno with rollup  limit ".($fetchresultcount).",1 ";
		$fetchresult1 =runmysqlquery($invoiceresult);
		if(mysqli_num_rows($fetchresult1) > 0)
		{
			$fetchresult = runmysqlqueryfetch($invoiceresult);
		...
		$query = "select distinct inv_invoicenumbers.slno,left(inv_invoicenumbers.createddate,10) as createddate,inv_invoicenumbers.invoiceno,inv_invoicenumbers.customerid,inv_invoicenumbers.businessname,inv_invoicenumbers.amount,(inv_invoicenumbers.servicetax+IFNULL(inv_invoicenumbers.sbtax,0)+IFNULL(inv_invoicenumbers.kktax,0)+IFNULL(inv_invoicenumbers.igst,0)+IFNULL(inv_invoicenumbers.sgst,0)+IFNULL(inv_invoicenumbers.cgst,0)) as servicetax,inv_invoicenumbers.netamount,inv_invoicenumbers.dealername,inv_invoicenumbers.createdby,inv_invoicenumbers.status,inv_invoicenumbers.products as products,inv_invoicenumbers.servicedescription as servicedescription,inv_invoicenumbers.productquantity,inv_invoicenumbers.seztaxtype as seztaxtype,inv_invoicenumbers.seztaxfilepath as seztaxfilepath from inv_invoicenumbers
left join dealer_online_purchase on dealer_online_purchase.onlineinvoiceno = inv_invoicenumbers.slno
where  ".$addcategory."  and left(inv_invoicenumbers.createddate,10) = '".date('Y-m-d')."' order by inv_invoicenumbers.createddate desc LIMIT ".$startlimit.",".$limit." ; "; 
		$result = runmysqlquery($query);
	...
	case "getregionwiseinvoicelistBKM":
	{
		...
		if($type == 'BKM')
		{
			$addcategory = "inv_invoicenumbers.category = 'BKM'";
		}
...
where  ".$addcategory."  and left(inv_invoicenumbers.createddate,10) = '".date('Y-m-d')."' order by inv_invoicenumbers.createddate desc LIMIT ".$startlimit.",".$limit." ; "; 
	...
	$productwisegrid .= '</table>';
		...
		case "getregionwiseinvoicelistCSD":
		{
			...
			if($type == 'CSD')
			{
				$addcategory = " inv_invoicenumbers.category = 'CSD'";
			}
	...
	from inv_invoicenumbers left join dealer_online_purchase on dealer_online_purchase.onlineinvoiceno = inv_invoicenumbers.slno where  ".$addcategory." and left(inv_invoicenumbers.createddate,10) = '".date('Y-m-d')."'  group by inv_invoicenumbers.slno with rollup  limit ".($fetchresultcount).",1 ";
			...
			$query = "select distinct inv_invoicenumbers.slno,left(inv_invoicenumbers.createddate,10) as createddate,inv_invoicenumbers.invoiceno,inv_invoicenumbers.customerid,inv_invoicenumbers.businessname,inv_invoicenumbers.amount, (inv_invoicenumbers.servicetax+IFNULL(inv_invoicenumbers.sbtax,0)+IFNULL(inv_invoicenumbers.kktax,0)+IFNULL(inv_invoicenumbers.igst,0)+IFNULL(inv_invoicenumbers.sgst,0)+IFNULL(inv_invoicenumbers.cgst,0)) as servicetax,inv_invoicenumbers.netamount,inv_invoicenumbers.dealername,inv_invoicenumbers.createdby,inv_invoicenumbers.status,inv_invoicenumbers.products as products,inv_invoicenumbers.servicedescription as servicedescription,inv_invoicenumbers.productquantity,inv_invoicenumbers.seztaxtype as seztaxtype,inv_invoicenumbers.seztaxfilepath as seztaxfilepath from inv_invoicenumbers left join dealer_online_purchase on dealer_online_purchase.onlineinvoiceno = inv_invoicenumbers.slno
	where ".$addcategory."  and left(inv_invoicenumbers.createddate,10) = '".date('Y-m-d')."' order by inv_invoicenumbers.createddate desc LIMIT ".$startlimit.",".$limit." ; "; 
			$result = runmysqlquery($query);
	...
	$productwisegrid .= '</table>';
		...
		case "getregionwiseinvoicelistOnline":
		{
			...
			if($type == 'Online')
			{
				$addcategory = "inv_invoicenumbers.category = 'Online'";
			}
			$resultcount = "select count(distinct inv_invoicenumbers.slno) as count from inv_invoicenumbers  
	left join dealer_online_purchase on dealer_online_purchase.onlineinvoiceno = inv_invoicenumbers.slno
	where  ".$addcategory."  and left(inv_invoicenumbers.createddate,10) = '".date('Y-m-d')."' order by inv_invoicenumbers.slno;";
