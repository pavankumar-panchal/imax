...
switch($type)
{
	case 'productsave':
	{
		...
		$scheme = $_POST['scheme'];
		$pinno = $_POST['pinno'];
		$pinnosplit = explode(',',$pinno);
		$pinnocount = count($pinnosplit);
		$firstbillnumber = '';
		
		for($i = 0; $i < $pinnocount; $i++)
		{
			$cardid = $pinnosplit[$i];
			...
			if(mysqli_num_rows($result378) == 0)
			{
				...
				{
					$responsearray['errormessage'] = '2^Invalid Card^'.$cardid;
					echo(json_encode($responsearray)); exit;
				}
			}
		}
		
		if($pinnocount <> $productquantity)
		{
			$responsearray['errormessage'] = "2^ Product Quantity and PIN No entered doesnt match";
		}
		else
		{
			if($productlastslno == '')
			{
				...
				{
					$query = "INSERT INTO inv_bill(dealerid,total,billstatus,remarks,billdate,userid) VALUES('".$dealerid."','0','justcreated','".$remarks."','".changedateformat($billdate)."','".$userid."');";
			...
			$responsearray['errormessage'] = "1^ Record Saved Successfully. ^".$firstbillnumber.'^'.$pinno;
		}
...
WHERE inv_billdetail.cusbillnumber = '".$cusbillnumber."'";
		...
		$grid .= '<tr class="tr-grid-header"><td nowrap = "nowrap" class="td-border-grid">Sl No</td><td nowrap = "nowrap" class="td-border-grid">Bill Number</td><td nowrap = "nowrap" class="td-border-grid">Product Name</td><td nowrap = "nowrap" class="td-border-grid">Product Quantity</td><td nowrap = "nowrap" class="td-border-grid">Product Amount</td><td nowrap = "nowrap" class="td-border-grid">Usage Type</td><td nowrap = "nowrap" class="td-border-grid">Purchase Type</td><td nowrap = "nowrap" class="td-border-grid">scheme</td></tr>';
		$i_n = 0;
		$slno = 0;
		$result = runmysqlquery($query);
...
inv_billdetail.productquantity, inv_billdetail.productamount, inv_billdetail.usagetype, inv_billdetail.purchasetype, inv_mas_scheme.schemename
...
LEFT JOIN inv_bill on inv_bill.slno = inv_billdetail.cusbillnumber WHERE inv_bill.slno = '".$billlastslno."';";
		...
		$grid = '<table width="100%" cellpadding="3" cellspacing="0" class="table-border-grid">';
		$grid .= '<tr class="tr-grid-header" align ="left"><td nowrap = "nowrap" class="td-border-grid" align ="left">Sl No</td><td nowrap = "nowrap" class="td-border-grid" align ="left"> Bill Number</td><td nowrap = "nowrap" class="td-border-grid" align ="left">Product Name</td><td nowrap = "nowrap" class="td-border-grid" align ="left">Billed Quantity</td><td nowrap = "nowrap" class="td-border-grid" align ="left">Billed Amount</td><td nowrap = "nowrap" class="td-border-grid" align ="left">Usage Type</td><td nowrap = "nowrap" class="td-border-grid" align ="left">Purchase Type</td><td nowrap = "nowrap" class="td-border-grid" align ="left">Scheme</td></tr>';
		$i_n = 0;$slno = 0;
	...
	case 'getamount':
	{
		...
		{ $price = 'newsuprice'; $revenueshare = 'revenuesharenewsale'; }
		if($purchasetype == 'new' && $usagetype == 'multiuser') { $price = 'newmuprice'; $revenueshare = 'revenuesharenewsale';}
		if($purchasetype == 'new' && $usagetype == 'additionallicense'){ $price = 'newaddlicenseprice'; $revenueshare = 'revenuesharenewsale';}
		if($purchasetype == 'updation' && $usagetype == 'singleuser') { $price = 'updatesuprice'; $revenueshare = 'revenueshareupsale';}
		if($purchasetype == 'updation' && $usagetype == 'multiuser') { $price = 'updatemuprice'; $revenueshare = 'revenueshareupsale'; }
		if($purchasetype == 'updation' && $usagetype == 'additionallicense') { $price = 'updationaddlicenseprice'; $revenueshare = 'revenueshareupsale'; }
		if($price <> '')
		{
		...
		if($count > 0)
		{
			...
			while($fetch = mysqli_fetch_array($result)) 
			{
				...
				for($i=0; $i< $scratchlimit; $i++)
				{
					...
					$query2 = "INSERT INTO inv_dealercard ( dealerid, cardid, productcode, date, remarks, cusbillnumber, usagetype, purchasetype, userid,scheme,initialusagetype,initialpurchasetype,initialproduct,initialdealerid) values('".$fetch['dealerid']."', '".$cardid."', '".$fetch['productcode']."', '".$fetch['billdate']."', '".$fetch['remarks']."', '".$fetch['cusbillnumber']."', '".$fetch['usagetype']."', '".$fetch['purchasetype']."', '".$userid."', '".$fetch['scheme']."','".$fetch['usagetype']."','".$fetch['purchasetype']."','".$fetch['productcode']."','".$fetch['dealerid']."');";
	...
	values('".$fetch['dealerid']."','".$cardid."','".$fetch['productcode']."','".$fetch['netamount']."','".$fetch['usagetype']."','".$fetch['purchasetype']."','".$fetch['scheme']."','".$fetch['billdate']."','".$userid."','".$_SERVER['REMOTE_ADDR']."','user_module')";
...
WHERE inv_dealercard.cusbillnumber='".$cusbillnumber."' order by inv_mas_scratchcard.cardid;";
...
function sendpurchasemail($cusbillnumber,$dealerid,$table,$type)
{
	...
	$filearray = array(
		array('../images/relyon-logo.jpg','inline','8888888888'),array('../images/relyon-rupee-small.jpg','inline','88888888898')
	);
