...
for($j = 0;$j < count($newarray);$j++)
{
	...
	if($gst_tax_date <= $invoicedate)
	{
		if($letterhead == 'on')
			...
			$msg = file_get_contents("../pdfbillgeneration/dealer-bill-format-gst-duplicate-withouthead.php");	
		else
		{
			if($fetchresult['irn'] == "")
				$msg = file_get_contents("../pdfbillgeneration/dealer-bill-format-gst-duplicate.php");
			else
				$msg = file_get_contents("../pdfbillgeneration/dealer-bill-format-gst-duplicate-irn.php");
		}
	}
