...
switch($switchtype)
{
	case 'save':
	{
		...
		if($lastslno == '')
		{
			...
			$query = "INSERT INTO inv_mas_receipt(invoiceno,receiptamount,paymentmode,receiptremarks,privatenote,createddate,createdby,createdip,lastmodifieddate,lastmodifiedby,lastmodifiedip,customerreference,chequedate,chequeno,drawnon,depositdate,receiptdate,receipttime,module) values('".$invoiceno."','".$receiptamount."','".$paymentmode."','".$receiptremarks."','".$privatenote."','".datetimelocal('Y-m-d').' '.datetimelocal('H:i:s')."','".$userid."','".$_SERVER['REMOTE_ADDR']."','".datetimelocal('Y-m-d').' '.datetimelocal('H:i:s')."','".$userid."','".$_SERVER['REMOTE_ADDR']."','".$customerreference."','".$chequedate."','".$chequeno."','".$drawnon."','".$depositdate."','".$receiptdate."','".datetimelocal('H:i:s')."','user_module');";
			$result = runmysqlquery($query);
			$eventquery = "Insert into inv_logs_event(userid,system,eventtype,eventdatetime,remarks) values('".$userid."','".$_SERVER['REMOTE_ADDR']."','71','".date('Y-m-d').' '.date('H:i:s')."','".$customerreference."')";
	...
	case 'delete':
	{
		...
		$result = runmysqlquery($query);
		$eventquery = "Insert into inv_logs_event(userid,system,eventtype,eventdatetime,remarks) values('".$userid."','".$_SERVER['REMOTE_ADDR']."','73','".date('Y-m-d').' '.date('H:i:s')."','".$customerreference."')";
		$eventresult = runmysqlquery($eventquery);
	...
	case 'gridtoform':
	{
		...
		$resultfetch = runmysqlqueryfetch($query);
		$query1 = "select sum(receiptamount) as receiptamount  from inv_mas_receipt where invoiceno = '".$resultfetch['invoiceno']."' and restatus <> 'CANCELLED'";
		$fetch1 = runmysqlqueryfetch($query1);
		$receiptamount = $fetch1['receiptamount'];


		if($resultfetch['module'] == 'user_module')
			$querypiece = "select inv_mas_receipt.slno,sum(inv_mas_receipt.receiptamount) as receiptamount,inv_mas_receipt.invoiceno,inv_mas_receipt.receiptremarks,
			inv_mas_receipt.privatenote,inv_mas_receipt.paymentmode,inv_mas_receipt.createddate,inv_mas_receipt.chequedate,inv_mas_receipt.chequeno,inv_mas_receipt.drawnon,inv_mas_receipt.depositdate,inv_mas_receipt.receiptdate,inv_mas_receipt.createddate,inv_mas_users.fullname as createdby,inv_invoicenumbers.netamount as invoiceamount,inv_mas_receipt.restatus as status,inv_mas_receipt.cancelledremarks,inv_mas_receipt.cancelledby,inv_mas_receipt.cancelleddate from inv_mas_receipt 
			...
			$querypiece = "select inv_mas_receipt.slno,sum(inv_mas_receipt.receiptamount)  as  receiptamount,inv_mas_receipt.invoiceno,inv_mas_receipt.receiptremarks,
			inv_mas_receipt.privatenote,inv_mas_receipt.paymentmode,inv_mas_receipt.createddate,inv_mas_receipt.chequedate,inv_mas_receipt.chequeno,inv_mas_receipt.drawnon,inv_mas_receipt.depositdate,inv_mas_receipt.receiptdate,inv_mas_receipt.createddate,inv_mas_dealer.businessname  as createdby,inv_invoicenumbers.netamount as invoiceamount from inv_mas_receipt left join inv_invoicenumbers on inv_invoicenumbers.slno = inv_mas_receipt.invoiceno
			left join inv_mas_dealer on inv_mas_dealer.slno = inv_invoicenumbers.dealerid  where inv_mas_receipt.slno = '".$lastslno."' group by inv_mas_receipt.slno;";
...
where inv_mas_customer.slno = '".$customerreference."' and inv_invoicenumbers.status <> 'CANCELLED';";
			...
			$query = "select distinct inv_mas_receipt.slno as slno,inv_mas_customer.customerid,inv_invoicenumbers.netamount,inv_mas_receipt.receiptamount ,inv_mas_receipt.createddate,inv_mas_dealer.businessname as createdby,inv_mas_receipt.receiptremarks,inv_mas_receipt.privatenote,inv_mas_receipt.lastmodifieddate,inv_mas_receipt.invoiceno,inv_invoicenumbers.status , inv_mas_receipt.restatus as receiptstatus, inv_mas_receipt.cancelledremarks as cancelledremarks from inv_mas_receipt
left join inv_mas_customer on inv_mas_customer.slno = inv_mas_receipt.customerreference 
left join inv_invoicenumbers on inv_invoicenumbers.slno = inv_mas_receipt.invoiceno
left join inv_mas_dealer on inv_mas_dealer.slno = inv_mas_receipt.createdby 
where inv_mas_customer.slno = '".$customerreference."' and inv_invoicenumbers.status <> 'CANCELLED'  ORDER BY inv_mas_receipt.createddate desc LIMIT ".$startlimit.",".$limit."; ";
			$result = runmysqlquery($query);
			if($startlimit == 0)
			{
	...
	case 'cancelreceipt':
	{
		...
		echo $query = "update inv_mas_receipt set restatus = 'CANCELLED',cancelleddate = '".date('Y-m-d').' '.date('H:i:s')."',cancelledip = '".$_SERVER['REMOTE_ADDR']."',cancelledby = '".$userid ."',cancelledremarks = '".$cancelremarks."',receiptamount=null  where slno  ='".$receiptno."';";
		$result = runmysqlquery($query);
...
function getinvoiceamount($invoiceno)
{
	...
	$query = "select sum(receiptamount) as receiptamount from inv_mas_receipt where invoiceno = '".$invoiceno."' and restatus <> 'CANCELLED'" ;
	$resultfetch = runmysqlqueryfetch($query);
