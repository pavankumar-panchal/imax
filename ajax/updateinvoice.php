...
switch($switchtype)
{
	case 'generateinvoicenolist':
	{
		...
		$query = "SELECT slno,invoiceno FROM inv_invoicenumbers where purchasetype = 'Product'  ORDER BY invoiceno";
	...
	/*case 'getuserinvoicelist':
	{
		...
		$query = "select slno, invoiceno from inv_invoicenumbers where right(customerid,5) = '".$customerreference."'";
		$result = runmysqlquery($query);
	...
	case 'listinvoicedetails':
	{
		...
		$pingrid = '<table width="100%" cellspacing="0" cellpadding="4" border="0" class="grey-table-border-grid" ><tr bgcolor="#cccccc"><td width="9%" nowrap="nowrap" class="grey-td-border-grid" style="text-align:left;">Sl No</td><td width="37%" nowrap="nowrap" class="grey-td-border-grid" style="text-align:left;">Product</td><td width="19%" nowrap="nowrap" class="grey-td-border-grid" style="text-align:left;">Purchase Type</td><td width="17%" nowrap="nowrap" class="grey-td-border-grid" style="text-align:left;">Usage Type</td><td width="20%" nowrap="nowrap" class="grey-td-border-grid" style="text-align:left;">Amount</td></tr>';
    
 		$count = 0;$servicecount = 0;$offercount = 0;
		while($fetch = mysqli_fetch_array($result))
		{
			...
			for($i=0;$i<count($descriptionsplit);$i++)
			{
				$descriptionline = explode('$',$descriptionsplit[$i]);
				{
					if($description <> '')
					{
						...
						$pingrid .= '<td style="text-align:centre;" class="grey-td-border-grid" width="9%" id="slno'.$descriptionline[0].'">'.$descriptionline[0].'</td>';
						$pingrid .= '<td nowrap="nowrap" class="grey-td-border-grid"  style="text-align:left; " width="27%" id="productdespt"><div id="productinputdiv'.$count.'" style="display:block;padding-bottom:3px"><input type="text" size="50" autocomplete="off" maxlength="300" id="productvalue'.$count.'" class="'.$classdefine.'" name="productvalue'.$count.'" value="'.$descriptionline[1].'" '.$disable.' ></div><div id="productselectiondiv'.$count.'" style="display:none;padding-bottom:3px">'.$productlistarray.'<br/></div><strong>PIN Number : <font color="#FF3300">'.$descriptionline[4].'</font></strong> (<strong>Serial</strong> : '.$descriptionline[5].')</span></span><input name="productselectedhidden[]" class="swiftselect" id="productselectedhidden " type = "hidden" value = "'.$descriptionline[4].'%%'.$descriptionline[5].'"> <input name="productslnoarrayhidden[]" class="swiftselect" id="productslnoarrayhidden " type = "hidden" value = "'.$count.'"><input name="itemtype'.$count.'" class="swiftselect" id="itemtype'.$count.'" type = "hidden" value = "product"></td>';
						$pingrid .= ' <td valign="top" class="grey-td-border-grid" width="15%"  align="center"><span align="center" id="editpurchasetype'.$count.'">'.$descriptionline[2].'</span><div align="center" id="displaypurchasediv'.$count.'" style="display:none"><a class="r-text" onclick ="editpurchasetype(\''.'editpurchasetype'.$count.'\',\''.'purchasetypehidden'.$count.'\');" style="cursor: pointer;" id="editpurchasetypelinkid'.$count.'">( Change )</a> </div> <input name="purchasetypehidden'.$count.'" class="swiftselect" id="purchasetypehidden'.$count.'" type = "hidden" value = "'.strtolower($descriptionline[2]).'" '.$disable.'></td>';
						$pingrid .= '<td valign="top" class="grey-td-border-grid" width="16%" align="center"><span align="center" id="editusagetypehtml'.$count.'">'.$descriptionline[3].'</span> <div align="center" id="displayusagediv'.$count.'" style="display:none"><a class="r-text" onclick ="editusagetype(\''.'editusagetypehtml'.$count.'\',\''.'usagetypehidden'.$count.'\');" style="cursor: pointer;" id="editusagetypelinkid'.$count.'">( Change )</a></div><input name="usagetypehidden'.$count.'" class="swiftselect" id="usagetypehidden'.$count.'" type = "hidden" value = "'.str_replace(' ', '',strtolower($descriptionline[3])).'" '.$disable.' ></td>';
						$pingrid .= '<td nowrap="nowrap" class="grey-td-border-grid" style="text-align: right;" width="23%"><input type="text"  style="width: 80px; text-align: right;" autocomplete="off" maxlength="10" id="productamount'.$count.'" class="'.$classdefine.'" name="productamount'.$count.'" value="'.$descriptionline[6].$appendzero.'" '.$disable.'  onkeyup ="calculatenormalprice();getpercentvalue();" onchange ="calculatenormalprice();getpercentvalue();"></td>';
						$pingrid .= "</tr>";
			...
			if($fetch['servicedescription'] <> '')
			{
				for($i=0; $i<$servicedescriptioncount; $i++)
				{
					...
					$pingrid .= '<tr id="seletedproductgrid" height="30px">';
					$pingrid .= '<td  style="text-align:centre;" class="grey-td-border-grid" width="9%" id="serviceslno'.$servicedescriptionline[0].'">'.$servicedescriptionline[0].'</td>';
					$pingrid .= '<td  style="text-align:left;" class="grey-td-border-grid" width="37%">'.$servicedescriptionline[1].' <input name="itemtype'.$count.'" class="swiftselect" id="itemtype'.$count.'" type = "hidden" value = "service"><input name="serviceselectedhidden[]" class="swiftselect" id="serviceselectedhidden" type = "hidden" value = "'.$servicedescriptionline[1].'"></td>';
					$pingrid .= '<td style="text-align:left;" class="grey-td-border-grid" width="19%">&nbsp;</td>';
					$pingrid .= '<td style="text-align:left;" class="grey-td-border-grid" width="17%">&nbsp;</td>';
					$pingrid .= '<td  style="text-align:right;" class="grey-td-border-grid" width="20%"><input type="text"  style="width: 80px; text-align: right;" autocomplete="off" maxlength="10" id="productamount'.$count.'" class="'.$classdefine.'" name="productamount'.$count.'" value="'.$servicedescriptionline[2].$appendzero.'" '.$disable.'  onkeyup ="calculatenormalprice();getpercentvalue();" onchange ="calculatenormalprice();getpercentvalue();"></td>';
					$pingrid .= "</tr>";
			...
			if($fetch['offerdescription'] <> '')
			{
				for($i=0; $i<$offerdescriptioncount; $i++)
				{
					...
					$pingrid .= '<tr id="adddescriptionrows" height="30px">';
					$pingrid .= '<td style="text-align:centre;" class="grey-td-border-grid" width="9%">&nbsp;</td>';
					if($offerdescriptionline[0] == "add")
					{
					    $total_productamount = $total_productamount + $offerdescriptionline[2];
					}
					if($offerdescriptionline[0] == "percentage" || $offerdescriptionline[0] == "PERCENTAGE")
					{
					    $pingrid .= '<td style="text-align:left;" class="grey-td-border-grid" width="37%" ><span id="descriptiontype'.$offercount.'">'.strtoupper($offerdescriptionline[0]).'</span>:  <input type="text"  style="width: 250px; text-align: left;" autocomplete="off" maxlength="50" id="description'.$offercount.'" class="'.$classdefine.'" name="description'.$offercount.'" value="'.$offerdescriptionline[1].'" '.$disable.'><br><span>Change Discount % : <input type="text"  style="width: 50px; text-align: left;margin-top:1%" autocomplete="off" maxlength="2" onkeyup="getpercentvalue()" id="discountpercent" class="'.$classdefine.'" name="discountpercent" value=""'.$disable.'></span></td>';
					    $pingrid .= '<td style="text-align:left;" class="grey-td-border-grid" width="19%">&nbsp;</td>';
					    $pingrid .= '<td style="text-align:left;" class="grey-td-border-grid" width="17%">&nbsp;</td>';
					    $pingrid .= '<td  style="text-align:right;" class="grey-td-border-grid" width="20%"><input type="text"  style="width: 80px; text-align: right;" autocomplete="off" maxlength="10" id="descriptionamount'.$offercount.'" class="'.$classdefine.'" name="descriptionamount'.$offercount.'" value="'.$offerdescriptionline[2].'" '.$disable.'  onkeyup ="calculatenormalprice();" onchange ="calculatenormalprice();" readonly ></td>';
					    
					}
					else
					{
					    $pingrid .= '<td style="text-align:left;" class="grey-td-border-grid" width="37%" ><span id="descriptiontype'.$offercount.'">'.strtoupper($offerdescriptionline[0]).'</span>:  <input type="text"  style="width: 250px; text-align: left;" autocomplete="off" maxlength="50" id="description'.$offercount.'" class="'.$classdefine.'" name="description'.$offercount.'" value="'.$offerdescriptionline[1].'" '.$disable.'></td>';
					    $pingrid .= '<td style="text-align:left;" class="grey-td-border-grid" width="19%">&nbsp;</td>';
					    $pingrid .= '<td style="text-align:left;" class="grey-td-border-grid" width="17%">&nbsp;</td>';
				    	$pingrid .= '<td  style="text-align:right;" class="grey-td-border-grid" width="20%"><input type="text"  style="width: 80px; text-align: right;" autocomplete="off" maxlength="10" id="descriptionamount'.$offercount.'" class="'.$classdefine.'" name="descriptionamount'.$offercount.'" value="'.$offerdescriptionline[2].'" '.$disable.'  onkeyup ="calculatenormalprice();" onchange ="calculatenormalprice();"></td>';
			...
			if($fetch['offerremarks'] <> '')
				$pingrid .= '<tr height="30px" ><td class="grey-td-border-grid">&nbsp;</td><td style="text-align:left;" class="grey-td-border-grid">'.$fetchresult['offerremarks'].'</td><td class="grey-td-border-grid">&nbsp;</td><td class="grey-td-border-grid">&nbsp;</td><td class="grey-td-border-grid">&nbsp;</td></tr>';
		...
		$pingrid .= '<tr height="30px"><td style="text-align: right;" colspan="4"  class="grey-td-border-grid"><strong>Total</strong></td><td width="10%" style="text-align: right;"  class="grey-td-border-grid"><input type="text" style="width: 80px; text-align: right;" autocomplete="off" maxlength="20" name="totalamount" id="totalamount" class="'.$classdefine.'" value = "'.$fetchresult['amount'].$appendzero.'" '.$disable.'></td></tr>';
        ...
        $kk_expirydate = strtotime('2016-06-01');
      ...
      if($currentdate < $gst_tax_date)
      {  
    	//	echo "***". $currentdate . "-" . $kk_expirydate;
    		if($expirydate > $currentdate)
    		{
    			$sevicetaxname = "Service Tax @ 12.36%";
    			$pingrid .= '<tr height="30px"><td style="text-align: right;" colspan="4"  class="grey-td-border-grid"><strong>'. $sevicetaxname.'</strong></td><td width="10%" style="text-align: right;"  class="grey-td-border-grid"><input type="text" style="width: 80px; text-align: right;" autocomplete="off" maxlength="20" id="sevicetax" class="'.$classdefine.'" name="sevicetax" value = "'.$fetchresult['servicetax'].$appendzero.'" '.$disable.'><input type="hidden" style="width: 80px; text-align: right;" autocomplete="off" maxlength="20" id="sbtax" class="'.$classdefine.'" name="sbtax" value = "0" '.$disable.'></td></tr>';
    		
        		/*$pingrid .= '<tr height="30px"><td style="text-align: right;" colspan="4"  class="grey-td-border-grid"><strong>'. $sbtaxname.'</strong></td><td width="10%" style="text-align: right;"  class="grey-td-border-grid"><input type="text" style="width: 80px; text-align: right;" autocomplete="off" maxlength="20" id="sevicetax" class="'.$classdefine.'" name="sevicetax" value = "'.$fetchresult['sbtax'].$appendzero.'" '.$disable.'></td></tr>';*/
        
                if($kk_expirydate <= $currentdate)
                 {
                 	//$new_amount = ceil($fetchresult['amount']*0.005) + $fetchresult['netamount'];
                  	$new_amount = $fetchresult['netamount'];
        	     	$pingrid .= '<tr height="30px"><td style="text-align: right;" colspan="4"  class="grey-td-border-grid"><strong>KK Cess @ 0.5%</strong></td><td width="10%" style="text-align: right;"  class="grey-td-border-grid"><input type="text" style="width: 80px; text-align: right;" autocomplete="off" maxlength="20" id="kk_cess" class="'.$classdefine.'" name="kk_cess" value = "'.ceil($fetchresult['amount']*0.005).$appendzero.'" '.$disable.'></td></tr>';
                 }
             ...
             //adding gst column
                 ...
                 $pingrid .= '<tr height="30px"><td style="text-align: right;" colspan="4"  class="grey-td-border-grid"><strong>Net Amount</strong></td><td width="10%" style="text-align: right;"  class="grey-td-border-grid"><input type="text" style="width: 80px; text-align: right;" autocomplete="off" maxlength="10" id="netamount" class="'.$classdefine.'" name="netamount"  value = "'.$new_amount.$appendzero.'" '.$disable.'></td></tr>';
        		 
                 $pingrid .= '<tr><td style="text-align: left;" colspan="5"  class="grey-td-border-grid" id="rupeeinwords"><strong>Rupee In Words</strong>: <span id="amountinwords">'.$fetchresult['amountinwords'].'</span> only</td></tr></table>';
        			
    		}
    		else if ($sb_expirydate > $currentdate)
    		{
    		    $sevicetaxname = "Service Tax @ 14%";
    			$pingrid .= '<tr height="30px"><td style="text-align: right;" colspan="4"  class="grey-td-border-grid"><strong>'. $sevicetaxname.'</strong></td><td width="10%" style="text-align: right;"  class="grey-td-border-grid"><input type="text" style="width: 80px; text-align: right;" autocomplete="off" maxlength="20" id="sevicetax" class="'.$classdefine.'" name="sevicetax" value = "'.$fetchresult['servicetax'].$appendzero.'" '.$disable.'><input type="hidden" style="width: 80px; text-align: right;" autocomplete="off" maxlength="20" id="sbtax" class="'.$classdefine.'" name="sbtax" value = "0" '.$disable.'></td></tr>';
    		...
    		{
    				$sbtaxname = "SB Cess @ 0.5%";
    				$sevicetaxname = "Service Tax @ 14%"; 
    				$pingrid .= '<tr height="30px"><td style="text-align: right;" colspan="4"  class="grey-td-border-grid"><strong>'. $sevicetaxname.'</strong></td><td width="10%" style="text-align: right;"  class="grey-td-border-grid"><input type="text" style="width: 80px; text-align: right;" autocomplete="off" maxlength="20" id="sevicetax" class="'.$classdefine.'" name="sevicetax" value = "'.$fetchresult['servicetax'].$appendzero.'" '.$disable.'></td></tr>';
    		
    		        $pingrid .= '<tr height="30px"><td style="text-align: right;" colspan="4"  class="grey-td-border-grid"><strong>'. $sbtaxname.'</strong></td><td width="10%" style="text-align: right;"  class="grey-td-border-grid"><input type="text" style="width: 80px; text-align: right;" autocomplete="off" maxlength="20" id="sbtax" class="'.$classdefine.'" name="sbtax" value = "'.$fetchresult['sbtax'].$appendzero.'" '.$disable.'></td></tr>';
    
                    if($kk_expirydate <= $currentdate)
                    {
                    	$kktax=$fetchresult['kktax'];	
                    	//$new_amount = $kktax+ $fetchresult['netamount'];
                    	$new_amount = $fetchresult['netamount'];
                    $pingrid .= '<tr height="30px"><td style="text-align: right;" colspan="4"  class="grey-td-border-grid"><strong>KK Cess @ 0.5%</strong></td><td width="10%" style="text-align: right;"  class="grey-td-border-grid"><input type="text" style="width: 80px; text-align: right;" autocomplete="off" maxlength="20" id="kktax" class="'.$classdefine.'" name="kktax" value = "'.$kktax.$appendzero.'" '.$disable.'></td></tr>';
                    }
                    else
                    {
                     $new_amount = $fetchresult['netamount'];
    ...
    else
    {
        $gst_customer = $fetch_customer_details['gst_no'];
         ...
         #aaded for multiple GSTIN 
		    ...
		    $customer_gst_slno = substr($search_customer, -5);
		    
		    $select_gstin_count = "select count(*) as countgstin from customer_gstin_logs 
		    left join inv_invoicenumbers on customer_gstin_logs.gstin_id = inv_invoicenumbers.gst_no
		    where customer_slno = '".$customer_gst_slno."' and inv_invoicenumbers.slno = '".$slno."'";
	        $fetch_gstin_count = runmysqlqueryfetch($select_gstin_count);
	     
	     if($fetch_gstin_count['countgstin'] != 0) 
	     {
		    $new_gst_details = "select customer_gstin_logs.gst_no from customer_gstin_logs 
		    left join inv_invoicenumbers on customer_gstin_logs.gstin_id = inv_invoicenumbers.gst_no
		    where customer_slno = '".$customer_gst_slno."' and inv_invoicenumbers.slno = '".$slno."'";
		    $fetch_new_gst_details = runmysqlqueryfetch($new_gst_details);
		    
		    $gst_customer = $fetch_new_gst_details['gst_no'];
		    $customer_gst_code = substr($fetch_new_gst_details['gst_no'], 0, 2);

	     }

	     else 
	     {
	     	//$first_cust_gst_no = "select gst_no from inv_mas_customer where slno = RIGHT($search_customer,5)";
	     	$first_cust_gst_no = "select inv_mas_customer.sez_enabled as sez_enabled,
        	inv_mas_district.statecode as state_code,inv_mas_state.statename as statename,inv_mas_state.statecode as statecode
        	,inv_mas_state.state_gst_code as state_gst_code, inv_mas_customer.gst_no from inv_mas_customer 
        	left join inv_mas_district on inv_mas_customer.district = inv_mas_district.districtcode
        	left join inv_mas_state on inv_mas_state.statecode = inv_mas_district.statecode 
        	where inv_mas_customer.slno = RIGHT($search_customer,5)";
	     	//$first_cust_result = runmysqlquery($first_cust_gst_no);
	        //$gst_no_count = mysqli_num_rows($first_cust_result);
	        //if($gst_no_count > 0)
	       // {
        	$fetch_first_gst_details = runmysqlqueryfetch($first_cust_gst_no);
        	if($fetchresult['gst_no']!= '0') {
				if (is_numeric($fetch_first_gst_details['gst_no']) && $fetch_first_gst_details['gst_no'] != "") {
					$gst_customer = $fetch_first_gst_details['gst_no'];
					$get_gst_first_no = "select gst_no from customer_gstin_logs where gstin_id = '" . $fetch_first_gst_details['gst_no'] . "' ";
					$fetch_first_gst_no = runmysqlqueryfetch($get_gst_first_no);
					$gst_customer = $fetch_first_gst_no['gst_no'];
					$customer_gst_code = substr($fetch_first_gst_no['gst_no'], 0, 2);
				} else if ($fetch_first_gst_details['gst_no'] != "") {
					$gst_customer = $fetch_first_gst_details['gst_no'];
					$customer_gst_code = substr($fetch_first_gst_details['gst_no'], 0, 2);
				} else {
					$gst_customer = 'Not Registered Under GST';
    ...
    if(($fetchresult['cgst'] != '0') &&  ($fetchresult['sgst'] != '0') || ($customer_gst_code == 29 && $fetchresult['cgst'] == '0' &&  $fetchresult['sgst'] == '0' && ($fetch['seztaxtype'] == 'yes' || $fetch_first_gst_details['sez_enabled'] == 'yes')))
     {
             $net_amount = $fetchresult['netamount'];
                  ...
                  $pingrid .= '<tr height="30px"><td style="text-align: right;" colspan="4"  class="grey-td-border-grid"><strong>CGST @ '.$cgst_tax_rate.'%</strong></td><td width="10%" style="text-align: right;"  class="grey-td-border-grid"><input type="text" style="width: 80px; text-align: right;" autocomplete="off" maxlength="20" id="cgst_tax" class="'.$classdefine.'" name="cgst_tax" value = "'.$fetchresult['cgst'].'" '.$disable.'></td></tr>';
                  $pingrid .= '<tr height="30px"><td style="text-align: right;" colspan="4"  class="grey-td-border-grid"><strong>SGST @ '.$sgst_tax_rate.'%</strong></td><td width="10%" style="text-align: right;"  class="grey-td-border-grid"><input type="text" style="width: 80px; text-align: right;" autocomplete="off" maxlength="20" id="sgst_tax" class="'.$classdefine.'" name="sgst_tax" value = "'.$fetchresult['sgst'].'" '.$disable.'></td></tr>';
         ...
         else
         {
                     ...
                     $pingrid .= '<tr height="30px"><td style="text-align: right;" colspan="4"  class="grey-td-border-grid"><strong>IGST @ '.$igst_tax_rate.'%</strong></td><td width="10%" style="text-align: right;"  class="grey-td-border-grid"><input type="text" style="width: 80px; text-align: right;" autocomplete="off" maxlength="20" id="igst_tax" class="'.$classdefine.'" name="igst_tax" value = "'.$fetchresult['igst'].'" '.$disable.'></td></tr>';
                     $pingrid .= '<tr height="30px"><td style="text-align: right;" colspan="4"  class="grey-td-border-grid"><strong>Net Amount</strong></td><td width="10%" style="text-align: right;"  class="grey-td-border-grid"><input type="text" style="width: 80px; text-align: right;" autocomplete="off" maxlength="10" id="netamount" class="'.$classdefine.'" name="netamount"  value = "'.$net_amount.$appendzero.'" '.$disable.'></td></tr>';
                     $pingrid .= '<tr><td style="text-align: left;" colspan="5"  class="grey-td-border-grid" id="rupeeinwords"><strong>Rupee In Words</strong>: <span id="amountinwords">'.$fetchresult['amountinwords'].'</span> only</td></tr></table>';    

         }
     /*if($fetchresult['seztaxtype'] == 'yes' || $fetch_customer_details['sez_enabled'] == 'yes')
     {
       ...
       if($fetch['cgst'] == 0 && $fetch['sgst'] == 0)
       {
             ...
             $pingrid .= '<tr height="30px"><td style="text-align: right;" colspan="4"  class="grey-td-border-grid"><strong>sezIGST @ '.$igst_tax_rate.'%</strong></td><td width="10%" style="text-align: right;"  class="grey-td-border-grid"><input type="text" style="width: 80px; text-align: right;" autocomplete="off" maxlength="20" id="igst_tax" class="'.$classdefine.'" name="igst_tax" value = "'.$fetchresult['igst'].$appendzero.'" '.$disable.'></td></tr>';
             $pingrid .= '<tr height="30px"><td style="text-align: right;" colspan="4"  class="grey-td-border-grid"><strong>Net Amount</strong></td><td width="10%" style="text-align: right;"  class="grey-td-border-grid"><input type="text" style="width: 80px; text-align: right;" autocomplete="off" maxlength="10" id="netamount" class="'.$classdefine.'" name="netamount"  value = "'.$net_amount.$appendzero.'" '.$disable.'></td></tr>';
             $pingrid .= '<tr><td style="text-align: left;" colspan="5"  class="grey-td-border-grid" id="rupeeinwords"><strong>Rupee In Words</strong>: <span id="amountinwords">'.$fetchresult['amountinwords'].'</span> only</td></tr></table>';    
       }
       else
       {
             $net_amount = $fetchresult['netamount'];
                  ...
                  $pingrid .= '<tr height="30px"><td style="text-align: right;" colspan="4"  class="grey-td-border-grid"><strong>sezCGST @ '.$cgst_tax_rate.'%</strong></td><td width="10%" style="text-align: right;"  class="grey-td-border-grid"><input type="text" style="width: 80px; text-align: right;" autocomplete="off" maxlength="20" id="cgst_tax" class="'.$classdefine.'" name="cgst_tax" value = "'.$fetchresult['cgst'].$appendzero.'" '.$disable.'></td></tr>';
                  $pingrid .= '<tr height="30px"><td style="text-align: right;" colspan="4"  class="grey-td-border-grid"><strong>sezSGST @ '.$sgst_tax_rate.'%</strong></td><td width="10%" style="text-align: right;"  class="grey-td-border-grid"><input type="text" style="width: 80px; text-align: right;" autocomplete="off" maxlength="20" id="sgst_tax" class="'.$classdefine.'" name="sgst_tax" value = "'.$fetchresult['sgst'].$appendzero.'" '.$disable.'></td></tr>';
                  $pingrid .= '<tr height="30px"><td style="text-align: right;" colspan="4"  class="grey-td-border-grid"><strong>Net Amount</strong></td><td width="10%" style="text-align: right;"  class="grey-td-border-grid"><input type="text" style="width: 80px; text-align: right;" autocomplete="off" maxlength="10" id="netamount" class="'.$classdefine.'" name="netamount"  value = "'.$net_amount.$appendzero.'" '.$disable.'></td></tr>';
                  $pingrid .= '<tr><td style="text-align: left;" colspan="5"  class="grey-td-border-grid" id="rupeeinwords"><strong>Rupee In Words</strong>: <span id="amountinwords">'.$fetchresult['amountinwords'].'</span> only</td></tr></table>';
       }
     }
...
//end gstif		
	...
	case 'editinvoice';
	{
	    //var_dump($_POST);
	    //exit();
		$servicetaxamount = str_replace(".00","",$_POST['servicetaxamount']);
		$sbtaxamount = str_replace(".00","",$_POST['sbtaxamount']);
		$kk_taxamount = str_replace(".00","",$_POST['kktaxamount']);
		
		//$igst_taxamount = str_replace(".00","",$_POST['igsttaxamount']);
		//$cgst_taxamount = str_replace(".00","",$_POST['cgsttaxamount']);
		//$sgst_taxamount = str_replace(".00","",$_POST['sgsttaxamount']);
		...
		//Sevice values
		$serviceslnovalues = trim(removedoublecomma($_POST['serviceslnovalues']),',');
		$servicelist = $_POST['servicelist'];
		$serviceamountvalues = str_replace(".00","",$_POST['serviceamountvalues']);
		$servicelistsplit = explode('#',$servicelist);
		$servicesplitcount = count($servicelistsplit);
		$serviceamountvaluessplit = explode('~',$serviceamountvalues);
		$serviceslnovaluessplit = explode(',',$serviceslnovalues);
		...
		$servicecount = 0;$offercount = 0;$productarraycount = 0;
		...
		$calculatedprice = calculatenormalprice($productamountsplit);
		...
		$netamount = $servicetax + $sbtax + $kk_taxamount + $igsttax + $cgsttax + $sgsttax + $totalproductprice;
		...
		{
			//sevice description
			for($k=0; $k<$servicesplitcount; $k++)
			{
				if($servicecount > 0)
					$servicegrid .= '*';
				$servicegrid .= $serviceslnovaluessplit[$k].'$'.$servicelistsplit[$k].'$'.$serviceamountvaluessplit[$k];
				$servicecount++;
		...
		{
			//offer description
			for($j=0; $j<$descriptiontypevaluescount; $j++)
			{
			
				if($offercount > 0)
					$offerdescriptiongrid .= '*';
				$offerdescriptiongrid .= $descriptiontypevaluessplit[$j].'$'.$descriptionvaluesplit[$j].'$'.$descriptionamountvaluessplit[$j];
				$offercount++;
		...
		if($productslnovalues <> '')
		{
			...
			{
				$proquery = "select subgroup,productname from inv_mas_product where productcode = '".$productlistsplit[$i]."'";
				$profetch = runmysqlqueryfetch($proquery);
				$subgroup = $profetch['subgroup'];

				if($subgroup == 'ESS')
				{
					$productname[] = $profetch['productname'];
				}
				else
				{
					$query = "select inv_mas_product.productcode,inv_mas_product.productname,inv_mas_product.year as `year` from inv_dealer_pricing left join inv_mas_product on inv_dealer_pricing.product = inv_mas_product.productcode where inv_mas_product.notinuse = 'no' and inv_mas_product.productcode = '".$productlistsplit[$i]."'";
					$result666 = runmysqlqueryfetch($query);
					if($productlistsplit[$i] == "222" || $productlistsplit[$i] == "221" 
					|| $productlistsplit[$i] == "219" || $productlistsplit[$i] == "223" || $productlistsplit[$i] == "220" || 
					$productlistsplit[$i] == "101")
					{ 
						$productname[] = $result666['productname'];
					}
					else
					{
						$productname[] = $result666['productname'].' '.'-'.' '.'('.$result666['year'].')';
					}
				}
		...
		if($productslnovalues <> '')
		{
			...
			for($i=0; $i<$productlistsplitcount; $i++)
			{
				...
				{
					if($usagevaluesplit[$i] == 'singleuser')
					$usagetype = 'Single User';
					else if($usagevaluesplit[$i] == 'multiuser')
						$usagetype = 'Multi User';
					else if($usagevaluesplit[$i] == 'addlicense')
				...
				$description .= $productslnovaluessplit[$i].'$'.$productname[$i].'$'.$purchasetype.'$'.$usagetype.'$'.$productpinvalue[$i][0].'$'.$productpinvalue[$i][1].'$'.$productamountsplit[$i];
				$productarraycount++;
		...
		//print_r($productlistsplit);exit;
		if($productslnovalues <> '')
		{
			for($i = 0; $i < $productlistsplitcount; $i++)
			{
				$usagetypevalue = $usagevaluesplit[$i];
				if($usagetypevalue == 'addlicense')
				{
				...
				$query7 = "UPDATE inv_dealercard set productcode = '".$productlistsplit[$i]."',dealerid = '".$dealerid."', usagetype = '".$usagetype."', purchasetype = '".$purchasetypesplit[$i]."',addlicence = '".$addlicence."' where cardid = '".$productpinvalue[$i][1]."' and invoiceid = '".$invoiceno."' ;";
		...
		//Update the inv_invoicenumbers table
		$invoicequery = "update inv_invoicenumbers set description = '".$description."', amount = '".$totalproductprice."',servicetax = '".$servicetax."',igst = '".$igsttax."',cgst = '".$cgsttax."',sgst = '".$sgsttax."',sbtax = '".$sbtax."',kktax = '".$kktax."', netamount = '".$netamount."',businessname = '".$businessname."',phone =  '".$phone."',cell = '".$cell."',emailid = '".$emailid."',contactperson = '".$contactperson."',address ='".addslashes($address)."', amountinwords = '".$amountinwords."', offerdescription = '".$offerdescriptiongrid."', invoiceremarks = '".$invoiceremarks."', servicedescription = '".$servicegrid."',servicetype = '".$servicelist."',serviceamount = '".$serviceamountvalues."', dealername = '".$result55['dealername']."', dealerid = '".$dealerid."',status = 'EDITED', editeddate = '".date('Y-m-d').' '.date('H:i:s')."',editedip = '".$_SERVER['REMOTE_ADDR']."',editedby = '".$userid ."',editedremarks = '".$editremarks."',amountinwords = '".$amountinwords."',totalproductpricearray = '".$totalproductpricearray."',products = '".$productlist."'  where slno  ='".$invoiceno."';";
		$invoiceresult = runmysqlquery($invoicequery);
		
		//echo "this is messis";
		//exit();
		
		$eventquery = "Insert into inv_logs_event(userid,system,eventtype,eventdatetime,remarks) values('".$userid."','".$_SERVER['REMOTE_ADDR']."','82','".date('Y-m-d').' '.date('H:i:s')."','Invoice_editing')";
		$eventresult = runmysqlquery($eventquery);
		
		//Fetch details from invoice table
		$queryres5 = "Select right(customerid,5) as slno from inv_invoicenumbers where slno = '".$invoiceno."'";
		$result57 = runmysqlqueryfetch($queryres5);
		$cusid = $result57['slno'];
		...
		//Update the customer master table
		$query01 = "Update inv_mas_customer set currentdealer = '".$dealerid."',region = '".$regionid."',branch = '".$branchid."' where slno = '".$cusid."'";
		$fetch01 = runmysqlquery($query01);
		...
		$queryresult88 = "Select `status`,editedby,editeddate,editedremarks from inv_invoicenumbers where slno  ='".$invoiceno."';";
	...
	case 'cancelinvoice':
	{
		...
		$cnlquery = "select * from inv_invoicenumbers where slno  ='".$invoiceno."';";
		...
		if($irnstatus == 1)
		{
			...
			for($i=0;$i<$productlistsplitcount;$i++)
			{
				$productpinvalue[] = explode('%%',$productlistsplit[$i]);
			}
			for($i=0; $i<$productlistsplitcount; $i++)
			{
				if($productpinvalue[$i][1]!= 0)
				{
					//Blocked and cancelled the PIN 
					$query11 = "UPDATE inv_mas_scratchcard SET blocked = 'yes',cancelled = 'yes' ,remarks = 'invoicing' where cardid =
					'".$productpinvalue[$i][1]."' ";
					$resultvalue = runmysqlquery($query11);
					
					$blockquery = "insert into inv_blockcanceldetails(cardid,userid,datetime,status,remarks) 
					values('".$productpinvalue[$i][1]."','".$userid."','".date('Y-m-d').' '.date('H:i:s')."','Blocked','invoicing')";
					$blockresult = runmysqlquery($blockquery);
					
					/*
					$query12 = "select remarks from inv_mas_scratchcard where cardid = '".$productpinvalue[$i][1]."' ";
					$result12 = runmysqlquery($query12);
					while($fetch12 = mysqli_fetch_array($result12))
					{
						$emptyremarks = $fetch12['remarks'];
					}
					
					if($emptyremarks == "")
					{
						//update remarks as invoicing when there is no remarks mentioned or empty
						$query11 = "UPDATE inv_mas_scratchcard SET blocked = 'yes',cancelled = 'yes', remarks = 'invoicing' 
						where cardid = '".$productpinvalue[$i][1]."' ";
						$resultvalue = runmysqlquery($query11);
					}
					else
					{
					//remarks removed because it is updating the given remarks of blocked pin during cancellation of invoice
					//$query11 = "UPDATE inv_mas_scratchcard SET blocked = 'yes',cancelled = 'yes', remarks = 'invoicing' where cardid = 
					$query11 = "UPDATE inv_mas_scratchcard SET blocked = 'yes',cancelled = 'yes' where cardid =
					'".$productpinvalue[$i][1]."' ";
					$resultvalue = runmysqlquery($query11);
					}*/
				}
			...
			//Update the inv_invoicenumbers table
			$invoicequery = "update inv_invoicenumbers set status = 'CANCELLED',cancelleddate = '".date('Y-m-d').' '.date('H:i:s')."',cancelledip = '".$_SERVER['REMOTE_ADDR']."',cancelledby = '".$userid ."',cancelledremarks = '".$cancelremarks."'  where slno  ='".$invoiceno."';";
			$invoiceresult = runmysqlquery($invoicequery);
			
			$eventquery = "Insert into inv_logs_event(userid,system,eventtype,eventdatetime,remarks) values('".$userid."','".$_SERVER['REMOTE_ADDR']."','83','".date('Y-m-d').' '.date('H:i:s')."','Invoice_cancellation')";
			$eventresult = runmysqlquery($eventquery);
			
			//resend the mail after Cancelled sucessfully	
			//resendinvoice($invoiceno);	
			
			$queryresult55 = "Select `status`,cancelledby,cancelleddate,cancelledremarks from inv_invoicenumbers where slno  ='".$invoiceno."';";
	...
	case 'canceleinv':
	{
		...
		$productlistsplitcount =  count($productlistsplit);
		$cancelreason = $_POST['cancelreason'];
		$cancelconfirm = $_POST['cancelconfirm'];
	...
	case 'searchbycustomerid':
	{
		...
		$cusid = strlen($customerid);
		if($cusid == 17)
			$customerid = substr($customerid,12);
		else if($cusid == 20)
			$customerid = substr($customerid,15);
		else
			$customerid = $customerid;
			
		$query = "select slno, invoiceno from inv_invoicenumbers where right(customerid,5) = '".$customerid."'";
		$result = runmysqlquery($query);
		$count = 0;
		while($fetch = mysqli_fetch_array($result))
		{
			$searchbycustomeridarray[$count] = $fetch['invoiceno'].'^'.$fetch['slno'];
			$count++;
	...
	case 'searchcustomerlist':
	{
		...
		$branchpiece = ($branch2 == "")?(""):(" AND inv_invoicenumbers.branchid = '".$branch2."' ");
		...
		switch($databasefield)
		{
			case "slno":
				...
				$query = "select inv_invoicenumbers.invoiceno,inv_invoicenumbers.slno from inv_invoicenumbers 
...
where right(inv_invoicenumbers.customerid,5) = '".$customerid."' AND left(inv_invoicenumbers.createddate,10) between '".$fromdate."' and '".$todate."' AND (".$finalproductlist.") ".$regionpiece.$district_typepiece.$state_typepiece.$dealer_typepiece.$branchpiece.$typepiece.$categorypiece.$generatedpiece.$statuspiece.$seriespiece."  ORDER BY invoiceno";
				break;
...
left join inv_mas_customer on inv_mas_customer.slno = right(inv_invoicenumbers.customerid,5)
left join inv_mas_district on inv_mas_district.districtcode = inv_mas_customer.district
where inv_invoicenumbers.contactperson LIKE '%".$textfield."%' AND left(inv_invoicenumbers.createddate,10) between '".$fromdate."' and '".$todate."' AND (".$finalproductlist.") ".$regionpiece.$district_typepiece.$state_typepiece.$dealer_typepiece.$branchpiece.$typepiece.$categorypiece.$generatedpiece.$statuspiece.$seriespiece."  ORDER BY invoiceno";
				break;
			case "phone":
				$query = "select inv_invoicenumbers.invoiceno,inv_invoicenumbers.slno from inv_invoicenumbers 
left join inv_mas_customer on inv_mas_customer.slno = right(inv_invoicenumbers.customerid,5)
left join inv_mas_district on inv_mas_district.districtcode = inv_mas_customer.district
where inv_invoicenumbers.phone LIKE '%".$textfield."%' OR inv_invoicenumbers.cell LIKE '%".$textfield."%' AND left(inv_invoicenumbers.createddate,10) between '".$fromdate."' and '".$todate."' AND (".$finalproductlist.") ".$regionpiece.$district_typepiece.$state_typepiece.$dealer_typepiece.$branchpiece.$typepiece.$categorypiece.$generatedpiece.$statuspiece.$seriespiece."  ORDER BY invoiceno ";
				break;
			case "place":
				$query = "select inv_invoicenumbers.invoiceno,inv_invoicenumbers.slno from inv_invoicenumbers 
left join inv_mas_customer on inv_mas_customer.slno = right(inv_invoicenumbers.customerid,5)
left join inv_mas_district on inv_mas_district.districtcode = inv_mas_customer.district
where inv_invoicenumbers.place LIKE '%".$textfield."%' AND left(inv_invoicenumbers.createddate,10) between '".$fromdate."' and '".$todate."' AND (".$finalproductlist.") ".$regionpiece.$district_typepiece.$state_typepiece.$dealer_typepiece.$branchpiece.$typepiece.$categorypiece.$generatedpiece.$statuspiece.$seriespiece."  ORDER BY invoiceno";
				break;
			case "emailid":
				$query = "select inv_invoicenumbers.invoiceno,inv_invoicenumbers.slno from inv_invoicenumbers 
left join inv_mas_customer on inv_mas_customer.slno = right(inv_invoicenumbers.customerid,5)
left join inv_mas_district on inv_mas_district.districtcode = inv_mas_customer.district
where inv_invoicenumbers.emailid LIKE '%".$textfield."%' AND left(inv_invoicenumbers.createddate,10) between '".$fromdate."' and '".$todate."' AND (".$finalproductlist.") ".$regionpiece.$district_typepiece.$state_typepiece.$dealer_typepiece.$branchpiece.$typepiece.$categorypiece.$generatedpiece.$statuspiece.$seriespiece."  ORDER BY invoiceno";
				break;
				case "cardid":
					$query = "select inv_invoicenumbers.invoiceno,inv_invoicenumbers.slno from inv_invoicenumbers 
left join inv_mas_customer on inv_mas_customer.slno = right(inv_invoicenumbers.customerid,5)
...
where inv_mas_scratchcard.cardid LIKE '%".$textfield."%' AND left(inv_invoicenumbers.createddate,10) between '".$fromdate."' and '".$todate."' AND (".$finalproductlist.") ".$regionpiece.$district_typepiece.$state_typepiece.$dealer_typepiece.$branchpiece.$typepiece.$categorypiece.$generatedpiece.$statuspiece.$seriespiece."  ORDER BY invoiceno";
					break;
			case "scratchnumber":
				$query = "select inv_invoicenumbers.invoiceno,inv_invoicenumbers.slno from inv_invoicenumbers 
left join inv_mas_customer on inv_mas_customer.slno = right(inv_invoicenumbers.customerid,5)
...
inv_mas_scratchcard.scratchnumber LIKE '%".$textfield."%' AND left(inv_invoicenumbers.createddate,10) between '".$fromdate."' and '".$todate."' AND (".$finalproductlist.") ".$regionpiece.$district_typepiece.$state_typepiece.$dealer_typepiece.$branchpiece.$typepiece.$categorypiece.$generatedpiece.$statuspiece.$seriespiece."  ORDER BY invoiceno";
				break;
...
left join inv_mas_customer on inv_mas_customer.slno = right(inv_invoicenumbers.customerid,5)
left join inv_mas_district on inv_mas_district.districtcode = inv_mas_customer.district
where inv_invoicenumbers.invoiceno LIKE '%".$textfield."%' AND left(inv_invoicenumbers.createddate,10) between '".$fromdate."' and '".$todate."' AND (".$finalproductlist.") ".$regionpiece.$district_typepiece.$state_typepiece.$dealer_typepiece.$branchpiece.$typepiece.$categorypiece.$generatedpiece.$statuspiece.$seriespiece."  ORDER BY invoiceno ";
				break;
...
left join inv_mas_customer on inv_mas_customer.slno = right(inv_invoicenumbers.customerid,5)
left join inv_mas_district on inv_mas_district.districtcode = inv_mas_customer.district
where inv_invoicenumbers.businessname LIKE '%".$textfield."%' AND left(inv_invoicenumbers.createddate,10) between '".$fromdate."' and '".$todate."' AND  (".$finalproductlist.") ".$regionpiece.$district_typepiece.$state_typepiece.$dealer_typepiece.$branchpiece.$typepiece.$categorypiece.$generatedpiece.$statuspiece.$seriespiece."  ORDER BY invoiceno";
				break;
		}
...
function updateaddlinebreak($linecount)
{
	switch($linecount)
	{
		case '1':
		{
			$linebreak = '<tr><td  class="grey-td-border-grid"><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br></td><td  class="grey-td-border-grid">&nbsp;</td><td  class="grey-td-border-grid">&nbsp;</td><td  class="grey-td-border-grid">&nbsp;</td><td  class="grey-td-border-grid">&nbsp;</td></tr>';
		}
		break;
		case '2':
		{
...
function calculatenormalprice($productamountsplit)
{
	...
	return $totalproductprice.'$'.$totalproductpricearray;
}
...
function selectionforproduct($productcodelist,$productslnolist)
{
	...
	{
		$count = 1;
		$query11 = "(select inv_mas_product.productcode,inv_mas_product.productname,inv_mas_product.year as `year` from inv_dealer_pricing left join inv_mas_product on inv_dealer_pricing.product = inv_mas_product.productcode where inv_mas_product.notinuse = 'no')
					union
					(select productcode,productname,subgroup from inv_mas_product where subgroup = 'ESS') order by productname;";
		...
		{
			if($fetch['productcode'] ==  "222" || $fetch['productcode'] == "221" || $fetch['productcode'] == "219" || $fetch[
			'productcode'] == "223" || $fetch['productcode'] == "220" || $fetch['productcode'] == "101")
			{
			$dealerselectres .= '<option  style="width:295px;" ' . ($productcodelist[$i] == $fetch['productcode'] ? 'selected' : '') . ' value="'.$fetch['productcode'].'">'.$fetch['productname'].'</option>'; 
			}
			else
			{
				$dealerselectres .= '<option  style="width:295px;" ' . ($productcodelist[$i] == $fetch['productcode'] ? 'selected' : '') . ' value="'.$fetch['productcode'].'">'.$fetch['productname'].' '.'-'.' '.'('.$fetch['year'].')'.'</option>'; 
			}
		}
