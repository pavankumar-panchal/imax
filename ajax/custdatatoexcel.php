...
LEFT JOIN inv_mas_customertype on inv_mas_customertype.slno = inv_mas_customer.type 
LEFT JOIN inv_mas_customercategory on inv_mas_customercategory.slno = inv_mas_customer.category
LEFT JOIN inv_mas_dealer on inv_mas_dealer.slno = inv_mas_customer.currentdealer  where inv_mas_customer.slno not in (select inv_contactdetails.customerid as slno from inv_contactdetails) group  by inv_mas_customer.slno order by inv_mas_customer.businessname";
						}
						break;
			case 'dummy' : 
						{
...
or  (inv_contactdetails.emailid REGEXP '^[ ]$') or  (inv_contactdetails.emailid like '%df@dff.com%')group  by inv_contactdetails.customerid  order by inv_mas_customer.businessname";
	...
	$objPHPExcel->setActiveSheetIndex(0)
		...
		if(($_SERVER['HTTP_HOST'] == "meghanab") || ($_SERVER['HTTP_HOST'] == "rashmihk") || ($_SERVER['HTTP_HOST'] == "vijaykumar"))
			...
			$eventquery = "Insert into inv_logs_event(userid,system,eventtype,eventdatetime,remarks) values('".$userid."','".$_SERVER['REMOTE_ADDR']."','90','".date('Y-m-d').' '.date('H:i:s')."','excel_dataanalysis_report".'-'.strtolower($username)."')";
