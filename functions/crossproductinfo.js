	...
	ajaxcall4 = $.ajax(
	{
		...
		{	
			...
			else
			{
				...
				if(response[0] == 1)
				{
					$("#displaydetails").html(response[1]);
				}
				else
...
function formsubmit(command)
{
	...
	if(command == 'save')
	{
		...
		else if(command == 'delete')
		{
			...
			if(confirmation)
			{
				passData =  "switchtype=delete&lastslno=" + encodeURIComponent($("#lastslno").val()) + "&dummy=" + Math.floor(Math.random()*10000000000);//alert(passData)
			}
			else
			return false;
		}
...
function crossdatagrid(id,startlimit)
{
	...
	$('#cusslno').val(id);
	$('#filterdiv').show();
	displaydivdisplay('filterdiv','toggleimg2')

	//newentry();
	var passData = "switchtype=generategrid&lastslno="+ encodeURIComponent(id) + "&startlimit=" + encodeURIComponent(startlimit);//alert(passData)
	var queryString = "../ajax/crossproduct.php";
...
function gridtoform(slno)
{
	...
	{//alert(slno);
		...
		error.html('');
		$('#gridslno').val(slno);
		var passData = "switchtype=gridtoform&slno=" + encodeURIComponent(slno) + "&dummy=" + Math.floor(Math.random()*100032680100);//alert(passData)
		$('#form-error').html(getprocessingimage());
		var queryString = "../ajax/crossproduct.php";
		ajaxcall7 = $.ajax(
		{
...
function newentry()
{
	...
	$('#displaydetails').show();
	displaydivdisplay('filterdiv','toggleimg2');
	displaydivdisplay('displaydetails','toggleimg1');
	//disableupdate();
	//$('#DPC_entereddate').val('');
...
{ 
	...
	var businessname = $('#displaycompanynamehidden').val(); //alert($('#displaycompanynamehidden').val());
	var error = $('#form-error');
	var field = $('#status'); 
...
function displaydivdisplay(elementid,imgname)
{
