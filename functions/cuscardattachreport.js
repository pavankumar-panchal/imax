...
function formsubmit(command)
{
	...
	{
		var field = $('#region'); 
		if(!field.val()) { error.html(errormessage('Select A Region.')); field.focus(); return false; } 
	}
	if(field1 == 'state') 
	{
		var field = $('#state'); 
		if(!field.val()) { error.html(errormessage('Select A State.')); field.focus(); return false; } 
	}
	...
	else
	{
		...
		{
			error.html('');
			$('#submitform').attr("action", "../reports/excelcuscardattachreport.php?id=view") ;
		...
		{
			error.html('');
			$('#submitform').attr("action", "../reports/excelcuscardattachreport.php?id=toexcel") ;
			$('#submitform').submit();
		}
	}
}
