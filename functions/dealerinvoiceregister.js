...
function getinvoicedetails(startlimit)
{
	...
	var queryString = "../ajax/dealerinvoiceregister.php";
...
function filtertoexcel(command)
{
	...
	{
		error.html('');
		$('#submitform').attr("action", "../ajax/dealerinvoiceregistertoexcel.php?id=toexcel") ;
		$('#submitform').submit();
	}
}
...
function invoicelistOnline(type,startlimit)
{
	var error = $('#form-error');
	$("#category").val(type);
	var passData = "switchtype=getregionwiseinvoicelistOnline&type="+encodeURIComponent(type)+"&startlimit="+ encodeURIComponent(startlimit)+ "&dummy=" + Math.floor(Math.random()*1000782200000); 
	var queryString = "../ajax/dealerinvoiceregister.php";
	error.html(getprocessingimage());
	ajaxcall9 = $.ajax(
	{
		...
		{	
			...
			else
			{
				...
				if(response[0] == '1')
				{
					gridtab6(5,'tabgroupgrid','&nbsp; &nbsp;Online'); 
					$('#tabgroupgridwb5').html("Total Count :  " + response[2]);
					$('#tabgroupgridc5_1').html(response[1]);
				...
				{
					$('#tabgroupgridc5_1').html("No datas found to be displayed.");
				}
			}
		}, 
		error: function(a,b)
...
function getmoreofinvoicelistOnline(startlimit,slnocount,showtype)
{
	var error = $('#form-error');
	var passData = "switchtype=getregionwiseinvoicelistOnline&type="+encodeURIComponent($("#category").val())+"&startlimit=" + encodeURIComponent(startlimit) + "&slnocount=" + encodeURIComponent(slnocount) + "&showtype=" + encodeURIComponent(showtype) + "&dummy=" + Math.floor(Math.random()*1000782200000); 
	...
	ajaxcall10 = $.ajax(
	{
		...
		{	
			...
			else
			{
				...
				if(ajaxresponse[0] == '1')
				{
					$('#tabgroupgridwb5').html("Total Count :  " + ajaxresponse[2]);
					$('#searchresultgridOnline').html($('#tabgroupgridc5_1').html());
					$('#tabgroupgridc5_1').html($('#searchresultgridOnline').html().replace(/\<\/table\>/gi,'')+ ajaxresponse[1]);
					$('#tabgroupgridc5link').html(ajaxresponse[3]);
					$('#totalinvoices').val(ajaxresponse[4]);
					$('#totalsalevalue').val(ajaxresponse[5]);
					$('#totaltax').val(ajaxresponse[6]);
					$('#totalamount').val(ajaxresponse[7]);
...
function resendinvoice(invoiceno,slno)
{
	...
	else
	{
		...
		if(confirmation)
		{
			...
			queryString = "../ajax/dealerinvoiceregister.php";
			ajaxcall11 = $.ajax(
			{
				type: "POST",url: queryString, data: passData, cache: false,
				success: function(ajaxresponse,status)
...
function generateservicedetails()
{
	...
	var queryString = "../ajax/dealerinvoiceregister.php";//alert(passData)
	$('#productdetailsgridc1_1').html(getprocessingimage());
	$('#productdetailsgridc1link').html('');
