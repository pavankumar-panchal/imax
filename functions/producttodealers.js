...
function newentry()
{
	...
	document.getElementById('dofilter').disabled=true;
	...
	var chk_arr =  document.getElementsByName("checkvalue[]");
...
function filter(input)
{
	...
	if(selected=="all")
	{ 
		if(chk_arr[(chklength-1)].checked==true)
		{
		  for(k=0;k< chklength;k++)
		  {
		...
		{
			chk_arr[(chklength-1)].checked=false;
			for(k=0;k< chklength;k++)
			{
				chk_arr[k].checked = false;
				enable(chk_arr[k]);
	...
	else
	{
		...
		if(chk==length)
		{
			chk_arr[length].checked = true;
			selected="all";
		}
		else
		{
			chk_arr[length].checked = false;
		}
	}
}

//enable branch and region dropdown
function enable(input)
{
	var selected=input.value;
	if(selected=="branchwise")
	{
		//region
	...
	else if(selected=="regionwise")
	{
		//region
		if(document.getElementById('region').disabled==true)
		{
...
function filterdealter()
{
	...
	var pass="";
	var errormsg="";
	for(k=0;k< (chklength-1);k++)
	{
		if(k<2)
		{
		  if(chk_arr[k].checked == true)
			  pass+="&"+chk_arr[k].value+"="+encodeURIComponent(chk_arr[k].value);
		}
		else if(k == 2 && (chk_arr[k].checked == true))
		{
			var region=document.getElementById('region');
			var sel=region.selectedIndex;
			if(sel==0)
				errormsg+="Select Region ";
			else
				pass+="&region="+encodeURIComponent(region.value);
		}
		else if(k == 3 && (chk_arr[k].checked == true))
		{
			var branch=document.getElementById('branch');
			var sel=branch.selectedIndex;
			if(sel==0)
			{
				if(errormsg!="")
					errormsg+=" and Branch";
				...
				pass+="&branch="+encodeURIComponent(branch.value);
		}
	}
	if(errormsg=="")
	{			
		if(selected=="all")
		{
			passData = "changetype=filter&productcode="+ encodeURIComponent($("#productcode").val()) + pass+"&all=all";
		}
		else
		{
			passData = "changetype=filter&productcode="+ encodeURIComponent($("#productcode").val()) + pass+"&all=no";
		}
		//passData = "changetype=filter";
		//alert(passData);
		queryString = '../ajax/producttodealers.php';
		error.html('<img src="../images/imax-loading-image.gif" border="0" />');
		ajaxcall22 = $.ajax(
