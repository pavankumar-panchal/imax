...
function refreshdealerarray()
{
	...
	queryString = "../ajax/productmapping.php";
	ajaxobjext14 = $.ajax(
	{
...
function productdatagrid(startlimit)
{
	if($("#dealerlist").val())
	{
		var form = $("#submitform");
		var startlimit = '';
		var passData = "type=generategrid&dealerid=" + encodeURIComponent($("#dealerlist").val()) + "&startlimit=" + encodeURIComponent(startlimit) + "&dummy=" + Math.floor(Math.random()*10054300000);//alert(passData)
...
function getmoreproductdatagrid(startlimit,slnocount,showtype)
{
	if($("#dealerlist").val())
	{
		...
		$("#getmoredistrictlink").html(getprocessingimage());	
		queryString = "../ajax/productmapping.php";
		ajaxcall3 = $.ajax(
		{
			type: "POST",url: queryString, data: passData, cache: false,
...
function gridtoform(slno)
{
	if(slno != '')
	{
		...
		var queryString = "../ajax/productmapping.php";
		ajaxcall5 = $.ajax(
		{
			type: "POST",url: queryString, data: passData, cache: false,dataType: "json",
			success: function(ajaxresponse,status)
...
function formsubmit(command)
{
	...
	if(!field.val()) { $("#form-meg").html(errormessage("Select A Product. ")); field.focus(); return false; }
	var passData = "";
	if(command == 'save')
	{
		passData =  "type=save&dealerid=" + encodeURIComponent($("#dealerlist").val())  + "&productcode=" + encodeURIComponent($("#productcode").val()) + "&lastslno=" + encodeURIComponent($("#lastslno").val()) + "&dummy=" + Math.floor(Math.random()*10045606700000);//alert(passData);
	}
	else
	{
