...
function formsubmitduplicate()
{
	...
	else
	{
		...
		$('#submitform').attr("action", "../ajax/matrixbulkprintduplicate.php") ;
...
function resendinvoice(invoiceno,slno)
{
	...
	else
	{
		...
		if(confirmation)
		{
			var passData = "switchtype=resendmatrixinvoice&invoiceno=" + encodeURIComponent(invoiceno) + "&dummy=" + Math.floor(Math.random()*10054300000);
			$('#resendprocess'+slno).show();
			$('#resendemail'+slno).hide();
			$('#resendprocess'+slno).html(getprocessingimage());	
			queryString = "../ajax/matrixbulkprinting.php";
			ajaxcall2 = $.ajax(
			{
				type: "POST",url: queryString, data: passData, cache: false,
				success: function(ajaxresponse,status)
...
function generateservicedetails()
{
	...
	var queryString = "../ajax/matrixbulkprinting.php";//alert(passData)
	$('#productdetailsgridc1_1').html(getprocessingimage());
	$('#productdetailsgridc1link').html('');
	ajaxcall8 = $.ajax(
	{
...
function formsubmit()
{
	...
	else
	{
		...
		$('#hiddenslno').val(searcharray);
		$('#submitform').attr("action", "../ajax/matrixbulkprintingpdf.php") ;
		$('#submitform').attr( 'target', '_blank' );
		$('#submitform').submit();
	}
