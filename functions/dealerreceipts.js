...
function formsubmit(command)
{
	...
	if($('#invoivcelist').length > 0 && $('#receipttype').val()!='Online')
	{
...
function gettotaldealerrcount()
{
	var form = $('#customerselectionprocess');
	var passData = "switchtype=getdealercount&dummy=" + Math.floor(Math.random()*10054300000);
	queryString = "../ajax/dealerreceipts.php";
	ajaxcall1 = $.ajax(
	{
		type: "POST",url: queryString, data: passData, cache: false,dataType: "json",
		...
		{	
			...
			else
			{
				...
				refreshdealerarray(response['count']);
			}
		}, 
		error: function(a,b)
		{
...
function refreshdealerarray(customercount)
{
	var form = $('#customerselectionprocess');
	...
	var passData = "switchtype=generatedealerlist&dummy=" + Math.floor(Math.random()*10054300000) + "&limit=" + encodeURIComponent(limit) + "&startindex=" + encodeURIComponent(startindex);
	var passData1 = "switchtype=generatedealerlist&dummy=" + Math.floor(Math.random()*10054300000) + "&limit=" + encodeURIComponent(limit) + "&startindex=" + encodeURIComponent(startindex1);
	var passData2 = "switchtype=generatedealerlist&dummy=" + Math.floor(Math.random()*10054300000) + "&limit=" + encodeURIComponent(limit) + "&startindex=" + encodeURIComponent(startindex2);
	var passData3 = "switchtype=generatedealerlist&dummy=" + Math.floor(Math.random()*10054300000) + "&limit=" + encodeURIComponent(limit) + "&startindex=" + encodeURIComponent(startindex3);
...
function selectfromlist()
{
	...
	//alert(selectbox);
	$('#dealerslno').val($("#customerlist option:selected").val());
	//alert($("#dealerslno").val());
	$('#detailsearchtext').val($("#customerlist option:selected").text());
	$('#detailsearchtext').select();
	$('#displaydealername').html($("#customerlist option:selected").text());
	newcreditentry();
	$('#form-error').html('');
	generatereceiptgrid('');
	getuserinvoicelist();
...
{
	var form = $('#submitform');
	var passData = "switchtype=getuserinvoicelist&dealerreference=" + encodeURIComponent($('#dealerslno').val()) + "&dummy=" + Math.floor(Math.random()*10054300000);//alert(passData);
...
function generatereceiptgrid(startlimit)
{
	if($('#dealerslno').val() != '')
	{
		var form = $('#submitform');
		var startlimit = '';
		var passData = "switchtype=generatereceiptgrid&startlimit="+ encodeURIComponent(startlimit) + "&customerreference=" + $('#dealerslno').val();
		var queryString = "../ajax/dealerreceipts.php";
...
function getmoregeneratereceiptgrid(startlimit,slnocount,showtype)
{
	if($('#dealerslno').val() != '')
	{
		var form = $('#submitform');
	//	$('#lastslno').val() = id;	
		var passData = "switchtype=generatereceiptgrid&startlimit="+ encodeURIComponent(startlimit) + "&slnocount=" + encodeURIComponent(slnocount) + "&showtype=" + encodeURIComponent(showtype) + "&customerreference=" + $('#dealerslno').val() + "&dummy=" + Math.floor(Math.random()*1000782200000);
		//alert(passData);
		var queryString = "../ajax/dealerreceipts.php";
			...
			{	
				...
				else
				{
					...
					{ 
						...
						$('#receipttype').val(response['module']);
						if(response['module'] == 'Online')
						{
							$('#viewinvoice').hide();
						}
						else
						...
						if(response['status'] == 'CANCELLED')
							disablereceptsbutton();
						else
							enablereceptsbutton();
						if(response['paymentmode'] == 'creditordebit')
						{
							$('#save').attr("disabled", true); 
							$("#save").removeClass('swiftchoicebutton');
							$("#save").addClass('swiftchoicebuttondisabled');
					...
					//	$("input[name='paymentmode']:checked").val(response[7]);
						//$('#'+response[7]).is(':checked');
					}
					else
					{
...
function viewdealerinvoice()
{
	...
	if($('#onlineslno').val() == '')
	{
		$('#form-error').html(errormessage('Please select a Dealer.')); return false;
	}
	else
	{
...
{
	if($('#lastslno').val() != '')
		$('#dealerreceiptno').val($('#lastslno').val());
		//alert($('#dealerreceiptno').val());
	if($('#dealerreceiptno').val() == '')
	{
		$('#form-error').html(errormessage('Please select a Dealer.')); return false;
	}
	else
...
function receiptcancellation()
{
	...
	var receiptno = $('#lastslno',form).val();

	if(!$('#cancelremarks').val()) { alert("Enter the Cancellation Remarks"); $('#cancelremarks').focus(); return false; }
	passData =  "switchtype=cancelreceipt&receiptno=" + encodeURIComponent(receiptno)+ "&cancelremarks=" + encodeURIComponent($('#cancelremarks').val())  + "&dummy=" + Math.floor(Math.random()*10000000000);
	...
	ajaxcall9 = $.ajax(
	{
		...
		{	
			...
			else
			{
				...
				if(response[0] == '1')
				{
					receiptgridtoform($('#lastslno',form).val());
					$('#form-error1').html('');
					alert(response[1]);
					$().colorbox.close();
				}
				...
				{
					alert('Unable to Connect...' + ajaxresponse);
				}
			}
		}, 
		error: function(a,b)
