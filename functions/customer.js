...
//added new gst option
function newgstin() {
    $("#tabg1h3").trigger("click");
    return true;
}
...
function formsubmit(command)
{
	...
	if(command == 'save')
	{
		...
		if(fieldval!='')
		{
			if(field.val()) { if(!validategstin(field.val())) { error.html(errormessage('For GSTIN only Alpha / Numeric are allowed.')); field.focus(); newgstin(); return false; } }
		    if(field.val()) { if(!validategstinregex(field.val(),state_gst_code)) { error.html(errormessage('State GST Code Not Matching.')); field.focus(); newgstin(); return false; } }
	        if(field.val()) { if(fieldval.length != 15) { error.html(errormessage('GSTIN should be 15 chars.')); field.focus(); newgstin(); return false; }} 
	        var gstinformat = new RegExp('^[0-9]{2}[A-Z]{5}[0-9]{4}[A-Z]{1}[1-9A-Z]{1}[0-9A-Z]{2}$|^[0-9]{2}[A-Z]{4}[0-9]{5}[A-Z]{1}[1-9A-Z]{1}[0-9A-Z]{2}$');
	        if(!gstinformat.test(fieldval)) {  error.html(errormessage('State GST Code Not in Correct Format.')); field.focus(); newgstin(); return false; }
		...
		// 	if(fieldpan.val() == '')
		// 	{
		// 		error.html(errormessage('PAN NO is mandatory for Non GSTIN customer.')); fieldpan.focus();  return false;
		// 	}
		// 	else
		// 	{
		// 		var regex = /[A-Z]{5}[0-9]{4}[A-Z]{1}$/;    
		...
		//var field = $("#gst_no" );
		//if(!field.val()) { error.html(errormessage("Enter the Gst No. ")); field.focus(); return false; }
		var field = $("#state" );
		...
		//alert($("#tanno").val());
		passData =  "switchtype=save&businessname=" + encodeURIComponent($("#businessname").val()) + "&oldbusinessname=" + encodeURIComponent($("#oldbusinessname").val()) +
			"&gst_no=" + encodeURIComponent($("#gst_no").val())  + "&sez_enabled=" + encodeURIComponent(sez_enabled)  + "&customerid=" + encodeURIComponent($("#customerid").val())  +
			"&address=" + encodeURIComponent($("#address").val()) + "&place=" + encodeURIComponent($("#place").val()) + "&pincode=" + encodeURIComponent($("#pincode").val()) +
			"&district=" + encodeURIComponent($("#district").val()) + "&region=" + encodeURIComponent($("#region").val()) + "&branch=" + encodeURIComponent($("#branch").val()) + "&category=" + encodeURIComponent($("#category").val()) +
			"&type=" + encodeURIComponent($("#type").val()) + "&stdcode=" + encodeURIComponent($("#stdcode").val()) + "&fax=" + encodeURIComponent($("#fax").val())  + "&website=" + encodeURIComponent($("#website").val()) +
			"&remarks=" + encodeURIComponent($("#remarks").val())   + "&currentdealer=" + encodeURIComponent($("#currentdealer").val()) + "&password=" + encodeURIComponent($("#password").val()) +
			"&createddate=" + encodeURIComponent($("#createddate").html()) + "&disablelogin=" + encodeURIComponent(disablelogin) + "&corporateorder=" + encodeURIComponent(corporateorder) +
			"&companyclosed=" + encodeURIComponent(companyclosed) + "&lastslno=" + encodeURIComponent($("#lastslno").val())+ "&isdealer=" + encodeURIComponent(isdealer)+ "&contactarray=" + encodeURIComponent(contactarray)+
			"&totalarray=" + encodeURIComponent(totalarray)+ "&displayinwebsite=" + encodeURIComponent(displayinwebsite)+ "&promotionalsms=" + encodeURIComponent(promotionalsms)+ "&promotionalemail=" + encodeURIComponent(promotionalemail) +
		 "&effective_from="+encodeURIComponent(effective_from) + "&tanno=" + encodeURIComponent($("#tanno").val()) + "&panno=" + encodeURIComponent($("#panno").val()) + "&dummy=" + Math.floor(Math.random()*100000000);
...
// { 
// 	//alert($('#detailsearchtext').val());
// 	var passData = "switchtype=searchcustomer&searchtext=" + encodeURIComponent($('#detailsearchtext').val());
...
// 				//alert(response['customerdata']);
// 			 	 $("#customerlist").empty();
				
// 					$.each(response['customerdata'], function(key, value) {   
// 						$('#customerlist').append($("<option></option>").attr("value", key).text(value)); 
// 				   });
...
// {
// 	var form = $('#customerselectionprocess');
// 	var passData = "switchtype=generatecustomerlist";
// 	queryString = "../ajax/customer.php";
// 	ajaxcall2 = $.ajax(
// 	{
// 		type: "POST",url: queryString, data: passData, cache: false,dataType: "json",
		...
		{	
			...
			else
			{
				...
				if(response[0] == 1)
				{
					...
					$("#disforcesurrender").html(response[4]);
				}
				else
				{
		...
		ajaxcall6 = $.ajax(
		{
			...
			{	
				...
				else
				{
					...
					//surrenderhistory(response['cusslno']);
					...
					$("#oldbusinessname").val(response['oldbusinessname']);
					$("#short_url").html(response['rescontact'] +"\r\n"+ response['businessname']+"\r\n"+ response['address']+"\n"+ response['place']+"\n"+ response['districtname']+"\n"+ response['statename']+"\n" + response['pincode']+"\r\n"+ response['resphone']+"\n"+ response['rescell']+"\n"+ response['resemailid']);
					$("#address").val(response['address']);
					$("#place").val(response['place']);
					
					$("#gst_delete_res").html('');//added by samar for new gst
					/*-------------------Commented for New GST Number ----------------------*/
/*					if(response['gst_no'] == "" || response['gst_no'] == null )
					...
					else
					{
					    ...
					    //alert(response['gst_no']);
    					 ...
    					 $("#gst_hidden").val(response['gst_no']);
    					 
					}*/
					/*-------------------Commented for New GST Number Ends----------------------*/
					...
					$('#gstdetails tr.newaddedgst').remove();
					$("#gstdetails").append(response['gst_new_grid']);
					//$("#gst_hidden").val(response['last_new_gst_no']);
					$("#gst_no").val(response['last_new_gst_no']);
					$("#effective_from").val(response['last_effective_from']);
					...
					if(response['p_registration'] == 'yes') 
					{ 
					    generatecustomerautoregistration(response['cusslno'],''); 
					    generatecustomerregistration(response['cusslno'],''); 
						enableregistration(); 
...
function deletegstin(value) {
    
    //$('#gst_delete_res').text('').hide();
    $("#gst_delete_res").html(getprocessingimage());
    var deleteid = value;
    var deleteid = deleteid.split('*');
    
    var idtodelete = deleteid['0'];
    var rowid = deleteid['1'];
    
    var customerslno = $('#lastslno').val();
    
    var queryString = "../ajax/customer.php";
    var passData = "switchtype=deletegstin&deleteid="+encodeURIComponent(idtodelete)+"&customerslno="+encodeURIComponent(customerslno)+"&dummy="+Math.floor(Math.random()*100032680100);//alert(passData)
    ...
    ajaxcall37 = $.ajax(
	{
		...
		{	
			...
			else
			{
                ...
                //debugger;
                var response = ajaxresponse;
                var delete_status = response['delete_status'];
                
                var delete_status_res = delete_status.split("*");
                
                if(delete_status_res['0'] == '1') {
                    $('table#gstdetails tr#'+rowid).remove();
                    $('#gst_delete_res').html(response['delete_status']['1']).css("color", "red").show();
                    //$('#gst_delete_res').text(response['delete_status']);
                    if(response['gst_no']!= "")
                    {
                    ...
                    $('#gst_delete_res').html(response['delete_status']).css("color", "red").show();
                }
                //debugger;
			}
		}, 
