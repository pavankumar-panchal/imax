...
function customerdetailstoform(cusid)
{
	    ...
	    //console.log(productprice_actual);
			...
			{	
				...
				else
				{
					...
					if(response['errorcode'] == '1')
					{
						...
						//added on 03/11/2017 for disabling gstin column
						...
						//$('#states_gst').html(response['state_gst_code']);
...
function selectfromlist()
{
	...
	enablebuttontype();
	sezfunc();
	$('#gst_type_no').removeAttr('checked');
	$('#gst_type_yes').attr('checked', true);
...
function deleteproductrow(id)
{
	...
	productprice_actual = productprice_actual - 1; //edited here
	//productprice_hidden = productprice_actual - 1;
	...
	for(i=1;i<=(rowcount+1);i++)
	{
		...
		if($(productrowid).length > 0)
		{
			...
			//$("#productgroup"+ i).attr("id","productgroup"+ countval);
...
function removedescriptionrows()
{
	...
	var toremove = 'false';
    $("[name*=descriptiontype]").each(function (index, value) { 
      if($(this).val() == 'percentage' || $(this).val() == 'amount' )
      {
          toremove = 'true';
      }
    });
...
function newproductentry()
{
	...
	$("#seletedproductgrid").append('<tr class="tr-grid-header"><td width="9%" nowrap="nowrap" class="td-border-grid">Sl No</td><td width="27%" nowrap="nowrap" class="td-border-grid">Product</td><td width="15%" nowrap="nowrap" class="td-border-grid">Buy Type</td><td width="13%" nowrap="nowrap" class="td-border-grid">Usage Type</td><td width="10%" nowrap="nowrap" class="td-border-grid">Qty</td><td width="15%" nowrap="nowrap" class="td-border-grid">Listed Price</td><td width="15%" nowrap="nowrap" class="td-border-grid">Unit Price</td><td width="11%" nowrap="nowrap" class="td-border-grid">Remove</td></tr><tr><td colspan="8" nowrap = "nowrap" class="td-border-grid" id="messagerow"><div align="center" style="height:15px;"><strong><font color="#FF0000">Select a Item First</font></strong></div></td></tr>');
...
function calculatenormalprice(command)
{
	...
	if(isNaN(totalamount))
	{
		...
		//$('#sbtaxamount').val('0');	
		$('#netamount').val('0.00');
		$('#igsttaxamount').val('0.00');
		$('#cgsttaxamount').val('0.00');
		$('#sgsttaxamount').val('0.00');
	}
	if(productamount == 0)
	{
		$('#totalamount').val('0.00');		
...
function showhidepaymentdiv()
{
	...
	//alert($('#databasefield1').is(':checked'));
	if($('#databasefield2').is(':checked'))
	{
		$("#paymentmadenow").show();
		$("#paymentdiv").show();
		$('#databasefield1').removeAttr('checked');
		$('#databasefield1').attr("disabled", true);
		$('#databasefield8').removeAttr('checked');
		$('#databasefield8').attr("disabled", true);

		$('#databasefield2').attr("disabled", false);
		$('#upimode').val('');
		$('#cardmode').val('');
	}
	else if($('#databasefield1').is(':checked') || $('#databasefield8').is(':checked'))
	{
		$("#paymentmadenow").hide();
		$("#paymentdiv").hide();
		$('#databasefield2').removeAttr('checked');
		$('#databasefield2').attr("disabled", true);

		$('#databasefield1').attr("disabled", false);
		$('#databasefield8').attr("disabled", false);

		if($('#databasefield8').is(':checked') && $('#databasefield1').is(':checked'))
		{
			$('#cardmode').val('card');
			$('#upimode').val('upi');
		}
		else if($('#databasefield8').is(':checked'))
		{
			$('#upimode').val('upi');
			$('#cardmode').val('');
		}
		else if($('#databasefield1').is(':checked'))
		{
			$('#cardmode').val('card');
			$('#upimode').val('');
		}
...
function getdealerdetails()
{
	...
	var passData = "switchtype=getdealerdetails&dealerid="+ encodeURIComponent($('#dealer').val()) + "&customer_hidden_id=" + customer_hidden_id + "&customer_gstno=" + encodeURIComponent($('#displaycustomergst').val()) +"&customer_gst=" + encodeURIComponent($('#customer_gst').val());
	//alert(passData);
	var queryString = "../ajax/invoicing.php";
...
function paynow(onlineslno)
{
	...
	$('#submitform').attr("action", "http://imax.relyonsoft.com/user/makepayment/paymode.php") ;
	$('#submitform').attr( 'target', '_blank' );
	$('#submitform').submit();
}

// function getconfirmsg(code,msg)
// {
...
function proceedforpurchase(getMessage)
{
	//alert(getMessage);
	var form = document.getElementById('submitform');
	//alert(form.modeofpayment);
	var modeofpayment = [];
	$("input:checkbox[name='modeofpayment']:checked").each(function(){    
		modeofpayment.push($(this).val());    		
	});
	// alert(modeofpayment.join(", "));
	// return false;
	if(getMessage!= 'gstconfirm')
	  ...
	  if(confirmation || getMessage == 'gstconfirm')
	  {
		...
		//   var paymenttype = getradiovalue(form.modeofpayment);
		  var paymenttype = modeofpayment.join(",");
		  var inclusivetaxamount = document.getElementById('inclusivetaxamount').value;
		  var offeramount = document.getElementById('offeramount').value;
		 ...
		 var productlist = productvalues;//productvalues.substring(0,(productvalues.length-1));//alert(productamountvalues);
		  ...
		  if($('#databasefield1').is(':checked') || $('#databasefield8').is(':checked'))
		  {
			  disableproceedbutton();
		  }
		  else
...
function previewinvoice() //chnaged for gst
{
	...
	if(field.val()) { if(emailvalidation(field.val()) == 'false') { alert('Enter the valid Email Id.'); field.focus(); return false; } }
	...
	{ 
		...
		{ 
			...
			{ 
				...
				{ 
					alert('State GST Code Not Matching.'); field.focus();return false; 
				} }
			}
		}
	...
	//alert($("input:checkbox[name='modeofpayment']:checked").length);
	if (!$("input:checkbox[name='modeofpayment']:checked").length) {
		alert('Select atleast one payment type.'); return false;
	}
	...
	{	
		if($("#itemtype"+ j).val() == 'product')
		{
			purchasearrray[i] = 'purchasetypehidden' + j;
			purchasevalues[i] = document.getElementById(purchasearrray[i]).value;
			purchasearr.push(purchasevalues[i]);
			usagearrray[i] = 'usagetypehidden' + j;
			usagevalues[i] = document.getElementById(usagearrray[i]).value;
...
//	return false;
	...
	if(minutes < 10)
		minutes = "0" + minutes;
	var currentdate1 = year + "-" + (month+1) + "-" + day; 
	//alert(currentdate1);
	var currentdate2 = new Date(currentdate1.split('-')[2],currentdate1.split('-')[1],currentdate1.split('-')[0]);
	var expiredate1 = '2022-4-2';
	var expiredate2 = new Date(expiredate1.split('-')[2],expiredate1.split('-')[1],expiredate1.split('-')[0]);
	...
	if(invoicedated == 'yes')
	{
		...
		if(t1 > t2)
			$('#invoicedatepreview').html('27-03-2022 (23:55)');
		else
	...
	if(productlist != '')
	{
		...
		for(i=0;i<productcount.length;i++)
		{
			...
			//if(productcount[i] == '111')	
			//alert(productgrouparr[i]);
	...
	if(servicelist != '')
	{
		...
		for(i=0;i<servicelistcount.length;i++)
		{
			...
			k++; 
			var productgrid = '<tr><td width="9%" class="grey-td-border-grid" style="">' + k +'</td> <td colspan="2" nowrap="nowrap" class="grey-td-border-grid" style="text-align: left;">'+ servicelistcount[i] +'<br/><font color="#000"><strong>Item Description </strong></font>: '+itemdescriptiontype+' / <font color="#000"><strong>SAC </strong></font> : Information Technology Services</td><td width="19%" nowrap="nowrap" style="text-align: right;" class="grey-td-border-grid">'+ intToFormat(serviceamountvaluessplit[i]) +'</td> </tr>';
	...
	{
	  var amountgrid = '<tr><td colspan="2" class="grey-td-border-grid">&nbsp;</td><td valign="top" class="grey-td-border-grid"  width="20%" style="border-left:none"><div align = "right">Net Amount</div></td><td nowrap="nowrap" style="text-align: right;" class="grey-td-border-grid" valign="top" ><span id="netamountpreview"></span></td></tr><tr><td colspan="2" class="grey-td-border-grid"  style="border-right:none"><span style="font-size:7px;color:#FF0000;text-align:left" >'+sezremarks+'</span></td><td valign="top" class="grey-td-border-grid" width="20%"  style="border-left:none"><div align = "right">IGST @ '+igst_rate+'%</div></td><td nowrap="nowrap" style="text-align: right;" class="grey-td-border-grid" id="igstpreview">&nbsp;</td><td valign="top" class="grey-td-border-grid" width="20%"  style="border-left:none; display:none"><div align = "right">Service Tax @ 14%</div></td><td nowrap="nowrap" style="text-align: right;display:none" class="grey-td-border-grid" id="servicetaxpreview">&nbsp;</td></tr><tr style="display:none;"><td colspan="2" class="grey-td-border-grid"  style="border-right:none"></td><td valign="top" class="grey-td-border-grid" width="20%"  style="border-left:none;display:none"><div align = "right">SB Cess @ 0.5%</div></td><td nowrap="nowrap" style="text-align: right;display:none" class="grey-td-border-grid" id="sbtaxpreview">&nbsp;</td></tr><tr style="display:none;"><td colspan="2" class="grey-td-border-grid"  style="border-right:none"></td><td valign="top" class="grey-td-border-grid" style="border-left:none;display:none"><div align = "right">KK Cess @ 0.5%</div></td><td nowrap="nowrap" style="text-align: right;display:none" class="grey-td-border-grid" id="kk_cesstaxpreview">&nbsp;</td></tr><tr><td colspan="2" class="grey-td-border-grid"  style="border-right:none"></td><td valign="top" class="grey-td-border-grid" style="border-left:none"><div align = "right">Total</div></td><td nowrap="nowrap" style="text-align: right;" class="grey-td-border-grid" id="totalamountpreview">&nbsp;</td></tr><tr><td colspan="4" class="grey-td-border-grid" style="border-right:none">Rupee In Words: <span id="rupeeinwordspreview"></span></td> </tr>';
	  $("#previewproductgrid").append(productgridheader+productgridrow+emptyrow+amountgrid);
...
function getpaymentremarks()
{
	...
	if($('#databasefield8').is(':checked') && $('#databasefield1').is(':checked'))
		paymentremarks = 'Selected for Credit/Debit Card/UPI Payment.This is subject to successful transaction.';
	else if($('#databasefield1').is(':checked'))
