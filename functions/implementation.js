...
{
	var selectbox = $("#customerlist option:selected").val(); //alert(selectbox);
...
function invoicedetailsdisplay(cusid)
{
	if(cusid != '')
	{
		...
		ajaxcall1 = $.ajax(
		{
			...
			error: function(a,b)
			{
				$('#invoice_error').html(scripterror());
			}
		});	
	}
}
		...
		ajaxcall1 = $.ajax(
		{
			...
			{	
				...
				if(response['errorcode'] == '1')
				{
					...
					$('#imp_statustype').val(response['implementationtype']);
					$('#imptype_remarks').val(response['impremarks']);
					// if($('#imp_statustype option:selected').text() == "Others")
					// {
					// 	$('#remarksdiv').show();
					// 	$('#imptype_remarks').val(response['impremarks']);
					// }
					/*autochecknew($("#collectedamt"),response['advancecollected']);
					checkboxvalidation('displaypayment');
					$("#paymentamt").val(response['advanceamount']);
					$("#paymentremarks").val(response['advanceremarks']);*/
					...
					var rowcount = $('#adddescriptionrows tr').length;
					$('#adddescriptionrows tr').remove();
					$('#finalinvoice').val(response['invoicenumber']);
					var row = '<tr><td width="21%" ><strong>Selected Invoice: </strong></td><td width="20%"><input type="text" name="invoicedetails"  class="swiftselect-mandatory" disabled="disabled" id="invoicedetails" size="30"  value="'+ response['invoicenumber']+'"  /></td><td width="59%"><div style="text-align:left"><a onclick="invoicedetailsdisplay(\'' + response['customerreference'] + '\');"  class="r-text">Change &#8250;&#8250;</a></div></td></tr><tr ><td colspan="3">'+ response['grid']+'</td> </tr>';
					$("#adddescriptionrows").append(row);*/
		...
		{	
			...
			if(response['errorcode'] == '1')
			{
				...
				if(response['count'] == 0)
				{
					...
					var row = '<tr><td width="21%" ><strong>Selected Invoice: </strong></td><td width="20%"><input type="text" name="invoicedetails"  class="swiftselect-mandatory" disabled="disabled" id="invoicedetails" size="25"  value="'+ response['invoiceno']+'"  /></td><td width="59%"><div style="text-align:left"><a  onclick="invoicedetailsdisplay(\''+response['customerid']+'\');"  class="r-text">Change &#8250;&#8250;</a></div></td></tr><tr ><td colspan="3">'+ response['grid']+'</td> </tr>';
					$("#adddescriptionrows").append(row);
				}
				else
				{
...
function formsubmit(command,type)
{
	...
	if($("#imp_statustype").val()!= '2' && $("#imp_statustype").val()!= '5' && $("#imp_statustype").val()!= '7' && $("#imp_statustype").val()!= '10')
	{
		...
		if(!field.val()) { alert("Enter the Purchase Order Date"); field.focus();field.scroll(); return false; }
		// var field = $("#po_link" );
	...
	if(customization == 'yes')
	{
		...
		var field = $("#customization_sppdata" );
		if(!field.val()) { alert("Please Attach SPP Data Backup Files"); field.focus();field.scroll(); return false; }
	}
	...
	if(web_implementation == 'yes')
	{
		...
		{
			passData =  "switchtype=approve&customerreference=" + encodeURIComponent($('#customerid').val()) + "&lastslno=" + encodeURIComponent($('#lastslno').val()) + "&dummy=" + Math.floor(Math.random()*100000000);//alert(passData)
		}
		else
		{
		...
		ajaxcall1 = $.ajax(
		{
			...
			{	
				...
				else
				{
					...
					if(type == 'approve')
					{
						$('#lastslno').val(response[2]);
						return false;
						
					}
					if(response[0] == '1' )
					{
						implemenationstatus(response[2]);
						generateraffilegrid('');
						$('#lastslno').val(response[2]);
						alert(response[1]);
					}
					...
					{
						$('#form-error').html(errormessage('Unable to Connect....' + ajaxcall0.responseText));
					}
				}
		...
		ajaxcall1 = $.ajax(
		{
			...
			{	
				...
				else
				{
					...
					{
						error.html(successmessage(response[1]));
						$('#rafslno').val(response[3]);
						$('#deleterafslno').val(response[2]);
						rafnewentry();
						generateraffilegrid('');
					}
				}
	...
	ajaxcall1 = $.ajax(
	{
		...
		{	
			...
			else
			{
				...
				if(response['errorcode'] == '1')
				{
					if(response['branchapproval'] == 'no' && response['coordinatorreject'] == 'no' && response['coordinatorapproval'] == 'no' && response['branchreject'] == 'no' && response['implementationstatus'] == 'pending')
					{
...
function checkvalidate()
{
	autochecknew($('#collectedamt'),'no');
	$('#paymentamt').attr('disabled',true);	
	$('#paymentremarks').attr('disabled',true);
	$('#paymentamt').val('');
	$('#paymentremarks').val('');
	$('#linkdisplay').hide();	
...
{
	var error = $('#form-error');
	var passData = "switchtype=appovedmail&lastslno=" + encodeURIComponent(lastslno)+ "&dealerid=" + ($('#dealeridhidden').val());//alert(passData)
	...
	ajaxcall1993 = $.ajax(
	{
		...
		{	
			...
			{
				var response1 = ajaxresponse.split('^');
				if(response1[0] == '1')
				{
					return true;
				}
...
{
	var error = $('#form-error');
	var passData = "switchtype=updatedmail&lastslno=" + encodeURIComponent(lastslno)+ "&dealerid=" + ($('#dealeridhidden').val());
	error.html(getprocessingimage());//alert(passData)
	var queryString = "../ajax/implementation.php";
	ajaxcall1991 = $.ajax(
	{
		...
		{	
			...
			else
			{
				...
				if(response1[0] == '1')
				{
					if(type == 'manual')
					{
						$('#error-'+type).html(successmessage(response1[1]));
					}
...
function getdealerdetails(type)
{
	var form = $('#submitform');
	if(type == 'selectdealer')
	{
		var field = $('#dealer');
		if(!field.val())
		{
...
//	$('#displaydealerdetails').html(getprocessingimage());
	...
	ajaxcall411.onreadystatechange = function()
	{
		if(ajaxcall411.readyState == 4)
		{
			if(ajaxcall411.status == 200)
				{
					var ajaxresponse = ajaxcall411.responseText;//alert(ajaxresponse)
					if(ajaxresponse == 'Thinking to redirect')
					{
						window.location = "../logout.php";
						return false;
	...
	else
	{
		...
		$('#displaydealerdetails').html('');
		$('#displaymarketingexe').html('');
	}
}
...
function approvedetails(type)
{
	...
	passData =  "switchtype=approve&customerreference=" + encodeURIComponent($('#customerlist',form2).val()) + "&lastslno=" + encodeURIComponent($('#lastslno',form).val())+ "&remarks=" + encodeURIComponent(remarks)+ "&type=" + encodeURIComponent(type)+ "&advremarks=" + encodeURIComponent(advremarks)+ "&dealerid=" + ($('#dealeridhidden').val()) + "&dummy=" + Math.floor(Math.random()*100000000);//alert(passData)
	queryString = '../ajax/implementation.php';
	error.html(getprocessingimage());
	ajaxobjext666 = $.ajax(
	{
		...
		{	
			...
			else
			{
				...
				if(response1['errorcode'] == '1')
				{
					...
					var row = '<tr><td width="21%" ><strong>Selected Invoice: </strong></td><td width="20%"><input type="text" name="invoicedetails"  class="swiftselect-mandatory" disabled="disabled" id="invoicedetails" size="25"  value="'+ response1['invoicenumber']+'"  /></td><td width="59%"><div style="text-align:left"><a onclick="invoicedetailsdisplay(\'' + response1['customerreference'] + '\');"  class="r-text">Change &#8250;&#8250;</a></div></td></tr><tr ><td colspan="3">'+ response1['grid']+'</td> </tr>';
					$("#adddescriptionrows").append(row);
				...
				// else if(response1['errorcode'] == '3'){
				// 	$('#displayinvoicediv').show();
				// 	$('#displayinvoicecfdiv').hide();
				// 	$("table#adddescriptionrows tr:nth-child(2)").remove();
				// 	autochecknew($("#collectedamt"),response1['advancecollected']);
				// 	$("#cfentrydiv").show();
				// 	$("#invoiceentrydiv").show();
				// 	$("#receiptdisplay").html('');
