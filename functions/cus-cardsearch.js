	...
	ajaxcall1 = $.ajax(
	{
		...
		{	
			...
			else
			{
				...
				{
				//form.cardcount.value = response;alert(response);
...
function carddetailstoform(cardid)
{
	...
	{	
		...
		var passData = "switchtype=cardsearchdetailstoform&cardlastslno=" + encodeURIComponent(cardid) + "&dummy=" + Math.floor(Math.random()*100032680100);//alert('here')
		$('#cardselectionprocess').html(getprocessingimage());
...
function selectcardfromlist()
{
    ...
    carddetailstoform(selectbox);//alert(selectbox);
}
	...
	else
	{
		...
		for( var i=0; i < cardarray.length; i++)
		{
			...
			if(pattern.test(comparestring.toLowerCase()))
			{
				...
				//carddetailstoform($("#scratchcardlist option:eq(0)").val()); //document.getElementById('delaerrep').disabled = true;
...
{
	document.getElementById('actiontype1').disabled = true;
	document.getElementById('actiontype2').disabled = true;
}
function enableaction()
{
	document.getElementById('actiontype1').disabled = false;
	document.getElementById('actiontype2').disabled = false;
}
...
function newentry()
{
	...
	var form = document.cardsearchfilterform;
	enableaction();
	document.getElementById('form-error').innerHTML = '';
	document.getElementById('scratchnumber').value = '';
	document.getElementById('remarks').value = '';
	document.getElementById('cardattached').innerHTML = '';
	document.getElementById('cardregistered').innerHTML = '';
	document.getElementById('cardstatus').innerHTML = '';
	document.getElementById('cardsearchtext').value = '';
