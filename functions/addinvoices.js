...
function gettotalcustomercount()
{
	...
	queryString = "../ajax/addinvoices.php";
	ajaxcall1 = $.ajax(
	{
...
function customerdetailstoform(cusid)
{
	...
	if(cusid != '')
	{
		...
		var queryString = "../ajax/addinvoices.php";
		ajaxcall2 = $.ajax(
		{
			type: "POST",url: queryString, data: passData, cache: false,dataType: "json",
			success: function(ajaxresponse,status)
			{	
				...
				else
				{
					...
					if(response['errorcode'] == '1')
					{
						...
						$("#emailhidden").val(response['emailidplit']);
						$("#phonehidden").val(response['phone']);
						$("#cellhidden").val(response['cell']);
		...
		{	
			...
			else
			{
				...
				else
				{
					...
					customerdetailstoform(response['slno']);
				}
			}
		}, 
		error: function(a,b)
...
function addselectedproduct(command)
{
	...
	var cardenteredhidden  = 'cardenteredhidden'+k;
	...
	var cardidentered = 'cardidentered'+k;
	var productname =$("#product option:selected").text();
	var productvalue =  $("#product").val();
	var cardvalue =  $("#cardidentered").val();
	...
	if(field.val() != '')
	{
		productgrid = '<tr id=\'' + productrowid + '\'><td nowrap="nowrap" class="td-border-grid" id=\'' + productrowslnoid + '\' >' + k + '</td><td nowrap="nowrap" class="td-border-grid">' + productname + ' <input name="productselectedhidden[]" class="swiftselect" id=\'' + productselectedhidden + '\' type = "hidden" value = ' + productvalue + '><input name="productnamehidden[]" class="swiftselect" id=\'' + productnamehidden + '\' type = "hidden" value = \'' + productname + '\' ><div style="padding-top:5px"><input type="text" value="Description (Optional)" style="width:200px;" autocomplete="off" maxlength="30" id=\'' + productleveldescription + '\' class="swifttext description-blur" onblur="descriptionblur(\'' + productleveldescription + '\')" onfocus="descriptionfocus(\'' + productleveldescription + '\')" name= "productleveldescriptiontype[]"></div><div style="padding-top:5px"><strong>PIN slno:</strong> <input type="text" value="" style="width:100px;" autocomplete="off" maxlength="6" id=\'' + cardidentered + '\' class="swifttext" name= "cardidenteredlist[]"><input name="cardenteredhidden[]" class="swiftselect" id=\'' + cardenteredhidden + '\' type = "hidden" value = ""></div></td><td valign="top"  class="td-border-grid"> <div align="center"><span id=\'' + editpurchasetypehtml + '\' align = "center">New </span><br/><span id= "editdiv' + productvalue + '" style="display:block;" align = "center"><a id="' + editpurchasetypelinkid + '" style="cursor:pointer" onclick = "editamountonpurchasetype(\'' + editpurchasetypehtml + '\',\'' + purchasetypehidden + '\')" class = "r-text">( Change )</a> </span><input name="purchasetypehidden" class="swiftselect" id=\'' + purchasetypehidden + '\' type = "hidden" value = "new" ><input name="itemtype" class="swiftselect" id=\'' + itemtype + '\' type = "hidden" value = "product"></div></td><td valign="top" class="td-border-grid"> <div align="center"><span id=\'' + editusagetypehtml + '\' align = "center">Single User</span><br/><span id= \'' + editusagetypeid + '\' style="display:block;" align = "center"><a id="' + editusagetypelinkid + '" style="cursor:pointer" onclick = "editamountonusagetype(\'' + editusagetypehtml + '\',\'' + usagetypehidden + '\')" class="r-text">( Change )</a> </span><input name="usagetypehidden" class="swiftselect" id=\'' + usagetypehidden + '\' type = "hidden" value = "singleuser" ></div></td><td nowrap="nowrap" class="td-border-grid" ><div align="center"><select name="productquantity" class="swiftselect" id=\'' + productquantity + '\' style="width:50px;" onchange ="calculatenormalprice();" onkeypress="calculatenormalprice();"><option value="1" selected="selected">1</option></select></div></td><td nowrap="nowrap" class="td-border-grid" ><input name="productamount" type="text" class="swifttext" id=\'' + productamount + '\'  maxlength="6"  autocomplete="off" style="width:80px;text-align:right" onkeyup ="calculatenormalprice();" onchange ="calculatenormalprice();"/></td><td nowrap="nowrap" class="td-border-grid" ><div align ="center"><font color = "#FF0000"><strong><a id="' + removelinkid + '" onclick = "deleteproductrow(\'' + k + '\');" style="cursor:pointer;" >X</a></strong></font></div></td></tr>';
	}
	else
	{
...
function deleteproductrow(id)
{
	...
	for(i=1;i<=(rowcount+1);i++)
	{
		...
		var cardidentered = '#cardidentered'+i;
		var cardenteredhidden = '#cardenteredhidden'+i;
		...
		if($(productrowid).length > 0)
		{
			...
			if($("#productleveldescription"+ i).length > 0)
			{
				$("#productleveldescription"+ i).attr("id","productleveldescription"+ countval);
				document.getElementById("productleveldescription" + countval).onfocus = new Function('descriptionfocus("productleveldescription' + countval + '")') ;
				document.getElementById("productleveldescription" + countval).onblur = new Function('descriptionblur("productleveldescription' + countval + '")') ;
				$("#cardidentered"+ i).attr("id","cardidentered"+ countval);
				$("#cardenteredhidden"+ i).attr("id","cardenteredhidden"+ countval);
				
			}
			if($("#itemleveldescription"+ i).length > 0)
			{
				$("#itemleveldescription"+ i).attr("id","itemleveldescription"+ countval);
				document.getElementById("itemleveldescription" + countval).onfocus = new Function('descriptionfocus("itemleveldescription' + countval + '")') ;
				document.getElementById("itemleveldescription" + countval).onblur = new Function('descriptionblur("itemleveldescription' + countval + '")') ;
			}
...
function proceedforpurchase()
{
	...
	if(confirmation)
	{
		...
		var field = $('#regiontype');
		if(!field.val())
		{
			alert('Select the Region in Invoice.');
			field.focus(); return false;
		}
		var field = $('#invoiceno');
		if(!field.val())
		...
		if(!validateamountfield(field.val())) {
		...
		var cardenteredvalues = new Array();
		var cardenteredarrray  = new Array();
		for(i=0,j=1; i<count; i++,j++)
		{	
			if($("#itemtype"+ j).val() == 'product')
			{
				...
				productamountarrray[i] = 'productamount' + j;
				productamountvalues[i] = document.getElementById(productamountarrray[i]).value;
				cardenteredarrray[i] = 'cardidentered' + j;
				cardenteredvalues[i] = document.getElementById(cardenteredarrray[i]).value;
				var field = document.getElementById(productamountarrray[i]);
				if(!field.value) { 
					$('#form-error').html('');
					alert('Please Enter the amount');
	...
	//	return false;
		...
		var field = $('#generatedby');
		if(!field.val())
		{
			alert('Please Select the Generated by'); field.focus(); return false;
		}
		$('#proceedprocessingdiv').html(getprocessingimage());
...
function previewinvoice()
{
	...
	var cardserialarrray = new Array();
	var cardserialvalues = new Array();
	for(i=0,j=1; i<count; i++,j++)
	{	
		if($("#itemtype"+ j).val() == 'product')
		{
			...
			cardserialarrray[i] = 'cardidentered' + j;
			cardserialvalues[i] = document.getElementById(cardserialarrray[i]).value;
			var field = document.getElementById(cardserialarrray[i]);
			if(!field.value) { 
				$('#form-error').html('');
				alert('Please Enter PIN Serial Number');
				field.focus(); return false;}
			else if(field.value)	{ 
			if(!validatepinnofield(field.value)) {
				$('#form-error').html('');
				alert('Pin Serial Number is not Valid');
				field.focus(); return false; } }
			var field = document.getElementById(productamountarrray[i]);
			if(!field.value) { 
				$('#form-error').html('');
	...
	if(descriptiontypearrray != '')
	{
		...
		descriptionamountarrraysplit = descriptionamountarrray.split('~');
		descriptionamountvaluessplit = descriptionamountvalues.split('~');
		//cardserialarraysplit = cardserialarray.split('~');
		for(var k=0;k<descriptiontypesplit.length;k++)
		{
			if(descriptiontypevaluesplit[k] != '')
			{
...
//	alert($("#customeridpreview").html());
	$("#companynamepreview").html($("#displaycompanyname").val());
	$("#customeridpreview").html($("#customerid").val());
	//alert('RSL/' + $("#invoiceno").val());
	var invoicenovalue = 'RSL/' +  $("#regiontype").val() + '/' + $("#invoiceno").val()
	$("#invoicenopreview").html(invoicenovalue);
	...
	//	document.write(month + "/" + day + "/" + year)
		...
		else
		{
			...
			if(minutes < 10)
				minutes = "0" + minutes;
		//	$('#invoicedatepreview').html(" " +day + "-" + month + "-" + year + " (" + hours + ":" + minutes + ")");
			$('#invoicedatepreview').html($('#DPC_invoicedate').val());
		}
	k=0;
	...
	if(productlist != '')
	{
		...
		var cardserialvalues = removedoublecomma(cardserialvalues);
		var usagevalues = removedoublecomma(usagevalues);
		var purchasevalues = removedoublecomma(purchasevalues);
	...
	var field = $('#displaycustomerid');
	if(!field.html())
	{
		alert('Please Enter  as "New customer" as the customer id is not there for the present select customer'); field.focus(); return false;
	}
	...
	$("#generatedbypreview").html(document.getElementById('generatedby').options[document.getElementById('generatedby').selectedIndex].text);
...
function formsubmitcontact(command)
{
	...
	if(!field.val()) { error.html(errormessage("Enter the Customer ID. ")); field.focus(); return false; }
	if(field.val())	{ if(!validatecustomerid(field.val())) { error.html(errormessage('Enter the valid Customer ID with hypen separated.')); field.focus(); return false; } }
	$('#cuslastslno').val(' ');
		var passdata = "";
		if(command == 'save')
		{
			passdata =  "switchtype=contactsave&businessname=" + encodeURIComponent($("#businessname" ).val())  + "&address=" + encodeURIComponent($('#address').val()) + "&place=" + encodeURIComponent($('#place').val()) + "&pincode=" + encodeURIComponent($('#pincode').val()) + "&district=" + encodeURIComponent($('#district').val())  + "&category=" + encodeURIComponent($('#category').val()) + "&type=" + encodeURIComponent($('#type').val()) + "&stdcode=" + encodeURIComponent($('#stdcode').val()) + "&fax=" + encodeURIComponent($('#fax').val()) + "&website=" + encodeURIComponent($('#website').val()) + "&remarks=" + encodeURIComponent($('#remarks').val())  + "&lastslno=" + encodeURIComponent($('#cuslastslno').val()) + "&contactarray=" + encodeURIComponent(contactarray)+ "&totalarray=" + encodeURIComponent(totalarray) + "&region=" + encodeURIComponent($("#region").val()) + "&branch=" + encodeURIComponent($("#branch").val()) + "&currentdealer=" + encodeURIComponent($("#currentdealer").val())+ "&customerid=" + encodeURIComponent($("#customerid").val())+ "&dummy=" + Math.floor(Math.random()*100000000);//alert(passdata)
		...
		ajaxcall5690 = $.ajax(
			{
				...
				{	
					...
					{
						error.html(errormessage(response['successmessage']));
					}
					else
					{
...
function validatepinnofield(amount)
{
	var numericExpression = /^\s*(?:[0-9]{6})\s*$/;
