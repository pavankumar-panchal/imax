...
function formsubmit(command,getMessage)
{
	...
	if(command == 'edit')
	{
		...
		if(typeof($('#sbtax').val()) == 'undefined') {
		    sb_taxvalue = 0;
		}
		else {
		    sb_taxvalue = $('#sbtax').val();
		}
		if(typeof($('#kktax').val()) == 'undefined') {
		    kk_taxvalue = 0;
		}
		else {
		    kk_taxvalue = $('#kktax').val();
		}
		if(typeof($('#sevicetax').val()) == 'undefined') {
		    service_taxvalue = 0;
		}
		else {
		    service_taxvalue = $('#sevicetax').val();
		}
		...
		"&productamountvalues=" + encodeURIComponent(productamountvalues) + "&descriptiontypevalues=" + 
	...
	else if(command == 'cancel' || command == 'einvcancel')
	{
		...
		if(command == 'cancel')
			passData =  "switchtype=cancelinvoice&invoiceno=" + encodeURIComponent(invoiceno)+ "&cancelremarks=" + encodeURIComponent($('#cancelremarks').val()) + "&cancelreason=" + encodeURIComponent($('#cnlrsn').val()) + "&productlist=" + encodeURIComponent(productlist)  + "&dummy=" + Math.floor(Math.random()*10000000000);
		else if(command == 'einvcancel')
			passData =  "switchtype=canceleinv&invoiceno=" + encodeURIComponent(invoiceno)+ "&cancelremarks=" + encodeURIComponent($('#cancelremarks').val()) + "&cancelreason=" + encodeURIComponent($('#cnlrsn').val()) + "&productlist=" + encodeURIComponent(productlist) + "&cancelconfirm=" + encodeURIComponent(getMessage) +  "&dummy=" + Math.floor(Math.random()*10000000000);
...
function resendmatrixinvoice(invoiceno)
{
	var form = $('#submitform');
	if(invoiceno == '')
	{
...
function getpercentvalue(){
    ...
    $("[name*=productamount]").each(function (index, value) { 
      amount_totals = amount_totals + parseInt($(this).val());
    });
   
    $("[id*=descriptiontype]").each(function (index, value) { 
          counters_add++;
          var inner_text = $.trim($(this).text());
          if(inner_text == 'ADD')
          {
              amount_totals = amount_totals + parseInt($('#descriptionamount'+counters_add).val());
          }
          if(inner_text == 'PERCENTAGE')
          {
              rowval++;
          }
    });
  
    //var discountval = amountval;
    var percent = $('#discountpercent').val();
    if(percent == '' || percent == 0)
    {
        var val = $('#description'+rowval).val();
        var old_percent = val.substr(val.indexOf(" ") + 1);
        var old_percentage = old_percent.split("%");
        //alert(old_percentage[0]);
        var valuecalulation = ((old_percentage[0]/100)*amount_totals).toFixed(2);
    }
    else
    { 
        var valuecalulation = ((percent/100)*amount_totals).toFixed(2);
    }
    
    $('#descriptionamount'+rowval).val(valuecalulation);
