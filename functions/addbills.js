...
function productformsubmit(command)
{
	...
	var field = $("#pinno");//alert(form.scheme.value);
	if(!field.val()) { error.html(errormessage('Enter the Pin No.')); field.focus(); return false; }
	if(field.val())	{ if(!validatepinno(field.val())) { error.html(errormessage('Pin No is Not valid.')); field.focus(); return false; } }
	var field = $("#DPC_attacheddate");
	if(!field.val()) { error.html(errormessage('Enter the Card Attached date')); field.focus(); return false; }

	var passData = "";
	if(command == 'save')
	{
		passData =  "type=productsave&cusbillnumber=" + $("#cusbillnumber").val() + "&productcode=" + $("#productcode").val() + "&productquantity=" + $("#productquantity").val() + "&productamount=" + $("#productamount").val() + "&productlastslno=" + $("#productlastslno").val() + "&usagetype=" + $("#usagetype").val() + "&dealerid=" + $("#dealerlist").val() + "&purchasetype=" + $("#purchasetype").val() + "&scheme=" + $("#scheme").val() +  "&billdate=" + $("#DPC_attacheddate").val() + "&pinno=" + $("#pinno").val()+ "&dummy=" + Math.floor(Math.random()*100000000);//alert(passData)
	...
	if(confirmation)
	{
		passData =  "type=productdelete&productlastslno=" + $("#productcode").val() + "&dummy=" + Math.floor(Math.random()*10000000000);
	}
		else
		return false;
	}
			...
			else
			{
				...
				if(response[0] == '1')
				{
					if($("#cardslnolist").val() == '')
						...
						var appendcomma = ', ';
					$("#cardslnolist").val($("#cardslnolist").val() + appendcomma + response[3]);
...
function productgridtoform(slno)
{
	if(slno != '')
	{
		...
		var queryString = "../ajax/addbills.php";
		ajaxcall6 = $.ajax(
		{
			type: "POST",url: queryString, data: passData, cache: false,dataType: "json",
			success: function(ajaxresponse,status)
...
function viewattachedcards(command)
{
	...
	var passData = "type=viewcards&cusbillnumber=" + encodeURIComponent($("#cusbillnumber").val())+ "&cardslnolist=" +encodeURIComponent($("#cardslnolist").val()) + "&dummy=" + Math.floor(Math.random()*10054300000);//alert(passData)
	error.html(getprocessingimage());	
	queryString = "../ajax/addbills.php";
	ajaxcall12 = $.ajax(
	{
...
function validatepinno(pinno)
{
	var numericExpression = /^[^9]\d{5}(?:(?:([,][\s])[^9]\d{5}))*$/i;
	if(pinno.match(numericExpression)) 
	return true;
	else 
	return false;
}
