...
function refreshinvoicenoarray()
{
	...
	queryString = "../ajax/updatematrixinvoice.php";
	ajaxcall1 = $.ajax(
	{
...
function listinvoicedetails(type,invoiceno)
{
	...
	var queryString = "../ajax/updatematrixinvoice.php";
	ajaxcall3 = $.ajax(
	{
		type: "POST",url: queryString, data: passData, cache: false,dataType: "json",
		success: function(ajaxresponse,status)
...
function formsubmit(command,getMessage)
{
	...
	if(command == 'edit')
	{
		...
		var  igst_taxvalue = cgst_taxvalue = sgst_taxvalue = 0;
		...
		encodeURIComponent(purchasevalues) + 
		...
		encodeURIComponent($('#invoiceremarks').val(),form)+ "&cardlist=" + encodeURIComponent(cardlist)+ 
		"&lastslno=" + encodeURIComponent($('#lastslno').val(),form) +"&igsttaxamount=" + 
		encodeURIComponent(igst_taxvalue,form)  +"&cgsttaxamount=" + encodeURIComponent(cgst_taxvalue,form) +"&sgsttaxamount=" + 
		encodeURIComponent(sgst_taxvalue,form)  
		...
		encodeURIComponent($("#selectdealer").val(),form)+ 
