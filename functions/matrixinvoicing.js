...
function editinvoice(requestid)
{
	...
	ajaxobjext12 = $.ajax(
	{
		...
		success: function(response,status)
		{
			...
			if(response['seztaxtype'] == 'yes')
			{
				$('#seztax').attr("checked",true);
				sezfunc();
			}
			else
			{
				$('#seztax').attr("checked",false);
				sezfunc();
			}
			...
			var hsncode = response['hsncode'].split('#');
			...
			for(k=0;k<description.length;k++)
			{
				...
				for(l=1;l<=quantity[k];l++)
				{
					...
					if(typeof serialnosplit[l-1] == 'undefined')
					{
						serialnosplit[l-1] = '';
					}
    ...
    ajaxcall10 = $.ajax(
    {
        ...
        {	
            ...
            else
            {
				...
				//alert(response['seztaxtype']);
				if(response['seztaxtype'] == 'yes')
				{
					alert("Requested Invoice is of SEZ. So, please edit the details, upload the required attachment and proceed.");
					return false;
				}
				else
				{
...
function previewuserinvoice()
{
   ...
   var field = $('#paymentamount');
	...
	if($('#seztax').is(':checked'))
	{
		...
		if(!field.val()) { error.html(errormessage("Please Attach the SEZ Certificate")); field.focus();field.scroll(); return false; }
