...
{
	var field = $("#lastslno").val();
	if(field == "")
	{
		alert("Pleast select dealer first.");
		return false;
	}
...
function removegrid(el)
{
	...
	if(tbodyRowCount > 1)
	{
		...
		for (row = 1; row <=table.rows.length; row++) 
		{
			...
			table.rows[k].cells[1].getElementsByTagName('select')[0].id = 'product2' + row;
			table.rows[k].cells[2].getElementsByTagName('input')[0].id = 'itemamount' + row;
			k++;
		}
	}
	else
	{
		alert("Minimum of ONE Item detail is mandatory if invoice of purchased pins is already generated or you can continue to generate invoice without adding the item detail.");
		return false;
	}
}
...
{
	var form = $("#submitform");
	var error = $("#dlr-form-error");
	var error1 = $("#dlr-form-error");
	var field = $("#cusbillnumber");
	if(field.val() == "New") { alert("Invoice will be genrated only for exisitng purchase(downloaded pins)."); return false; }
	var field = $("#dealer");
	if(!field.val()) { error.html(errormessage('Select the dealer.')); field.focus(); return false; }
	var field = $("#paymentamount");
	if(!field.val()){ error.html(errormessage('Please enter payment amount.')); field.focus(); return false; }
	var field = $("#paymentremarks");
	if(!field.val()){ error.html(errormessage('Please enter payment details.')); field.focus(); return false; }
	
	var productvalue = $('select[name^="product2"]').val();
	var itemvalue = $('input[name^="itemamount"]').val();
	var field = $("#getitemid").val();
	
	 if((productvalue!= "" || itemvalue!="") || field == 0)
	 {
		var emptyFields = false;
		$('select[name^="product2"]').each(function() {
			if($(this).val()=="")
			{
				$(this).focus();
				emptyFields = true;
		...
		if (emptyFields) {
			error.html(errormessage('Please select item other.'));
			//$(this).focus();
			return false;
		 }
		...
		if (emptyFields) {
			error.html(errormessage('Please enter item amount.'));
			return false;
		}
		...
		if (emptyFields) {
			error.html(errormessage('Item Amount must be digits'));
			return false;
		}
	...
	var passData = "";
	var getProductName = document.getElementsByName("product2[]");
	var itemAmount = document.getElementsByName("itemamount[]");
	if(getProductName.length > 0)
	{
		var servicearray=Array.prototype.slice.call(getProductName);
		var servicelist = servicearray.map((o) => o.value).join("#");

		//alert(servicelist);

		var servicearrayvalues=Array.prototype.slice.call(itemAmount);
		var servicelistvalues = servicearrayvalues.map(x => x.value).join("~");

		//alert(servicelistvalues);
	}
		
    //return false;
	//alert($("#dealer").val());
	passData = "type=generateinv&dealerid=" + $("#dealer").val() + "&dealerpurchasebillno=" + $("#cusbillnumber").val() + "&paymentamount=" + $("#paymentamount").val() + "&paymentremarks=" + $("#paymentremarks").val() + "&invoiceremarks=" + $("#invoiceremarks").val() + "&product2=" + servicelist + "&itemamount=" + servicelistvalues + "&dummy=" + Math.floor(Math.random()*100000000);
	var queryString = "../ajax/bills.php";
	error1.html('<img src="../images/imax-loading-image.gif" border="0" />');
	ajaxcall9 = $.ajax({
		type: "POST", url: queryString,data: passData,cache: false, dataType: "json",
		success: function(ajaxresponse,status)
		{
			...
			{
				var gstinconfirm = confirm("Customer GSTIN is not valid. Do you want to proceed further?");
				if(gstinconfirm)
				{
					//alert('true');
					generateinv('gstinconfirm');
				}
				else
				{
					invoicenewentry();
			...
			else if(response['responsecode'] == 3)
			{
				...
				$('#dlr-form-error').html(errormessage(response['errormsg']));
				invoicenewentry();
			}
		},
		error: function(a,b)
		{
			$('#dlr-form-error').html(scripterror());
		}
	});
}
...
function disableproceedbutton()
{
	...
	$('#generateinvoice').removeClass('swiftchoicebuttonbig');	
	$('#generateinvoice').addClass('swiftchoicebuttondisabledbig');	
...
function enableproceedbutton()
{
	...
	$('#generateinvoice').removeClass('swiftchoicebuttondisabledbig');	
	$('#generateinvoice').addClass('swiftchoicebuttonbig');	
...
function amountChange()
{
	...
	var proactualprice = $('#productrate').val();
	var totalamt = proquantity * proactualprice;
	$('#productamount').val(totalamt);
}
...
function productformsubmit(command)
{
	...
	var field = $("#productrate");
	if(!field.val()) { error.html(errormessage('Enter the Billed Product Unit Cost.')); field.focus(); return false; }
	else
	{
		var passData = "";
		if(command == 'save')
		{
			passData =  "type=productsave&cusbillnumber=" + $("#cusbillnumber").val() + "&productrate=" + $("#productrate").val() + "&productcode=" + $("#productcode").val() + "&productquantity=" + $("#productquantity").val() + "&productamount=" + $("#productamount").val() + "&productlastslno=" + $("#productlastslno").val() + "&usagetype=" + $("#usagetype").val() + "&dealerid=" + $("#dealerlist").val() + "&purchasetype=" + $("#purchasetype").val() + "&scheme=" + $("#scheme").val() +  "&billdate=" + $("#billdate").val() + "&dummy=" + Math.floor(Math.random()*100000000);//alert(passData)
		}
		else
		{
		...
		if(confirmation)
		{
			passData =  "type=productdelete&productlastslno=" + $("#productlastslno").val() + "&cusbillnumber=" + $("#cusbillnumber").val() +  "&dummy=" + Math.floor(Math.random()*10000000000);
		}
			else
			return false;
		}
		...
		{	
			...
			else
			{
				...
				if(response[0] == '1')
				{
					...
					// 	$("#itemamount").attr('disabled', 'disabled');
					// }
				}
				else
				{
			...
			{	
				...
				else
				{
					...
					if(response1['errorcode'] == '1')
					{
						...
						$("#productrate").val(response1['productrate']);
						enableprobuttonsonview();
					}
					else
					{
						$("#form-error").html(errormessage('No datas found to be diaplayed.'));
			...
			{	
				...
				else
				{
					...
					if(response['errorcode'] == '1')
					{
						...
						if(response['invoicecount'] > 0 && response['invoicestatus']!= 'CANCELLED')
						{
							//$("#product2").attr('disabled', false);
							//$("#itemamount").attr('disabled', false);
							$("#getitemid").val('0');
						}
						else if(response['invoicecount'] > 0 && response['invoicestatus'] == 'CANCELLED')
						{
							$("#getitemid").val('1');
						}
						else{
							//$("#product2").attr('disabled', true);
							//$("#itemamount").attr('disabled', true);
							$("#getitemid").val('1');
						}
						$("#userid").val(response['fullname']);
...
function getproductgroup()
{
	//alert("hi");
	$("#prd-form-error").html(getprocessingimage());
	var passData = "type=getproductgroup&productcode=" + encodeURIComponent($("#productcode").val())  + "&dummy=" + Math.floor(Math.random()*10054300000);
	queryString = "../ajax/bills.php";
	ajaxcall8 = $.ajax(
	{
		type: "POST",url: queryString, data: passData, cache: false,dataType: "json",
...
function formsubmit(command)
{
	...
	return false;
		...
		else
		{
			if(command == 'save')
			{
				passData =  "type=save&cusbillnumber=" + encodeURIComponent($("#cusbillnumber").val()) + "&dealerid=" + encodeURIComponent($("#lastslno").val()) + "&billdate=" + encodeURIComponent($("#billdate").val()) + "&remarks=" + encodeURIComponent($("#remarks").val()) + "&total=" + encodeURIComponent($("#total").val()) + "&taxamount=" + encodeURIComponent($("#taxamount").val()) + "&netamount=" + encodeURIComponent($("#netamount").val()) + "&totalcredit=" + encodeURIComponent($("#creditamountdisplay").html()) + "&billlastslno=" + encodeURIComponent($("#billlastslno").val()) + "&dummy=" + Math.floor(Math.random()*10045606700000);
			}
			else
			{
			var confirmation = confirm("Are you sure you want to delete the selected product?");
...
function disablebuttonsonview()
{
	$('#productnew').attr('disabled','disabled');
	$('#productsave').attr('disabled','disabled');
	$('#productdelete').attr('disabled','disabled');
	$('#save').attr('disabled','disabled');
	$('#delete').attr('disabled','disabled');
	$('#viewcards').removeAttr('disabled');
...
function enablebuttonsonview()
{
	$('#productnew').removeAttr('disabled');
	$('#productsave').removeAttr('disabled');
	$('#productdelete').removeAttr('disabled');
...
function enableprobuttonsonview()
{
	$('#productsave').removeAttr('disabled');
	$('#productdelete').removeAttr('disabled');
	$('#save').removeAttr('disabled');
