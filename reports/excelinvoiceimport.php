...
if($flag == '')
{
	$url = '../home/index.php?a_link=invoiceimport'; 
	header("location:".$url);
	exit;
}
elseif($flag == 'true')
{
	...
	$toinvoiceno = $_POST['toinvoiceno'];
	$frominvoiceno = $_POST['frominvoiceno'];
	$fromregiontype = $_POST['fromregiontype'];
	$toregiontype = $_POST['toregiontype'];
	$userid = imaxgetcookie('userid');
	
	if($fromregiontype <> $toregiontype)
	{
		echo('From Region Type and To Region Type should be same.');exit;
	}
	else
	{
	
		$query = "select * from inv_invoicenumbers where inv_invoicenumbers.onlineinvoiceno BETWEEN '".$frominvoiceno."' AND '".$toinvoiceno."' and category ='".$fromregiontype."' order by inv_invoicenumbers.slno;";	
