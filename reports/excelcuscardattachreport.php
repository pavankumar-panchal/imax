...
elseif($flag == 'true')
{
	...
	$scheme = $_POST['scheme'];
		if($todate <> '' && $fromdate <> '')
		{
			...
			$query = "select distinct inv_mas_customer.slno as cusslno, inv_mas_customer.businessname as cusname,inv_mas_district.districtname as districtname,inv_mas_state.statename as statename,inv_mas_dealer.businessname as dealername,inv_mas_scratchcard.cardid as cardid, inv_mas_product.productname as productname,inv_dealercard.cuscardattacheddate as attcheddate,inv_customerproduct.date as registereddate,inv_mas_scratchcard.registered as registered,inv_mas_scratchcard.scratchnumber as scratchnumber,inv_dealercard.usagetype as usagetype,inv_dealercard.purchasetype as purchasetype,inv_mas_users.fullname ,inv_mas_region.category as region ,inv_mas_scheme.schemename as schemename from inv_dealercard 
			...
			WHERE inv_dealercard.slno <> '9449599733' ".$datepiece.$geographypiece.$productcodepiece.$dealeridpiece.$usagetypepiece.$purchasetypepiece.$attachedbypiece.$schemepiece.$registeredpiece." and inv_customerproduct.reregistration = 'no'; ";		
			}
			//echo($query);exit;
...
GROUP_CONCAT(DISTINCT phone) as phone, GROUP_CONCAT(DISTINCT cell) as cell, GROUP_CONCAT(DISTINCT emailid) as emailid from inv_contactdetails where customerid = '".$fetch['cusslno']."'  group by customerid ";
		...
		if($id == 'toexcel')
		{
			...
			$filebasename = "Customer-pinno-attached-details".$localdate."-".$localtime."-".strtolower($fetchres['username']).".xls";
			...
			$eventquery = "Insert into inv_logs_event(userid,system,eventtype,eventdatetime,remarks) values('".$userid."','".$_SERVER['REMOTE_ADDR']."','95','".date('Y-m-d').' '.date('H:i:s')."','excel_customer_pinno_attached_report".'-'.strtolower($fetchres['username'])."')";
		...
		elseif($id == 'view')
		{
			...
			$eventquery = "Insert into inv_logs_event(userid,system,eventtype,eventdatetime,remarks) values('".$userid."','".$_SERVER['REMOTE_ADDR']."','94','".date('Y-m-d').' '.date('H:i:s')."','view_customer_pinno_attached_report')";
			$eventresult = runmysqlquery($eventquery);
