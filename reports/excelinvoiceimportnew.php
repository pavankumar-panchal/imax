...
if($flag == '')
{
	$url = '../home/index.php?a_link=invoiceimportnew'; 
	header("location:".$url);
	exit;
}
elseif($flag == 'true')
{
  ...
  $frominvoicenonew = 'RSL2018R'.$fromstatetype.$frominvoicenonew;
  $toinvoicenonew = 'RSL2018R'.$fromstatetype.$toinvoicenonew;
	...
	else
	{
	    ...
	    echo $query = "select * from inv_invoicenumbers where inv_invoicenumbers.onlineinvoiceno BETWEEN '".$frominvoicenonew."' AND '".$toinvoicenonew."' 
	    and executive_details ='".$frominvoicetype."' and state_info ='".$fromstatetype."' order by inv_invoicenumbers.slno;";	
		...
		$itembriefdescriptionsplit = explode('#',$itembriefdescription);
				...
				#aaded for multiple GSTIN 
        		    ...
        		    $customer_gst_slno = substr($customerid, -5);
        		    $invoice_gst_created = $fetch['createddate'];
        		    
        		    $select_gstin_count = "select count(*) as countgstin from customer_gstin_logs where customer_slno = '".$customer_gst_slno."' and effective_from <= '$invoice_gst_created' order by gstin_id desc";
        	        $fetch_gstin_count = runmysqlqueryfetch($select_gstin_count);
        	     
        	     if($fetch_gstin_count['countgstin'] != 0) 
        	     {
        		    $new_gst_details = "select gst_no from customer_gstin_logs where customer_slno = '".$customer_gst_slno."' and effective_from <= '$invoice_gst_created' order by gstin_id desc";
				...
				if($gst_tax_date < $invoicedate)
				{
				    $invoiceheader = 'InvHeader'.'^'.$category.'^'.$customerid.'^'.$businessname.'^'.$address.'^'.$contactperson.'^'.$emailid.'^'.$phone.'^'.$cell.'^'.$customertype.'^'.$customercategory.'^'.$onlineinvoiceno.'^'.$createddate.'^'.$createdtime.'^'.$dealername.'^'.$regionbranch.'^'.$status.'^'.$remarks.'^'.$podate.'^'.$poref.'^'.$gst_type.'^'.$gst_customer;
				...
				{
				   $invoiceheader = 'InvHeader'.'^'.$category.'^'.$customerid.'^'.$businessname.'^'.$address.'^'.$contactperson.'^'.$emailid.'^'.$phone.'^'.$cell.'^'.$customertype.'^'.$customercategory.'^'.$onlineinvoiceno.'^'.$createddate.'^'.$createdtime.'^'.$dealername.'^'.$regionbranch.'^'.$status.'^'.$remarks.'^'.$podate.'^'.$poref.'^'.$gst_customer;
		...
		//echo $filepath = $_SERVER['DOCUMENT_ROOT'].$addstring.'/filecreated/'.$filebasename;
		//echo $downloadlink = 'http://'.$_SERVER['HTTP_HOST'].$addstring.'/filecreated/'.$filebasename;
		//exit();
...
function customerdetails($customerid)
{
	...
	$query12 ="SELECT inv_mas_customer.slno, inv_mas_customer.customerid as customerid,inv_mas_customer.gst_no as gst_no, inv_mas_customer.businessname as businessname,
	inv_mas_customer.address as address, inv_mas_customer.place as place,inv_mas_customer.pincode as pincode,inv_mas_customer.website as website,
	inv_mas_district.districtname as district,inv_mas_state.statename as state,customer_gstin_logs.gst_no as new_gst_no,customer_gstin_logs.gstin_id 
	...
	left join customer_gstin_logs on customer_gstin_logs.gstin_id = inv_mas_customer.gst_no
	where inv_mas_customer.slno ='".substr($customerid,15,5)."'";
	...
	$custdetails = 'CutomerDetails'.'^'.$customerid.'^'.$fetchresult12['businessname'].'^'.$adds.'^'.$adds1.'^'.$adds2.'^'.$fetchresult12['place'].'^'.$fetchresult12['pincode'].'^'.$fetchresult12['state'].'^'.'India'.'^'.$fetchresult12['website'].'^'.$fetchresult12['new_gst_no'];
